...
def update_cdk_version(_, version):
    ...
    updated_lines = []
    with open(
        os.path.join(idea.props.requirements_dir, 'idea-administrator.in'), 'r'
    ) as f:
    ...
    with open(
        os.path.join(idea.props.requirements_dir, 'idea-administrator.in'), 'w'
    ) as f:
    ...
    updated_lines = []
    with open(
        os.path.join(idea.props.administrator_project_dir, 'install', 'install.sh'), 'r'
    ) as f:
        lines = f.read().splitlines()
    for line in lines:
    ...
    with open(
        os.path.join(idea.props.administrator_project_dir, 'install', 'install.sh'), 'w'
    ) as f:
        f.write(os.linesep.join(updated_lines))
    ...
    idea.console.warning(
        '- run `invoke devtool.upgrade-cdk` to upgrade CDK nodejs binary and install CDK python libraries for new version.'
    )
...
def upgrade_cdk(c):
    ...
    with idea.console.spinner(
        f'upgrading IDEA CDK version to: {idea.props.idea_cdk_version} ...'
    ):
