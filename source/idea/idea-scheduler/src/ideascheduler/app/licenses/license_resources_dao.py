...
class LicenseResourcesDAO:
    ...
    def convert_from_db(db_license_resource: Dict) -> HpcLicenseResource:
        ...
        license_resource.availability_check_cmd = Utils.get_value_as_string(
            'availability_check_cmd', db_license_resource
        )
        license_resource.available_count = Utils.get_value_as_int(
            'available_count', db_license_resource
        )
        license_resource.reserved_count = Utils.get_value_as_int(
            'reserved_count', db_license_resource
        )
        license_resource.created_on = arrow.get(
            Utils.get_value_as_int('created_on', db_license_resource)
        ).datetime
        license_resource.updated_on = arrow.get(
            Utils.get_value_as_int('updated_on', db_license_resource)
        ).datetime
        return license_resource

    @staticmethod
    def convert_to_db(license_resource: HpcLicenseResource) -> Dict:
        db_license_resource = {'name': license_resource.name}
        ...
        if license_resource.availability_check_cmd is not None:
            db_license_resource['availability_check_cmd'] = (
                license_resource.availability_check_cmd
            )
    ...
    def create_license_resource(self, license_resource: Dict) -> Dict:
        ...
        self.table.put_item(Item=created_license_resource)
    ...
    def list_license_resources(
        ...
        return ListHpcLicenseResourcesResult(
            listing=license_resources, paginator=SocaPaginator(cursor=response_cursor)
        )
