...
from ideadatamodel import (
    ...
    SocaMemory,
    JobValidationResult, JobValidationResultEntry, SocaJob,
    SocaSpotAllocationStrategy, SocaJobEstimatedBOMCost, SocaJobNotifications, SocaJobState,
    CheckServiceQuotaResult, ServiceQuota,
    DryRunOption, SocaJobEstimatedBudgetUsage, SubmitJobResult
)
from ideascheduler.app.scheduler.openpbs.openpbs_api import OpenPBSHookRequest, OpenPBSHookResult
...
DRY_RUN_HEADER = f' ---- THIS IS A DRY RUN, JOB WILL NOT BE QUEUED ----- {os.linesep}{os.linesep}'
...
class OpenPBSAPIInvocationContext:
    ...
    def build_and_validate_job(self):
        try:
            ...
            queue_profile = self.app_context.queue_profiles.get_queue_profile(queue_name=queue_name)
            ...
            if Utils.is_empty(project_name):
                project = self.app_context.projects_client.get_project_by_id(queue_profile.projects[0].project_id)
            ...
            if e.error_code in (errorcodes.SCHEDULER_QUEUE_PROFILE_DISABLED,
    ...
    def is_valid(self) -> bool:
        return self.job_validation_result.is_valid() and self.incidentals_validation_result.is_valid()

    def add_job_validation_entry(self, error_code: str, message: str):
        self.job_validation_result.results.append(JobValidationResultEntry(
            error_code=error_code,
            message=message
        ))

    def add_incidentals_validation_entry(self, error_code: str, message: str, line_break=False):
        if line_break:
            self.incidentals_validation_result.results.append(JobValidationResultEntry())
        self.incidentals_validation_result.results.append(JobValidationResultEntry(
            error_code=error_code,
            message=message
        ))
    ...
    def get_quotas_as_table(quotas: List[ServiceQuota]):
        table = PrettyTable(['QuotaName', 'Available vCPUs', 'Desired vCPUs', 'Consumed vCPUs'])
        for quota in quotas:
            table.add_row([quota.quota_name, quota.available, quota.desired, quota.consumed])
        return str(table)
        ...
        if has_access:
            ...
            try:
                ...
                if e.error_code == errorcodes.EC2_RESERVED_INSTANCES_NOT_PURCHASED:
                    self.add_incidentals_validation_entry(
                        error_code='ReservedInstancesNotPurchased',
                        message=e.message
                    )
    ...
    def get_budget_usage(self) -> Optional[SocaJobEstimatedBudgetUsage]:
        ...
        budget_helper = AwsBudgetsHelper(
            context=self.app_context,
            job=self.job
        )
    ...
    def get_estimated_price_as_table(self) -> str:
        ...
        costs.add_row([
            ...
            '',
            '',
            bom_cost.line_items_total.formatted()
        ])
        ...
        if savings:
            ...
            result += f'Potential Savings: {bom_cost.savings_total.formatted()} ({bom_cost.savings_percent()}%)' \
                      f'{os.linesep}' \
                      f'{savings}'
        return result
    ...
    def get_budget_usage_as_table(self) -> Optional[str]:
        ...
        if bom_cost.savings is not None:
            result += f', Job usage with Savings: {usage.job_usage_percent_with_savings}%'
    ...
    def dry_run_post_processing(self) -> OpenPBSHookResult:
        ...
        if dry_run == DryRunOption.JSON_JOB:
            ...
            formatted_user_message = f'Job Info: {os.linesep}' \
                                     f'{json_content}'
        elif dry_run == DryRunOption.NOTIFICATION_EMAIL:
            job = self.job
            job.notifications = SocaJobNotifications(
                started=True,
                completed=True
            )
            ...
            job.end_time = arrow.utcnow().shift(seconds=self.get_job_time_seconds()).datetime
            ...
            formatted_user_message = f'DryRun: Email notifications sent successfully.'

        else:

            formatted_user_message = DRY_RUN_HEADER

            formatted_user_message += f'If submitted, job will be accepted and queued. IDEA will provision AWS ' \
                                      f'resources required for this job. {os.linesep}{os.linesep}'

            formatted_user_message += f'Estimated Job Costs for {self.get_friendly_job_time()}: {os.linesep}:'
            formatted_user_message += f'{self.get_estimated_price_as_table()}{os.linesep}{os.linesep}'
            ...
            if self._reserved_instances_unavailable:
                formatted_user_message += 'You have requested for reserved instances, but purchased reserved ' \
            ...
            if self._service_quota_result is not None:
                ...
                if self._service_quota_unavailable:
                    formatted_user_message += 'The service quota for the requested class of instances is at capacity. ' \
                    ...
                    formatted_user_message += f'Applicable AWS Service Quotas: {os.linesep}' \
