...
class HpcQueueProfilesDAO:
    ...
    def get_table_name(self) -> str:
        return (
            f'{self.context.cluster_name()}.{self.context.module_id()}.queue-profiles'
        )
        ...
        self.context.aws_util().dynamodb_create_table(
            create_table_request={
                ...
                'AttributeDefinitions': [
                    {'AttributeName': 'queue_profile_id', 'AttributeType': 'S'}
                ],
                'KeySchema': [{'AttributeName': 'queue_profile_id', 'KeyType': 'HASH'}],
    ...
    def convert_from_db(db_queue_profile: Dict) -> HpcQueueProfile:
        ...
        queue_profile.queue_profile_id = Utils.get_value_as_string(
            'queue_profile_id', db_queue_profile
        )
        queue_profile.title = Utils.get_value_as_string('title', db_queue_profile)

        queue_profile.description = Utils.get_value_as_string(
            'description', db_queue_profile
        )
        ...
        queue_profile.keep_forever = Utils.get_value_as_bool(
            'keep_forever', db_queue_profile
        )

        queue_profile.terminate_when_idle = Utils.get_value_as_int(
            'terminate_when_idle', db_queue_profile
        )
        ...
        queue_management_params.max_running_jobs = Utils.get_value_as_int(
            'max_running_jobs', db_queue_profile
        )
        queue_management_params.max_provisioned_instances = Utils.get_value_as_int(
            'max_provisioned_instances', db_queue_profile
        )
        ...
        queue_management_params.allowed_instance_types = Utils.get_value_as_list(
            'allowed_instance_types', db_queue_profile
        )
        queue_management_params.excluded_instance_types = Utils.get_value_as_list(
            'excluded_instance_types', db_queue_profile
        )
        queue_management_params.restricted_parameters = Utils.get_value_as_list(
            'restricted_parameters', db_queue_profile
        )
        queue_management_params.allowed_security_groups = Utils.get_value_as_list(
            'allowed_security_groups', db_queue_profile
        )
        queue_management_params.allowed_instance_profiles = Utils.get_value_as_list(
            'allowed_instance_profiles', db_queue_profile
        )

        default_job_params = SocaJobParams()
        queue_profile.default_job_params = default_job_params
        default_job_params.nodes = Utils.get_value_as_int(
            'param_nodes', db_queue_profile
        )
        ...
        param_memory_unit = Utils.get_value_as_string(
            'param_memory_unit', db_queue_profile
        )
        if param_memory is not None and param_memory_unit is not None:
            default_job_params.memory = SocaMemory(
                value=param_memory, unit=SocaMemoryUnit(param_memory_unit)
            )
        default_job_params.gpus = Utils.get_value_as_int('param_gpus', db_queue_profile)
        default_job_params.mpiprocs = Utils.get_value_as_int(
            'param_mpiprocs', db_queue_profile
        )
        default_job_params.base_os = Utils.get_value_as_string(
            'param_base_os', db_queue_profile
        )
        default_job_params.instance_ami = Utils.get_value_as_string(
            'param_instance_ami', db_queue_profile
        )
        default_job_params.instance_types = Utils.get_value_as_list(
            'param_instance_types', db_queue_profile
        )
        default_job_params.force_reserved_instances = Utils.get_value_as_bool(
            'param_force_reserved_instances', db_queue_profile
        )
        default_job_params.spot = Utils.get_value_as_bool(
            'param_spot', db_queue_profile
        )
        ...
        default_job_params.spot_allocation_count = Utils.get_value_as_int(
            'param_spot_allocation_count', db_queue_profile
        )
        spot_allocation_strategy = Utils.get_value_as_string(
            'param_spot_allocation_strategy', db_queue_profile
        )
        if spot_allocation_strategy is not None:
            default_job_params.spot_allocation_strategy = SocaSpotAllocationStrategy(
                spot_allocation_strategy
            )
        default_job_params.subnet_ids = Utils.get_value_as_list(
            'param_subnet_ids', db_queue_profile
        )
        default_job_params.security_groups = Utils.get_value_as_list(
            'param_security_groups', db_queue_profile
        )
        default_job_params.instance_profile = Utils.get_value_as_string(
            'param_instance_profile', db_queue_profile
        )
        root_storage_size = Utils.get_value_as_float(
            'param_root_storage_size', db_queue_profile
        )
        root_storage_size_unit = Utils.get_value_as_string(
            'param_root_storage_size_unit', db_queue_profile
        )
        if root_storage_size is not None and root_storage_size_unit is not None:
            default_job_params.root_storage_size = SocaMemory(
                value=root_storage_size, unit=SocaMemoryUnit(root_storage_size_unit)
            )
        default_job_params.enable_scratch = Utils.get_value_as_bool(
            'param_enable_scratch', db_queue_profile
        )
        default_job_params.scratch_provider = Utils.get_value_as_string(
            'param_scratch_provider', db_queue_profile
        )
        scratch_storage_size = Utils.get_value_as_float(
            'param_scratch_storage_size', db_queue_profile
        )
        scratch_storage_size_unit = Utils.get_value_as_string(
            'param_scratch_storage_size_unit', db_queue_profile
        )
        if scratch_storage_size is not None and scratch_storage_size_unit is not None:
            default_job_params.scratch_storage_size = SocaMemory(
                value=scratch_storage_size,
                unit=SocaMemoryUnit(scratch_storage_size_unit),
            )
        default_job_params.scratch_storage_iops = Utils.get_value_as_int(
            'param_scratch_storage_iops', db_queue_profile
        )
        default_job_params.enable_instance_store = Utils.get_value_as_bool(
            'param_enable_instance_store', db_queue_profile
        )
        default_job_params.enable_efa_support = Utils.get_value_as_bool(
            'param_enable_efa_support', db_queue_profile
        )
        default_job_params.enable_ht_support = Utils.get_value_as_bool(
            'param_enable_ht_support', db_queue_profile
        )
        default_job_params.enable_placement_group = Utils.get_value_as_bool(
            'param_enable_placement_group', db_queue_profile
        )
        default_job_params.enable_system_metrics = Utils.get_value_as_bool(
            'param_enable_system_metrics', db_queue_profile
        )
        default_job_params.enable_anonymous_metrics = Utils.get_value_as_bool(
            'param_enable_anonymous_metrics', db_queue_profile
        )
        default_job_params.compute_stack = Utils.get_value_as_string(
            'param_compute_stack', db_queue_profile
        )
        default_job_params.stack_id = Utils.get_value_as_string(
            'param_stack_id', db_queue_profile
        )
        default_job_params.job_group = Utils.get_value_as_string(
            'param_job_group', db_queue_profile
        )

        if 'param_fsx_lustre_enabled' in db_queue_profile:
            fsx_lustre = SocaFSxLustreConfig()
            default_job_params.fsx_lustre = fsx_lustre
            fsx_lustre.enabled = Utils.get_value_as_bool(
                'param_fsx_lustre_enabled', db_queue_profile
            )
            fsx_lustre.existing_fsx = Utils.get_value_as_string(
                'param_fsx_lustre_existing_fsx', db_queue_profile
            )
            fsx_lustre.s3_backend = Utils.get_value_as_string(
                'param_fsx_lustre_s3_backend', db_queue_profile
            )
            fsx_lustre.import_path = Utils.get_value_as_string(
                'param_fsx_lustre_import_path', db_queue_profile
            )
            fsx_lustre.export_path = Utils.get_value_as_string(
                'param_fsx_lustre_export_path', db_queue_profile
            )
            fsx_lustre.per_unit_throughput = Utils.get_value_as_int(
                'param_fsx_lustre_per_unit_throughput', db_queue_profile
            )
            fsx_lustre_size = Utils.get_value_as_float(
                'param_fsx_lustre_size', db_queue_profile
            )
            fsx_lustre_size_unit = Utils.get_value_as_string(
                'param_fsx_lustre_size_unit', db_queue_profile
            )
            if fsx_lustre_size is not None and fsx_lustre_size_unit is not None:
                fsx_lustre.size = SocaMemory(
                    value=fsx_lustre_size, unit=SocaMemoryUnit(fsx_lustre_size_unit)
                )
    ...
    def convert_to_db(queue_profile: HpcQueueProfile) -> Dict:
        ...
        if queue_management_params is not None:
            if queue_management_params.max_running_jobs is not None:
                db_queue_profile['max_running_jobs'] = (
                    queue_management_params.max_running_jobs
                )
            if queue_management_params.max_provisioned_instances is not None:
                db_queue_profile['max_provisioned_instances'] = (
                    queue_management_params.max_provisioned_instances
                )
            ...
            if queue_management_params.wait_on_any_job_with_license is not None:
                db_queue_profile['wait_on_any_job_with_license'] = (
                    queue_management_params.wait_on_any_job_with_license
                )
            if queue_management_params.allowed_instance_types is not None:
                db_queue_profile['allowed_instance_types'] = (
                    queue_management_params.allowed_instance_types
                )
            if queue_management_params.excluded_instance_types is not None:
                db_queue_profile['excluded_instance_types'] = (
                    queue_management_params.excluded_instance_types
                )
            if queue_management_params.restricted_parameters is not None:
                db_queue_profile['restricted_parameters'] = (
                    queue_management_params.restricted_parameters
                )
            if queue_management_params.allowed_security_groups is not None:
                db_queue_profile['allowed_security_groups'] = (
                    queue_management_params.allowed_security_groups
                )
            if queue_management_params.allowed_instance_profiles is not None:
                db_queue_profile['allowed_instance_profiles'] = (
                    queue_management_params.allowed_instance_profiles
                )
        ...
        if job_params is not None:
            ...
            if job_params.memory is not None:
                db_queue_profile['param_memory'] = Utils.get_as_decimal(
            ...
            if job_params.force_reserved_instances is not None:
                db_queue_profile['param_force_reserved_instances'] = (
                    job_params.force_reserved_instances
                )
            if job_params.spot is not None:
                db_queue_profile['param_spot'] = job_params.spot
            if job_params.spot_price is not None:
                db_queue_profile['param_spot_price'] = Utils.get_as_decimal(
                    job_params.spot_price.amount
                )
            if job_params.spot_allocation_count is not None:
                db_queue_profile['param_spot_allocation_count'] = (
                    job_params.spot_allocation_count
                )
            if job_params.spot_allocation_strategy is not None:
                db_queue_profile['param_spot_allocation_strategy'] = str(
                    job_params.spot_allocation_strategy
                )
            ...
            if job_params.root_storage_size is not None:
                db_queue_profile['param_root_storage_size'] = Utils.get_as_decimal(
                    job_params.root_storage_size.value
                )
                db_queue_profile['param_root_storage_size_unit'] = str(
                    job_params.root_storage_size.unit
                )
            ...
            if job_params.scratch_storage_size is not None:
                db_queue_profile['param_scratch_storage_size'] = Utils.get_as_decimal(
                    job_params.scratch_storage_size.value
                )
                db_queue_profile['param_scratch_storage_size_unit'] = str(
                    job_params.scratch_storage_size.unit
                )
            if job_params.scratch_storage_iops is not None:
                db_queue_profile['param_scratch_storage_iops'] = (
                    job_params.scratch_storage_iops
                )
            if job_params.enable_instance_store is not None:
                db_queue_profile['param_enable_instance_store'] = (
                    job_params.enable_instance_store
                )
            if job_params.enable_efa_support is not None:
                db_queue_profile['param_enable_efa_support'] = (
                    job_params.enable_efa_support
                )
            if job_params.enable_ht_support is not None:
                db_queue_profile['param_enable_ht_support'] = (
                    job_params.enable_ht_support
                )
            if job_params.enable_placement_group is not None:
                db_queue_profile['param_enable_placement_group'] = (
                    job_params.enable_placement_group
                )
            if job_params.enable_system_metrics is not None:
                db_queue_profile['param_enable_system_metrics'] = (
                    job_params.enable_system_metrics
                )
            if job_params.enable_anonymous_metrics is not None:
                db_queue_profile['param_enable_anonymous_metrics'] = (
                    job_params.enable_anonymous_metrics
                )
            ...
            if job_params.fsx_lustre is not None:
                ...
                if fsx_lustre.existing_fsx is not None:
                    db_queue_profile['param_fsx_lustre_existing_fsx'] = (
                        fsx_lustre.existing_fsx
                    )
                if fsx_lustre.s3_backend is not None:
                    db_queue_profile['param_fsx_lustre_s3_backend'] = (
                        fsx_lustre.s3_backend
                    )
                if fsx_lustre.import_path is not None:
                    db_queue_profile['param_fsx_lustre_import_path'] = (
                        fsx_lustre.import_path
                    )
                if fsx_lustre.export_path is not None:
                    db_queue_profile['param_fsx_lustre_export_path'] = (
                        fsx_lustre.export_path
                    )
                if fsx_lustre.deployment_type is not None:
                    db_queue_profile['param_fsx_lustre_deployment_type'] = (
                        fsx_lustre.deployment_type
                    )
                if fsx_lustre.per_unit_throughput is not None:
                    db_queue_profile['param_fsx_lustre_per_unit_throughput'] = (
                        fsx_lustre.per_unit_throughput
                    )
                if fsx_lustre.size is not None:
                    db_queue_profile['param_fsx_lustre_size'] = Utils.get_as_decimal(
                        fsx_lustre.size.value
                    )
                    db_queue_profile['param_fsx_lustre_size_unit'] = str(
                        fsx_lustre.size.unit
                    )
    ...
    def create_queue_profile(self, queue_profile: Dict) -> Dict:
        ...
        self.table.put_item(Item=created_queue_profile)
    ...
    def get_queue_profile_by_id(self, queue_profile_id: str) -> Optional[Dict]:
        ...
        result = self.table.get_item(Key={'queue_profile_id': queue_profile_id})
    ...
    def update(self, queue_profile: Dict) -> Dict:
        ...
        result = self.table.update_item(
            Key={'queue_profile_id': queue_profile_id},
    ...
    def delete_queue_profile(self, queue_profile_id: str):
        ...
        self.table.delete_item(Key={'queue_profile_id': queue_profile_id})
