...
class CloudFormationStackBuilder:
    ...
    def __init__(
        ...
        job: SocaJob,
        target_capacity_override: Optional[int] = None,
    ):
    ...
    def get_common_tags(self):
        ...
        if hasattr(self.job, 'owner_email') and self.job.owner_email:
            tags[constants.IDEA_TAG_JOB_OWNER_EMAIL] = self.job.owner_email
        ...
        if self.job.project:
            ...
            get_project_result = self.context.projects_client.get_project(
                GetProjectRequest(project_name=self.job.project)
            )
    ...
    def build_user_data(self):
        ...
        # Fetch job owner's email and add it to job context
        try:
            get_user_result = self.context.accounts_client.get_user(
                GetUserRequest(username=self.job.owner)
            )
            if get_user_result and get_user_result.user and get_user_result.user.email:
                self.job.owner_email = get_user_result.user.email
                self.logger.info(
                    f'{self.job.log_tag} found owner email: {self.job.owner_email}'
                )
        except Exception as e:
            self.logger.warning(
                f'{self.job.log_tag} failed to fetch owner email: {str(e)}'
            )
        ...
        # Configure CloudWatch logging and metrics for compute nodes
        cloudwatch_logs_enabled = self.context.config().get_bool(
            'cluster.cloudwatch_logs.enabled', False
        ) and self.context.config().get_bool('scheduler.cloudwatch_logs.enabled', False)

        # Define log files for Linux compute nodes
        log_files = []
        if cloudwatch_logs_enabled:
            from ideasdk.metrics.cloudwatch import CloudWatchAgentLogFileOptions

            # Use the same naming pattern as metrics_namespace for log groups
            if self.job.is_shared_capacity():
                log_group_name = f'/{self.context.cluster_name()}/{self.context.module_id()}/job-{self.job.get_job_group()}'
            else:
                log_group_name = f'/{self.context.cluster_name()}/{self.context.module_id()}/job-{self.job.job_id}'
            ...
            self.logger.debug(
                f'{self.job.log_tag} configured CloudWatch log files for Linux compute node: {len(log_files)} files, log_group: {log_group_name}'
            )
        ...
        BootstrapUtils.check_and_attach_cloudwatch_logging_and_metrics(
            ...
            enable_logging=cloudwatch_logs_enabled,
            log_files=log_files,
            enable_metrics=Utils.is_true(
                self.job.params.enable_system_metrics, default=False
            ),
        )
        ...
        self.logger.info(
            f'{self.job.log_tag} built bootstrap package: {bootstrap_package_archive_file}'
        )
        ...
        self.logger.info(
            f'{self.job.log_tag} uploading bootstrap package: {bootstrap_package_uri}'
        )
        ...
        return BootstrapUserDataBuilder(
            ...
            install_commands=['/bin/bash compute-node/setup.sh'],
    ...
    def build_spot_fleet(self, launch_template: LaunchTemplate) -> ec2.SpotFleet:
        ...
        spot_fleet_request.AllocationStrategy = (
            self.job.params.spot_allocation_strategy.spot_value()
        )
        spot_fleet_request.ExcessCapacityTerminationPolicy = self.context.config().get_string(
            'scheduler.job_provisioning.spot_fleet_request.excess_capacity_termination_policy',
            default='noTermination',
        )
        spot_fleet_iam_role_arn = self.context.config().get_string(
            'scheduler.spot_fleet_request_iam_role_arn', required=True
        )
        spot_fleet_request.IamFleetRole = spot_fleet_iam_role_arn
        spot_fleet_request.InstanceInterruptionBehavior = self.context.config().get_string(
        ...
        spot_maintenance_strategies = self.context.config().get_string(
        ...
        if Utils.is_not_empty(spot_maintenance_strategies):
            spot_fleet_request.SpotMaintenanceStrategies = (
                ec2.SpotMaintenanceStrategies(
        ...
        spot_fleet_request.Type = self.context.config().get_string(
        ...
        if self.job.params.enable_efa_support:
            for instance_type in self.job.provisioning_options.instance_types:
                spot_fleet_ltc.Overrides.append(
        ...
        launch_template.LaunchTemplateData.TagSpecifications.append(
    ...
    def build_placement_group(self) -> Optional[PlacementGroup]:
        ...
        pg.Strategy = self.context.config().get_string(
    ...
    def build_auto_scaling_group(
        ...
        if placement_group is not None:
            ...
            # Add dependency to ensure ASG (and its instances) are deleted before placement group
            # This prevents "placement group is in use" errors during CloudFormation stack deletion
            if isinstance(asg.DependsOn, str):
                asg.DependsOn = [asg.DependsOn, 'ComputeNodePlacementGroup']
            elif isinstance(asg.DependsOn, list):
                asg.DependsOn.append('ComputeNodePlacementGroup')
            else:
                asg.DependsOn = ['NodeLaunchTemplate', 'ComputeNodePlacementGroup']
        ...
        if self.job.is_mixed_capacity():
            ...
            distribution.OnDemandAllocationStrategy = self.context.config().get_string(
            ...
            distribution.OnDemandPercentageAboveBaseCapacity = (
                '0'  # force the other instances to be SPOT
            )
            if (
                self.job.params.spot_price
                and float(self.job.params.spot_price.amount) > 0
            ):
            ...
            distribution.SpotAllocationStrategy = (
                self.job.params.spot_allocation_strategy.asg_value()
            )
    ...
    def build_launch_template(self) -> LaunchTemplate:
        ...
        if instance_type_option.cpu_options_supported:
            if (
                self.job.is_hyper_threading_disabled
                and not self.job.is_multiple_instance_types
            ):
        ...
        launch_template_data.KeyName = self.context.config().get_string(
        ...
        if self.job.is_spot_capacity():
            ...
            launch_template_data.InstanceMarketOptions = InstanceMarketOptions(
                MarketType='spot', SpotOptions=SpotOptions(MaxPrice=spot_max_price)
            )
        ...
        if Utils.get_as_bool(self.job.params.enable_scratch, False) and Utils.are_equal(
            'ebs', self.job.params.scratch_provider
        ):
                ...
                BlockDeviceMapping(
                    ...
                    Ebs=EBSBlockDevice(
                        ...
                        VolumeType=Utils.get_as_string(
                            constants.DEFAULT_VOLUME_TYPE_SCRATCH, default='io1'
                        )
                        if iops > 0
                        else Utils.get_as_string(
                            constants.DEFAULT_VOLUME_TYPE_COMPUTE, default='gp3'
                        ),
                        Iops=iops if iops > 0 else Ref('AWS::NoValue'),
                        DeleteOnTermination=not self.job.params.keep_ebs_volumes,
                        Encrypted=Utils.get_as_bool(
                            constants.DEFAULT_VOLUME_ENCRYPTION_COMPUTE, default=True
                        ),
                        KmsKeyId=kms_key_id,
                    ),
                )
        ...
        launch_template_data.TagSpecifications = [
            ec2.TagSpecifications(
                ResourceType='instance', Tags=Tags(**self.get_common_tags(), **tags)
            ),
            ec2.TagSpecifications(
                ResourceType='volume', Tags=Tags(**self.get_common_tags(), **tags)
            ),
        ]
    ...
    def build_fsx_lustre(self):
        ...
        fsx_lustre.FileSystemTypeVersion = self.context.config().get_string(
            'cluster.aws.fsx_lustre_version', required=False, default='2.15'
        )
        ...
        if deployment_type == 'PERSISTENT_1':
            fsx_lustre_configuration.PerUnitStorageThroughput = (
                self.job.params.fsx_lustre.per_unit_throughput
            )
    ...
    def build_template(self) -> str:
        ...
        if self.job.is_spot_capacity():
            ...
            self.template.add_resource(
                self.build_spot_fleet(launch_template=launch_template)
            )

        else:
            # build mixed or ondemand fleet
            self.template.add_resource(
                self.build_auto_scaling_group(launch_template=launch_template)
            )
    ...
    def get_job_as_yaml_comment(self) -> str:
        ...
        if self.target_capacity_override is not None:
            target_capacity_override = (
                f' (TargetCapacity Override: {self.target_capacity_override})'
            )
    ...
    def build(self) -> str:
        try:
            ...
            if self.logger.isEnabledFor(logging.DEBUG):
                self.logger.debug(
                    f'---- CFN Template: Start ---{os.linesep}'
