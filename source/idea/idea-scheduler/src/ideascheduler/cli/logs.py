...
def logs(tokens, **kwargs):
    ...
    \b
    # follow application logs for a specific JobIds
    $ ideactl logs -f 1056
    $ ideactl logs -f 1056 1199
    $ ideactl logs -f "JobId: 1056" "JobId: 1199"

    \b
    # follow application logs for a specific JobGroup
    $ ideactl logs -f cfb9f876
    $ ideactl logs -f "JobGroup: cfb9f876"

    \b
    # follow application logs for a specific queue
    $ ideactl logs -f compute

    \b
    # follow metrics log for a queue type
    $ ideactl logs -M -f compute

    \b
    # find a JobId's logs in last 1000 lines of application logs
    $ ideactl logs -t 1000 1598

    \b
    # find all ERROR logs for a QueueType
    $ ideactl logs -t 1000 -a ERROR job-shared
    """
    LogFinder(
        tokens=tokens,
        kwargs=kwargs
