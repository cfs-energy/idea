    ...
    def list_from_index(self, options: ListSessionsRequest) -> ListSessionsResponse:
        ...
        return ListSessionsResponse(
            ...
            paginator=SocaPaginator(
                page_size=options.paginator.page_size if Utils.is_not_empty(options.paginator) and Utils.is_not_empty(options.paginator.page_size) else self.DEFAULT_PAGE_SIZE,
                start=options.paginator.start if Utils.is_not_empty(options.paginator) and Utils.is_not_empty(options.paginator.start) else 0,
                total=Utils.get_value_as_int('value', Utils.get_value_as_dict('total', Utils.get_value_as_dict('hits', response, {}), {}), 0)
            ),
        ...
        if Utils.is_not_empty(options.filters):
            ...
            for listing_filter in options.filters:
                if listing_filter.key == sessions_constants.USER_SESSION_DB_FILTER_BASE_OS_KEY and listing_filter.value == '$all':
                    ...
                    listing_filter.value = [VirtualDesktopBaseOS.AMAZON_LINUX2.value, VirtualDesktopBaseOS.RHEL7.value, VirtualDesktopBaseOS.CENTOS7.value, VirtualDesktopBaseOS.RHEL8.value, VirtualDesktopBaseOS.RHEL9.value, VirtualDesktopBaseOS.ROCKY8.value, VirtualDesktopBaseOS.ROCKY9.value]
