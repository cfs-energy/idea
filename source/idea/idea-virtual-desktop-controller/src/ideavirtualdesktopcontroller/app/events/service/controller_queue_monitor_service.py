...
class ControllerQueueMonitorService(SocaService):
    ...
    def _initialize(self):
        self._service_thread = Thread(
            name='controller-service-thread', target=self._monitor_controller_queue
        )
    ...
    def _handle_scheduled_event(self, message_id: str, message_body: dict):
        ...
        if detail_type != 'Scheduled Event':
            self._logger.error(
                f'[msg-id: {message_id}] Invalid detail type from scheduled event.. {detail_type}'
            )
            return

        self._logger.info(f'[msg-id: {message_id}] Publishing scheduled event message')
        self._events_utils.publish_scheduled_event(
            time=Utils.get_value_as_string('time', message_body)
        )
    ...
    def _poll_and_process_queue(self):
        response = (
            ...
            .receive_message(
                QueueUrl=self.context.config().get_string(
                    'virtual-desktop-controller.controller_sqs_queue_url', required=True
                ),
        ...
        for message in Utils.get_value_as_list('Messages', response, []):
            ...
            message_body = Utils.from_json(
                Utils.get_value_as_string('Body', message, None)
            )
            ...
            if Utils.is_empty(topic_arn):
                ...
                if event.header.namespace == 'Accounts.UserDisabledEvent':
                    ...
                    self._logger.error(
                        f'Invalid message with header: {event.header} received. Not handling. NP=OP'
                    )
            else:
                # SNS event
                self._logger.info(
                    f'[msg-id: {message_id}] Processing message from source SNS'
                )
                if topic_arn == self.context.config().get_string(
                    'virtual-desktop-controller.ssm_commands_sns_topic_arn',
                    required=True,
                ):
                    self._logger.info(
                        f'[msg-id: {message_id}] Message is from SSM Commands Status Topic'
                    )
                    self._handle_ssm_commands(message_id, message_body)
                elif topic_arn == self.context.config().get_string(
                    'cluster.ec2.state_change_notifications_sns_topic_arn',
                    required=True,
                ):
                    self._logger.info(
                        f'[msg-id: {message_id}] Message is from Ec2 State Change Topic'
                    )
                    self._handle_ec2_state_change(message_id, message_body)
                else:
                    self._logger.error(
                        f'[msg-id: {message_id}] Topic Arn not recognized: {topic_arn}. Not handling. NO=OP'
                    )
        ...
        if len(delete_message_info) > 0:
            _ = (
                ...
                .delete_message_batch(
                    QueueUrl=self.context.config().get_string(
                        'virtual-desktop-controller.controller_sqs_queue_url',
                        required=True,
                    ),
    ...
    def _handle_ec2_state_change(self, message_id: str, message_body):
        message = Utils.from_json(
            Utils.get_value_as_string('Message', message_body, '')
        )
        ...
        if Utils.is_empty(server):
            self._logger.info(
                f'[msg-id: {message_id}] Invalid DCV Host Instance-ID {instance_id}. Ignoring Message'
            )
            return

        state = Utils.get_value_as_string('state', event.payload, None)
        self._logger.info(
            f'[msg-id: {message_id}] Sending Ec2 state change message for instance id: {instance_id} for state {state}'
        )
    ...
    def _handle_ssm_commands(self, message_id: str, message_body):
        ...
        if Utils.is_empty(command_id):
            self._logger.error(
                f'[msg-id: {message_id}] Invalid SSM Command ID: {command_id}'
            )
            return
        ...
        if ssm_command.command_type == VirtualDesktopSSMCommandType.RESUME_SESSION:
            self._events_utils.publish_resume_session_command_status_event(
                ...
                idea_session_owner=Utils.get_value_as_string(
                    'idea_session_owner', ssm_command.additional_payload, ''
                ),
                command_id=command_id,
                instance_id=Utils.get_value_as_string(
                    'instance_id', ssm_command.additional_payload, ''
                ),
                status=status,
            )
        elif (
            ssm_command.command_type
            == VirtualDesktopSSMCommandType.WINDOWS_ENABLE_USERDATA_EXECUTION
        ):
            self._events_utils.publish_enable_userdata_windows_status_event(
                ...
                software_stack_id=Utils.get_value_as_string(
                    'software_stack_id', ssm_command.additional_payload, ''
                ),
            )
        elif (
            ssm_command.command_type
            == VirtualDesktopSSMCommandType.WINDOWS_DISABLE_USERDATA_EXECUTION
        ):
            ...
            ssm_command.command_type
            == VirtualDesktopSSMCommandType.CPU_UTILIZATION_CHECK_STOP_SCHEDULED_SESSION
        ):
            ...
            self._logger.error(
                f'[msg-id: {message_id}] Unsupported command type {ssm_command.command_type}. NO=OP'
            )
