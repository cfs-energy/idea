...
class EventsQueueMonitoringService(IdeaThreadpoolService):
    ...
    def calculate_number_of_threads_required(self) -> int:
        min_threads = self.context.config().get_int(
            'virtual-desktop-controller.controller.request_handler_threads.min',
            default=1,
        )
        max_queue_monitor_threads = self.context.config().get_int(
        ...
        num_of_messages = Utils.get_value_as_int(
            'ApproximateNumberOfMessages', attributes, 1
        )
        num_of_messages += Utils.get_value_as_int(
            'ApproximateNumberOfMessagesNotVisible', attributes, 1
        )
        num_of_messages += Utils.get_value_as_int(
            'ApproximateNumberOfMessagesDelayed', attributes, 1
        )
        ...
        return min(
            max(
                min_threads,
                int(math.ceil(num_of_messages / self.MESSAGE_COUNT_PER_THREAD)),
            ),
            max_queue_monitor_threads,
        )
