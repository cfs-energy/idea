...
from ideadatamodel import VirtualDesktopSession, Notification
...
class BaseDBEventHandler(BaseVirtualDesktopControllerEventHandler, ABC):
    ...
    def _notify_session_owner_of_state_update(
        self, session: VirtualDesktopSession, deleted=False
    ):
        if deleted:
            ...
            template_name = self.context.config().get_string(
                f'virtual-desktop-controller.dcv_session.notifications.{session.state.value.lower()}.email_template',
                required=True,
            )
            enabled = self.context.config().get_bool(
                f'virtual-desktop-controller.dcv_session.notifications.{session.state.value.lower()}.enabled',
                required=True,
            )

        if not enabled:
            self._logger.debug(
                f'Notification NOT sent for {session.idea_session_id} | {session.name} to {session.owner} because notification is not enabled. deleted={deleted}'
            )
            return

        self._logger.info(
            f'Sending notification for {session.name} to {session.owner} at {template_name}'
        )
