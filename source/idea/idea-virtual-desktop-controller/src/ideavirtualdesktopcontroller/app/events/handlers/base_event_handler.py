...
from ideavirtualdesktopcontroller.app.app_protocols import (
    VirtualDesktopQueueMessageHandlerProtocol,
)
...
class BaseVirtualDesktopControllerEventHandler(
    VirtualDesktopQueueMessageHandlerProtocol
):
    def __init__(
        self, context: ideavirtualdesktopcontroller.AppContext, logger_prefix: str
    ):
        ...
        self.session_counter_db: VirtualDesktopSessionCounterDB = (
            VirtualDesktopSessionCounterDB(context=self.context)
        )
        ...
        self.session_permissions_db: VirtualDesktopSessionPermissionDB = (
            VirtualDesktopSessionPermissionDB(context=self.context)
        )
        ...
        self.session_permission_utils: VirtualDesktopSessionPermissionUtils = (
            VirtualDesktopSessionPermissionUtils(
                context=context,
    ...
    def do_not_delete_message_exception(message: str):
        return SocaException(
            error_code=errorcodes.DO_NOT_DELETE_MESSAGE, message=message
        )

    @staticmethod
    def message_source_validation_failed(message: str):
        return SocaException(
            error_code=errorcodes.MESSAGE_SOURCE_VALIDATION_FAILED, message=message
        )
    ...
    @abstractmethod
    def handle_event(
        self, message_id: str, sender_id: str, event: VirtualDesktopEvent
    ): ...
    ...
    def is_sender_dcv_host_role(self, sender_id: str) -> bool:
        ...
        return (
            self.context.config().get_string(
                'virtual-desktop-controller.dcv_host_role_id', required=True
            )
            == sender_id
        )
    ...
    def is_sender_scheduled_event_lambda_role(self, sender_id: str) -> bool:
        ...
        return (
            self.context.config().get_string(
                'virtual-desktop-controller.scheduled_event_transformer_lambda_role_id',
                required=True,
            )
            == sender_id
        )
    ...
    def is_sender_dcv_broker_role(self, sender_id: str) -> bool:
        ...
        return (
            self.context.config().get_string(
                'virtual-desktop-controller.dcv_broker_role_id', required=True
            )
            == sender_id
        )
    ...
    def is_sender_controller_role(self, sender_id: str) -> bool:
        ...
        return (
            self.context.config().get_string(
                'virtual-desktop-controller.controller_iam_role_id', required=True
            )
            == sender_id
        )
