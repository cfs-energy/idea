...
from ideavirtualdesktopcontroller.app.events.handlers.db_entry_event_handlers.base_db_event_handler import (
    BaseDBEventHandler,
)
from ideavirtualdesktopcontroller.app.session_permissions.constants import (
    SESSION_PERMISSIONS_FILTER_PERMISSION_PROFILE_ID_KEY,
)
from ideavirtualdesktopcontroller.app.sessions.constants import (
    USER_SESSION_DB_FILTER_SOFTWARE_STACK_ID_KEY,
)
...
class DbEntryUpdatedEventHandler(BaseDBEventHandler):
    def __init__(self, context: ideavirtualdesktopcontroller.AppContext):
        ...
        self.DETAIL_TYPE_TYPE_DB_ENTRY_UPDATED_HANDLER: Dict[
            str, Callable[[str, str, dict, dict, str], None]
        ] = {
    ...
    def _handle_software_stack_updated(
        self, _: str, __: str, old_value: dict, new_value: dict, ___: str
    ):
        new_software_stack = (
            self.software_stack_db.convert_db_dict_to_software_stack_object(new_value)
        )
        self.software_stack_utils.update_software_stack_entry_to_opensearch(
            new_software_stack
        )

        old_software_stack = (
            self.software_stack_db.convert_db_dict_to_software_stack_object(old_value)
        )
        if (
            new_software_stack.name != old_software_stack.name
            or new_software_stack.description != old_software_stack.description
        ):
    ...
    def _handle_schedule_updated(
        self, _: str, __: str, ___: dict, ____: dict, table_name: str
    ):
        self._logger.debug(
            f'updated entry for {table_name} not handled. No=OP. Returning'
        )

    def _handle_user_session_updated(
        self, _: str, __: str, old_value: dict, new_value: dict, ___: str
    ):
        ...
        if (
            Utils.is_not_empty(old_session.server)
            and Utils.is_not_empty(new_session.server)
            and old_session.server.instance_type != new_session.server.instance_type
        ):
            publish_permission_update_event = True
        ...
        if publish_permission_update_event:
            self.events_utils.publish_update_session_permissions_event(
                ...
                if Utils.is_empty(new_session.server)
                else new_session.server.instance_type,
                new_name=new_session.name,
                new_state=new_session.state,
            )

    def _handle_dcv_host_updated(
        self, _: str, __: str, ___: dict, ____: dict, table_name: str
    ):
        self._logger.debug(
            f'updated entry for {table_name} not handled. No=OP. Returning'
    ...
    def _handle_permission_profile_updated(
        self, _: str, __: str, old_value: dict, new_value: dict, ___: str
    ):
        new_permission_profile = (
            self.permission_profile_db.convert_db_dict_to_permission_profile_object(
                new_value
            )
        )
        old_permission_profile = (
            self.permission_profile_db.convert_db_dict_to_permission_profile_object(
                old_value
            )
        )
        ...
        for new_permission in new_permission_profile.permissions:
            for old_permission in old_permission_profile.permissions:
                if (
                    new_permission.key == old_permission.key
                    and new_permission.enabled != old_permission.enabled
                ):
        ...
        if is_permission_updated:
            response = self.session_permissions_db.list_from_index(
                ListPermissionsRequest(
                    filters=[
                        SocaFilter(
            ...
            for idea_session in idea_session_info:
                self.events_utils.publish_enforce_session_permissions_event(
                    idea_session_id=idea_session[0], idea_session_owner=idea_session[1]
                )

    def _handle_session_permission_updated(
        self, _: str, __: str, old_value: dict, new_value: dict, ___: str
    ):
        new_session_permission = (
            self.session_permissions_db.convert_db_dict_to_session_permission_object(
                new_value
            )
        )
        self.session_permission_utils.update_session_entry_to_opensearch(
            new_session_permission
        )

        notifications_enabled = self.context.config().get_bool(
        ...
        old_session_permission = (
            self.session_permissions_db.convert_db_dict_to_session_permission_object(
                old_value
            )
        )
        if (
            new_session_permission.permission_profile.profile_id
            != old_session_permission.permission_profile.profile_id
            or new_session_permission.expiry_date != old_session_permission.expiry_date
        ):
            self.context.notification_async_client.send_notification(
                Notification(
                    ...
                    params={
                        ...
                        'formatted_expiry_date': new_session_permission.expiry_date.strftime(
                            '%B %d, %Y at %I:%M %p'
                        ),
                    },
                )
    ...
    def handle_event(self, message_id: str, sender_id: str, event: VirtualDesktopEvent):
        ...
        self.DETAIL_TYPE_TYPE_DB_ENTRY_UPDATED_HANDLER[table_name](
            hash_key, range_key, old_value, new_value, table_name
        )
