...
class DCVHostReadyEventHandler(BaseVirtualDesktopControllerEventHandler):
    ...
    def handle_event(self, message_id: str, sender_id: str, event: VirtualDesktopEvent):
        ...
        if Utils.is_empty(session):
            self.log_error(message_id=message_id, message='Invalid IDEA Session')
            return
        ...
        # Also handle sessions in RESUMING state which could be coming from an ERROR state after reboot
        if session.state not in {
            ...
            self.log_info(
                message_id=message_id,
                message=f'Idea Session id: {session.idea_session_id} is currently in state {session.state}. Ignoring Event',
            )
            return

        # For sessions in RESUMING state, transition to CREATING to allow the session creation flow to proceed
        if session.state == VirtualDesktopSessionState.RESUMING:
            self.log_info(
                message_id=message_id,
                message=f'Idea Session id: {session.idea_session_id} is in RESUMING state. Transitioning to CREATING to proceed with creation flow.',
            )
        ...
        if session.state == VirtualDesktopSessionState.CREATING:
            self.log_info(
                message_id=message_id,
                message=f'handling dcv_host_ready. session state is {session.state}',
            )
            raise self.do_not_delete_message_exception(
                f'Session state is {session.state}. Not handling now.'
            )
    ...
    def _create_session(
        self, message_id: str, session: VirtualDesktopSession
        ...
        counter_db_entry = self.session_counter_db.get(
            ...
            counter_type=VirtualDesktopSessionCounterType.DCV_SESSION_CREATION_REQUEST_ACCEPTED_COUNTER,
        )
        ...
        if Utils.is_empty(session_response.failure_reason):
            ...
            self.log_info(
                message_id=message_id,
                message=f'session request created for {session_response.owner} with name: {session_response.name} and dcv_session_id {session_response.dcv_session_id}',
            )
            ...
            self.controller_utils.create_tag(
                ...
                constants.IDEA_TAG_DCV_SESSION_ID,
            ...
            self.log_info(
                message_id=message_id,
                message=f'session creation error: {session.failure_reason} current count is {counter_db_entry.counter}',
            )
