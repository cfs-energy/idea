...
class IDEASessionTerminateEventHandler(BaseVirtualDesktopControllerEventHandler):
    ...
    def handle_event(self, message_id: str, sender_id: str, event: VirtualDesktopEvent):
        ...
        self.log_info(message_id=message_id, message=f'handle event to terminate idea_session: {idea_session_id} for owner: {idea_session_owner}. Force: {force}')
        ...
        success_list, fail_list = self.session_utils.terminate_sessions([session])
        # we know there is only 1 session in either of success or fail list
        if Utils.is_not_empty(fail_list):
            raise self.do_not_delete_message_exception(f'Error in stopping idea_session_id: {fail_list[0].idea_session_id}:{fail_list[0].name}. Error: {fail_list[0].failure_reason}. NOT terminating the session now. Will handle later')
