...
class ValidateDCVSessionDeletionEventHandler(BaseVirtualDesktopControllerEventHandler):
    ...
    def handle_event(self, message_id: str, sender_id: str, event: VirtualDesktopEvent):
        ...
        self.log_info(message_id=message_id, message=f'Received session stop validation message for idea_session_id {idea_session_id}, {idea_session_owner}')
        ...
        if session.state not in {VirtualDesktopSessionState.STOPPING, VirtualDesktopSessionState.DELETING}:
            self.log_info(message_id=message_id, message=f'IDEA Session: {session.idea_session_id} in state {session.state}. NO=OP. Returning.')
            return
        ...
        if Utils.is_empty(state) or state == 'DELETED':
            ...
            self.log_info(message_id=message_id, message=f'Idea session {session.idea_session_id}:{session.name} is deleted with state: {state}. Validation complete.')
            ...
            raise self.do_not_delete_message_exception(f'Idea session {session.idea_session_id}:{session.name} is not deleted with state: {state}. Not doing anything will try again.')
