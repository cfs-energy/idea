...
class IDEASessionPermissionsEnforceEventHandler(
    BaseVirtualDesktopControllerEventHandler
):
    ...
    def handle_event(self, message_id: str, sender_id: str, event: VirtualDesktopEvent):
        ...
        if Utils.is_empty(session):
            self.log_error(
                ...
                message=f'No permissions found for idea session id: {idea_session_id}, owner: {idea_session_owner}',
            )
            return
        ...
        if session.state not in {
            ...
            self.log_info(
                message_id=message_id,
                message=f'Session {idea_session_id} in {session.state}. Ignoring enforce permission event. NO=OP',
            )
            return
        ...
        if session.state in {
            ...
            raise self.do_not_delete_message_exception(
                f'Session {idea_session_id} in {session.state}. Will handle later'
            )
