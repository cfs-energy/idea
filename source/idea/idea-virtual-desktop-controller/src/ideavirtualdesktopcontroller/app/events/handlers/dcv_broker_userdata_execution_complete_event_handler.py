...
class DCVBrokerUserdataExecutionCompleteEventHandler(
    BaseVirtualDesktopControllerEventHandler
):
    ...
    def __init__(self, context: ideavirtualdesktopcontroller.AppContext):
        ...
        self.application_autoscaling_client = (
            self.context.aws().application_autoscaling()
        )
    ...
    def _add_tags_to_table(self, message_id: str, table_name: str):
        ...
        self.dynamodb_client.tag_resource(
            ResourceArn=self.arn_builder.get_ddb_table_arn(
                table_name_suffix='.'.join(
                    table_name.split('.')[1:]
                )  # removing cluster-name from the table name because the utility appends cluster name internally
            ),
            Tags=tags,
        )

    def _create_scaling_policies_for_ddb_table_if_required(
        self, message_id: str, table_name: str
    ):
        self.log_debug(
            message_id=message_id,
            message=f'Trying to create scaling policies for {table_name}...',
        )
        ...
        response = self.application_autoscaling_client.describe_scaling_policies(
            ServiceNamespace=self.SERVICE_NAMESPACE_DYNAMO_DB, ResourceId=resource_id
        )

        existing_scaling_policies = Utils.get_value_as_list(
            'ScalingPolicies', response, []
        )
        for existing_scaling_policy in existing_scaling_policies:
            target_tracking_policy_config = Utils.get_value_as_dict(
                'TargetTrackingScalingPolicyConfiguration', existing_scaling_policy, {}
            )
            predefined_metric_spec = Utils.get_value_as_dict(
                'PredefinedMetricSpecification', target_tracking_policy_config, {}
            )
            predefined_metric_type = Utils.get_value_as_string(
                'PredefinedMetricType', predefined_metric_spec, None
            )
            if (
                predefined_metric_type
                == self.SCALING_POLICY_TYPE_READ_CAPACITY_UTILIZATION
            ):
                self.log_debug(
                    message_id=message_id,
                    message=f'Read scaling policy for {table_name} already exists...',
                )
                read_capacity_resource_policy_exists = True
            elif (
                predefined_metric_type
                == self.SCALING_POLICY_TYPE_WRITE_CAPACITY_UTILIZATION
            ):
                self.log_debug(
                    message_id=message_id,
                    message=f'Write scaling policy for {table_name} already exists...',
                )
                ...
                self.log_debug(
                    message_id=message_id,
                    message=f'Some other scaling policy for {table_name} already exists...',
                )
                pass

        if (
            write_capacity_resource_policy_exists
            and read_capacity_resource_policy_exists
        ):
        ...
        if not read_capacity_resource_policy_exists:
            self.log_info(
                message_id=message_id,
                message=f'Creating read scaling policy for {table_name}...',
            )
            _ = self.application_autoscaling_client.register_scalable_target(
                ...
                MaxCapacity=self.context.config().get_int(
                    'virtual-desktop-controller.dcv_broker.dynamodb_table.read_capacity.max_units',
                    required=True,
                ),
                MinCapacity=self.context.config().get_int(
        ...
        if not write_capacity_resource_policy_exists:
            self.log_info(
                message_id=message_id,
                message=f'Creating write scaling policy for {table_name}...',
            )
            _ = self.application_autoscaling_client.register_scalable_target(
                ServiceNamespace=self.SERVICE_NAMESPACE_DYNAMO_DB,
                ResourceId=resource_id,
