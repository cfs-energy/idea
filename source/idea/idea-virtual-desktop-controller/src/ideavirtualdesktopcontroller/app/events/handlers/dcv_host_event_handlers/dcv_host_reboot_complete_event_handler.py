...
class DCVHostRebootCompleteEventHandler(BaseVirtualDesktopControllerEventHandler):
    ...
    def handle_event(self, message_id: str, sender_id: str, event: VirtualDesktopEvent):
        ...
        self.log_info(message_id=message_id, message=f'Received message from dcv-host for idea_session_id {idea_session_id}, {idea_session_owner}')
        if Utils.is_empty(idea_session_id) or Utils.is_empty(idea_session_owner):
            self.log_error(message_id=message_id, message=f'idea_session_id: {idea_session_id}, owner: {idea_session_owner}')
            return
        ...
        if session.state not in {VirtualDesktopSessionState.RESUMING, VirtualDesktopSessionState.READY, VirtualDesktopSessionState.ERROR, VirtualDesktopSessionState.STOPPED}:
            self.log_error(message_id=message_id, message=f'Idea session {session.idea_session_id}:{session.name} is in state: {session.state}. Not handling.')
            return

        self.log_info(message_id=message_id, message=f'Idea session {session.idea_session_id}:{session.name} is in state {session.state}. Handling reboot complete event')
        ...
        if session.state in {VirtualDesktopSessionState.READY}:
            self.log_info(message_id=message_id, message=f"Idea session {session.idea_session_id}:{session.name} is state: {session.state}. Needs additional validations.")
            ...
            if state == 'CREATING':
                ...
                self.log_info(message_id=message_id, message=f"Idea session {session.idea_session_id}:{session.name} is stable with state: {state}. No need to handle event. Returning.")
                return
            elif state == 'READY':
                ...
                if counter_db_entry.counter < self.RESUME_REQUEST_COUNT_THRESHOLD:
                    ...
                    raise self.do_not_delete_message_exception(f'Idea session {session.idea_session_id}:{session.name} might stable with state: {state}. No need to handle event now. Resume request count: {counter_db_entry.counter}. Threshold: {self.RESUME_REQUEST_COUNT_THRESHOLD}')
                else:
                    # session is stable for sure.
                    self.session_counter_db.delete(counter_db_entry)
                    self.log_info(message_id=message_id, message=f"Idea session {session.idea_session_id}:{session.name} is stable with state: {state}. No need to handle event.")
                    return

            self.log_warning(message_id=message_id, message=f"Idea session {session.idea_session_id}:{session.name} is NOT stable with state: {state}. Handling.")
        else:
            self.log_info(message_id=message_id, message=f"Idea session {session.idea_session_id}:{session.name} is state: {session.state}. Resuming Session.")
        ...
        self.log_info(message_id=message_id, message=f"Submitted resume request for Idea session {session.idea_session_id}:{session.name}.")
