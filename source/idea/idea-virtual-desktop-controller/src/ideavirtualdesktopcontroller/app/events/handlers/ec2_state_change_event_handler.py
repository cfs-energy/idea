...
class EC2StateChangeEventHandler(BaseVirtualDesktopControllerEventHandler):
    ...
    def _handle_stopping_events(self, message_id: str, event: VirtualDesktopEvent):
        ...
        if Utils.is_empty(session):
            self.log_error(message_id=message_id, message='Invalid IDEA Session. Should probably do some DB cleanup for instances.')
            return
        ...
        if session.state in {VirtualDesktopSessionState.STOPPED, VirtualDesktopSessionState.DELETING, VirtualDesktopSessionState.DELETED, VirtualDesktopSessionState.ERROR}:
            # Event ignored.
            self.log_info(message_id=message_id, message=f'IDEA Session {session.idea_session_id} in state {session.state}. Ignoring Event. NO=OP.')
            return
            ...
            self.log_error(message_id=message_id, message=f'IDEA Session {session.idea_session_id} in state {session.state}. Error.')
