...
class EC2StateChangeEventHandler(BaseVirtualDesktopControllerEventHandler):
    ...
    def _handle_stopping_events(self, message_id: str, event: VirtualDesktopEvent):
        ...
        self._logger.info(
            f'Handling {state} state event for instance-id: {instance_id}'
        )
        ...
        if Utils.is_empty(session):
            self.log_error(
                ...
                message='Invalid IDEA Session. Should probably do some DB cleanup for instances.',
            )
            return
        ...
        if session.state in {
            ...
            self.log_info(
                message_id=message_id,
                message=f'IDEA Session {session.idea_session_id} in state {session.state}. Ignoring Event. NO=OP.',
            )
            return
            ...
            self.log_error(
                message_id=message_id,
                message=f'IDEA Session {session.idea_session_id} in state {session.state}. Error.',
            )
    ...
    def _handle_running_event(self, message_id: str, event: VirtualDesktopEvent):
        instance_id = Utils.get_value_as_string('instance_id', event.detail, None)
        self._logger.info(
            f'Handling running state event for instance-id: {instance_id}'
        )
        ...
        if session.hibernation_enabled and session.state in {
            VirtualDesktopSessionState.RESUMING
        }:
    ...
    def handle_event(self, message_id: str, sender_id: str, event: VirtualDesktopEvent):
        ...
        if Utils.is_empty(instance_id):
            self.log_error(
                message_id=message_id, message=f'Invalid Instance_id: {instance_id}'
            )
            return
        ...
        if state in {'stopping', 'stopped'}:
            ...
            self.log_info(
                message_id=message_id,
                message=f'instance_id is {instance_id} state is {state}. NO=OP. Ignoring.',
            )
