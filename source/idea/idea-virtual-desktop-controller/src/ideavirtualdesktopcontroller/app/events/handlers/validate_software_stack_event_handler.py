...
class ValidateSoftwareStackEventHandler(BaseVirtualDesktopControllerEventHandler):
    ...
    def handle_event(self, message_id: str, sender_id: str, event: VirtualDesktopEvent):
        ...
        software_stack_base_os = VirtualDesktopBaseOS(
            Utils.get_value_as_string('software_stack_base_os', event.detail, None)
        )
        ...
        self.log_info(
            ...
            message=f'Handling software stack validation request for {software_stack_id}:{software_stack_base_os}',
        )
        if Utils.is_empty(software_stack_id) or Utils.is_empty(software_stack_base_os):
            self.log_error(
                message_id=message_id,
                message=f'Invalid {software_stack_id}: {software_stack_base_os}',
            )
            return

        software_stack_db_entry = self.software_stack_db.get(
            base_os=software_stack_base_os, stack_id=software_stack_id
        )
        if Utils.is_empty(software_stack_db_entry):
            self.log_error(message_id=message_id, message='Invalid Software Stack.')
            return

        response = self.controller_utils.describe_image_id(
            software_stack_db_entry.ami_id
        )
        ...
        if status == 'available':
            ...
            is_windows = 'windows' in str(software_stack_base_os).lower()
            ...
            raise self.do_not_delete_message_exception(
                f'AMI {software_stack_db_entry.ami_id} is not yet ready to use with status {status}'
            )
