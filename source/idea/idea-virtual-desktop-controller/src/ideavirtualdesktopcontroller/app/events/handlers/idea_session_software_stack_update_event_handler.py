...
class IDEASessionSoftwareStackUpdatedEventHandler(
    BaseVirtualDesktopControllerEventHandler
):
    ...
    def handle_event(self, message_id: str, sender_id: str, event: VirtualDesktopEvent):
        ...
        new_software_stack = Utils.get_value_as_dict(
            'new_software_stack', event.detail, {}
        )
        ...
        if Utils.is_empty(session):
            self.log_error(
                message=message_id, message_id='Invalid session details. NO=OP'
            )
            return

        software_stack_id = Utils.get_value_as_string(
            'stack_id', new_software_stack, None
        )
        if session.software_stack.stack_id != software_stack_id:
            self.log_error(
                message=message_id,
                message_id='Invalid session/software stack combination. NO=OP',
            )
            return
