...
class DbEntryDeletedEventHandler(BaseDBEventHandler):
    def __init__(self, context: ideavirtualdesktopcontroller.AppContext):
        ...
        self.DETAIL_TYPE_TYPE_DB_ENTRY_DELETED_HANDLER: Dict[str, Callable] = {
    ...
    def _handle_software_stack_deleted(
        self, _: str, __: str, deleted_value: dict, ___: str
    ):
        software_stack = (
            self.software_stack_db.convert_db_dict_to_software_stack_object(
                deleted_value
            )
        )
    ...
    def _handle_permission_profile_deleted(
        self, _: str, __: str, ___: dict, table_name: str
    ):
        self._logger.debug(
            f'deleted entry for {table_name} not handled. No=OP. Returning'
        )
    ...
    def _handle_user_session_deleted(
        self, _: str, __: str, deleted_value: dict, ___: str
    ):
    ...
    def _handle_session_permission_deleted(
        self, _: str, __: str, deleted_value: dict, ___: str
    ):
        deleted_session_permission = (
            self.session_permissions_db.convert_db_dict_to_session_permission_object(
                deleted_value
            )
        )
        self.session_permission_utils.delete_session_entry_from_opensearch(
            deleted_session_permission
        )
    ...
    def handle_event(self, message_id: str, sender_id: str, event: VirtualDesktopEvent):
        ...
        self.DETAIL_TYPE_TYPE_DB_ENTRY_DELETED_HANDLER[table_name](
            hash_key, range_key, deleted_value, table_name
        )
