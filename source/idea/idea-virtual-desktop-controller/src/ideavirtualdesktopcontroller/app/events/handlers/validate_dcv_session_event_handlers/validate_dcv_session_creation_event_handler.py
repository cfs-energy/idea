...
class ValidateDCVSessionCreationEventHandler(BaseVirtualDesktopControllerEventHandler):
    ...
    def handle_event(self, message_id: str, sender_id: str, event: VirtualDesktopEvent):
        ...
        if session.state in {VirtualDesktopSessionState.DELETING, VirtualDesktopSessionState.DELETED}:
            ...
            self.log_info(message_id=message_id, message=f'Idea session {session.idea_session_id}:{session.name} is being deleted; state: {session.state}. Ignoring Validation.')
        ...
        if state == 'READY':
            # session is stable.
            self.log_info(message_id=message_id, message=f'Idea session {session.idea_session_id}:{session.name} is stable with state: {state}. Validation complete. Moving to ready state')
            ...
            self.log_error(message_id=message_id, message=f'Idea session creation failed {session.idea_session_id}:{session.name} Validation ERROR. count: {counter_db_entry.counter}')
        elif counter_db_entry.counter > self.VALIDATION_REQUEST_THRESHOLD:
            # error
            session.state = VirtualDesktopSessionState.ERROR
            session = self.session_db.update(session)
            self.log_error(message_id=message_id, message=f'Idea session {session.idea_session_id}:{session.name} is not stable with state: {state}. Validation ERROR. count: {counter_db_entry.counter}')
        elif session.state is VirtualDesktopSessionState.STOPPING:
            self.log_error(message_id=message_id, message=f'Idea session {session.idea_session_id}:{session.name} is currently stopping. IGNORING the validation')
        else:
            ...
            should_not_delete_message_text = f'Idea session {session.idea_session_id}:{session.name} is not stable with state: {state}. {counter_db_entry.counter} times already validated. Will continue to validate'
