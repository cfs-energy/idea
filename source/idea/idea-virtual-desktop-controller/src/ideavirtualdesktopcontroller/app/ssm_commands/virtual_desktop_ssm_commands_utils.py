    ...
    def submit_ssm_command_to_get_cpu_utilization(
        ...
        if is_windows:
            ...
            commands = [
                ...
                # lets an old record win; compare a zero-padded year-month-day-time key instead
                'SSH_Last_Disconnect_Time=$(LC_ALL=C last -Fi | LC_ALL=C awk \'BEGIN{best = ""; split("Jan Feb Mar Apr May Jun Jul Aug Sep Oct Nov Dec", names, " "); for (i = 1; i <= 12; i++) month[names[i]] = i} !/0\\.0\\.0\\.0|still logged in|wtmp|^[[:space:]]*$/ && ($11 in month) && $14 ~ /^[0-9][0-9][0-9][0-9]$/ {key = sprintf("%s%02d%02d%s", $14, month[$11], $12, $13); if (key > best) {best = key; latest = $11 " " $12 " " $13 " " $14}} END{print latest}\')',
