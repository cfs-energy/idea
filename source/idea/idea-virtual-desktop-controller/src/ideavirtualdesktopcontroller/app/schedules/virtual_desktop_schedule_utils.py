...
class VirtualDesktopScheduleUtils:
    ...
    def _get_default_schedule_for_day_of_week(
        ...
        if schedule_type == VirtualDesktopScheduleType.WORKING_HOURS:
            ...
            start_up_time = self.context.config().get_string(
                f'virtual-desktop-controller.dcv_session.schedule.{day_of_week.value}.start_up_time',
                required=True,
            )
            shut_down_time = self.context.config().get_string(
                f'virtual-desktop-controller.dcv_session.schedule.{day_of_week.value}.shut_down_time',
                required=True,
            )
        else:
    ...
    def _is_same_schedule(
        self, schedule_a: VirtualDesktopSchedule, schedule_b: VirtualDesktopSchedule
    ) -> bool:
        if Utils.is_empty(schedule_a) and Utils.is_empty(schedule_b):
            self._logger.debug('Both the schedules are empty and hence equivalent...')
            # both are empty.
            return True

        if (Utils.is_not_empty(schedule_a) and Utils.is_empty(schedule_b)) or (
            Utils.is_not_empty(schedule_b) and Utils.is_empty(schedule_a)
        ):
        ...
        if (schedule_a.schedule_type == schedule_b.schedule_type) and (
            schedule_a.schedule_type == VirtualDesktopScheduleType.STOP_ON_IDLE
            or schedule_a.schedule_type == VirtualDesktopScheduleType.START_ALL_DAY
            or schedule_a.schedule_type == VirtualDesktopScheduleType.NO_SCHEDULE
            or schedule_a.schedule_type == VirtualDesktopScheduleType.WORKING_HOURS
        ):
            self._logger.debug(
                'Both are not empty but are the same static schedule type...'
            )
            return True

        if (
            schedule_a.schedule_type == VirtualDesktopScheduleType.NO_SCHEDULE
            or schedule_b.schedule_type == VirtualDesktopScheduleType.NO_SCHEDULE
        ):
            # at this point both do not have the same schedule type, but one is NO SCHEDULE for sure. They are different
            self._logger.debug(
                'At least one of the schedules is NO_SCHEDULE and hence different...'
            )
            return False
    ...
    def _delete_schedule(self, schedule: VirtualDesktopSchedule):
        if (
            Utils.is_empty(schedule)
            or schedule.schedule_type == VirtualDesktopScheduleType.NO_SCHEDULE
        ):
    ...
    def _create_or_update_schedule_for_session_for_day_of_week(
        self,
        day_of_week: DayOfWeek,
        current_schedule: VirtualDesktopSchedule,
        new_schedule: VirtualDesktopSchedule,
        idea_session_id: str,
        idea_session_owner: str,
    ) -> VirtualDesktopSchedule:
        self._logger.debug(
        ...
        if Utils.is_empty(new_schedule):
            self._logger.debug(
                f'No schedule provided for {day_of_week}, will leave it as is.'
            )
            return current_schedule

        if self._is_same_schedule(new_schedule, current_schedule):
            self._logger.debug(
                f'Both schedules for {day_of_week} are the same. NO OP. Returning'
            )
            return current_schedule
        ...
        if new_schedule.schedule_type == VirtualDesktopScheduleType.NO_SCHEDULE:
            ...
            Utils.is_empty(current_schedule)
            or current_schedule.schedule_type != new_schedule.schedule_type
        ):
            self._logger.info(
                f'schedule types mismatch. Delete old, create new for {day_of_week}'
            )
            create_new = True

        if Utils.is_not_empty(current_schedule) and (
            current_schedule.schedule_type != VirtualDesktopScheduleType.NO_SCHEDULE
            or Utils.is_not_empty(current_schedule.schedule_id)
        ):
            self._delete_schedule(current_schedule)

        if create_new:
            new_schedule = self._create_schedule_for_day_of_week(
    ...
    def update_schedule_for_session(
        self, new_schedules: VirtualDesktopWeekSchedule, session: VirtualDesktopSession
    ) -> VirtualDesktopSession:
        if Utils.is_empty(session.schedule):
            session.schedule = VirtualDesktopWeekSchedule()

        session.schedule.monday = (
            self._create_or_update_schedule_for_session_for_day_of_week(
    ...
    def trigger_schedule(self, event_time: time, schedule: VirtualDesktopSchedule):
        should_resume = False
        should_stop = False
        working_hours_start_up_time = self.context.config().get_string(
            'virtual-desktop-controller.dcv_session.working_hours.start_up_time',
            required=True,
        )
        working_hours_shut_down_time = self.context.config().get_string(
            'virtual-desktop-controller.dcv_session.working_hours.shut_down_time',
            required=True,
        )

        if (
            VirtualDesktopScheduleType[schedule.schedule_type]
            == VirtualDesktopScheduleType.STOP_ON_IDLE
        ):
            should_stop = True
        elif (
            VirtualDesktopScheduleType[schedule.schedule_type]
            == VirtualDesktopScheduleType.START_ALL_DAY
        ):
            should_resume = True
        else:
            ...
            if (
                VirtualDesktopScheduleType[schedule.schedule_type]
                == VirtualDesktopScheduleType.WORKING_HOURS
            ):
            ...
            start_up_time = DateTimeUtils.to_time_object(
                hours=int(start_up_time[0]), minutes=int(start_up_time[1])
            )
            shut_down_time = DateTimeUtils.to_time_object(
                hours=int(shut_down_time[0]), minutes=int(shut_down_time[1])
            )
