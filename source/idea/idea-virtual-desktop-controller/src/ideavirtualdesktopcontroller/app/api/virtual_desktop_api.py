...
from ideavirtualdesktopcontroller.app.session_permissions.constants import (
    SESSION_PERMISSIONS_FILTER_SESSION_ID_KEY,
    SESSION_PERMISSIONS_FILTER_ACTOR_KEY,
)
...
class VirtualDesktopAPI(BaseAPI):
    ...
    def __init__(self, context: ideavirtualdesktopcontroller.AppContext):
        ...
        self.controller_utils: VirtualDesktopControllerUtils = (
            VirtualDesktopControllerUtils(context=self.context)
        )
        self.software_stack_db: VirtualDesktopSoftwareStackDB = (
            VirtualDesktopSoftwareStackDB(context=self.context)
        )
        self.server_db: VirtualDesktopServerDB = VirtualDesktopServerDB(
            context=self.context
        )
        self.schedule_db: VirtualDesktopScheduleDB = VirtualDesktopScheduleDB(
            context=self.context
        )
        self.permission_profile_db: VirtualDesktopPermissionProfileDB = (
            VirtualDesktopPermissionProfileDB(context=self.context)
        )
        ...
        self.ssm_commands_db: VirtualDesktopSSMCommandsDB = VirtualDesktopSSMCommandsDB(
            context=self.context
        )
        self.session_permissions_db: VirtualDesktopSessionPermissionDB = (
            VirtualDesktopSessionPermissionDB(self.context)
        )

        self.software_stack_utils: VirtualDesktopSoftwareStackUtils = (
            VirtualDesktopSoftwareStackUtils(
                context=self.context, db=self.software_stack_db
            )
        )
        self.server_utils: VirtualDesktopServerUtils = VirtualDesktopServerUtils(
            context=self.context, db=self.server_db
        )
        self.schedule_utils: VirtualDesktopScheduleUtils = VirtualDesktopScheduleUtils(
            context=self.context, db=self.schedule_db
        )
        self.ssm_commands_utils: VirtualDesktopSSMCommandsUtils = (
            VirtualDesktopSSMCommandsUtils(
                context=self.context, db=self.ssm_commands_db
            )
        )
        self.session_permissions_utils: VirtualDesktopSessionPermissionUtils = (
            VirtualDesktopSessionPermissionUtils(
                context=context,
        ...
        self.VDI_GROUPS = [
            self.controller_utils.get_virtual_desktop_users_group(),
            self.controller_utils.get_virtual_desktop_admin_group(),
        ]

    def get_session_if_owner(
        self, username: str, idea_session_id: str
    ) -> Optional[VirtualDesktopSession]:
        return self.session_db.get_from_db(
            idea_session_id=idea_session_id, idea_session_owner=username
        )
    ...
    def _validate_session_for_session_permission_request(
        ...
        is_windows = 'windows' in str(session.base_os).lower()

        if is_windows and not self.controller_utils.is_active_directory():
            return (
                False,
    ...
    ) -> tuple[bool, UpdateSessionPermissionRequest]:
        is_valid_request = True
        if Utils.is_not_empty(request.create):
            for permission in request.create:
                permission, is_valid = self._validate_session_permission_create_request(
                    permission
                )
                if is_valid:
                    session = self.get_session_if_owner(
                        username=permission.idea_session_owner,
                        idea_session_id=permission.idea_session_id,
                    )
                    is_valid, message = (
                        self._validate_session_for_session_permission_request(session)
                    )
                    if not is_valid:
                        permission.failure_reason = message
            ...
            for permission in request.create:
                if Utils.is_empty(permission.failure_reason):
                    permission.failure_reason = (
                        'Invalid request. Rejecting all permissions'
                    )
            for permission in request.update:
                if Utils.is_empty(permission.failure_reason):
                    permission.failure_reason = (
    ...
    @staticmethod
    def validate_create_session_permission_request(
        permission: VirtualDesktopSessionPermission,
    ) -> tuple[bool, str]:
    ...
    def validate_create_session_request(
        ...
        if self.context.config().get_bool(
            'vdc.controller.enforce_project_budgets', default=True
        ):
            self._logger.info('eVDI Project budgets is enabled. Checking budgets.')

            if not session.project.is_budgets_enabled():
                self._logger.info(
                    f'Project budget disabled or not configured for {session.project.name}'
                )
            else:
                project_budget_name = Utils.get_as_string(
                    session.project.budget.budget_name, default=None
                )

                if project_budget_name:
                    self._logger.debug(f'Found Budget name: {project_budget_name}')
                    project_budget = self.context.aws_util().budgets_get_budget(
                    ...
                    if Utils.is_not_empty(project_budget):
                        ...
                        if project_budget.actual_spend > project_budget.budget_limit:
                            self._logger.error(
                                'Budget exceeded. Denying session request'
                            )
                            ...
                            self._logger.debug('Budget usage is acceptable. Proceeding')
        else:
            self._logger.info(
                'VDI Project budgets are disabled (vdc.controller.enforce_project_budgets). Not checking budgets.'
            )
        ...
        if session.hibernation_enabled and is_windows:
            ram = self.controller_utils.get_instance_ram(
                session.server.instance_type
            ).as_unit(SocaMemoryUnit.GiB)
            if ram.value > 16:
        ...
        if session.type is VirtualDesktopSessionType.VIRTUAL and is_windows:
            session.failure_reason = (
                f'{session.software_stack.base_os} does not support Virtual Sessions'
            )
            return session, False

        gpu_manufacturer = self.controller_utils.get_gpu_manufacturer(
            session.server.instance_type
        )
        if (
            session.type is VirtualDesktopSessionType.VIRTUAL
            and gpu_manufacturer is VirtualDesktopGPU.AMD
        ):
        ...
        architecture = self.controller_utils.get_architecture(
            session.server.instance_type
        )
        if (
            session.type is VirtualDesktopSessionType.CONSOLE
            and architecture is VirtualDesktopArchitecture.ARM64
        ):
        ...
        if session.hibernation_enabled:
            min_root_volume = (
                software_stack.min_storage
                + self.controller_utils.get_instance_ram(session.server.instance_type)
            )
    ...
    @staticmethod
    def validate_delete_session_request(
        session: VirtualDesktopSession,
    ) -> tuple[VirtualDesktopSession, bool]:
        is_valid = True
        if Utils.is_empty(session.idea_session_id):
            session.failure_reason = 'missing session.idea_session_id'
            is_valid = False

        return session, is_valid

    @staticmethod
    def complete_delete_session_request(
        session: VirtualDesktopSession, context: ApiInvocationContext
    ) -> VirtualDesktopSession:
    ...
    def _get_software_stack_info(
        self, software_stack_id: str
    ) -> VirtualDesktopSoftwareStack:
        software_stack_response = self.software_stack_db.get_from_index(
    ...
    def _get_session_info(
        ...
        session_response = self.session_db.get_from_index(
            idea_session_id=session.idea_session_id
        )
        if Utils.is_empty(session_response):
            session.failure_reason = f'invalid session.idea_session_id: {session.idea_session_id} for {session.name} owned by user: {session.owner}'
        else:
            session = session_response
        return session

    @staticmethod
    def validate_resume_session_request(
        session: VirtualDesktopSession,
    ) -> tuple[VirtualDesktopSession, bool]:
        if Utils.is_empty(session.idea_session_id):
            session.failure_reason = 'missing session.idea_session_id'
    ...
    @staticmethod
    def validate_create_software_stack_request(
        software_stack: VirtualDesktopSoftwareStack,
    ...
    @staticmethod
    def validate_get_connection_info_request(
        ...
        if Utils.is_not_empty(
            connection_info.idea_session_owner
        ) and Utils.is_not_empty(connection_info.idea_session_id):
            return '', True

        return (
            'Need to provide either (idea_session_id and idea_session_owner) or dcv_session_id',
            False,
        )

    @staticmethod
    def validate_get_session_screenshot_request(
        screenshot: VirtualDesktopSessionScreenshot,
    ) -> tuple[VirtualDesktopSessionScreenshot, bool]:
        if Utils.is_empty(screenshot.idea_session_id) and Utils.is_empty(
            screenshot.dcv_session_id
        ):
            screenshot.failure_reason = (
                'Provide either idea_session_id or dcv_session_id'
            )
            return screenshot, False
        return screenshot, True

    @staticmethod
    def complete_get_session_info_request(
        session: VirtualDesktopSession, context: ApiInvocationContext
    ) -> VirtualDesktopSession:
        if Utils.is_empty(session.owner):
            session.owner = context.get_username()
    ...
    @staticmethod
    def complete_get_session_screenshots_request(
    ...
    @staticmethod
    def complete_resume_session_request(
        session: VirtualDesktopSession, context: ApiInvocationContext
    ):
        if Utils.is_empty(session.owner):
            session.owner = context.get_username()
    ...
    @staticmethod
    def complete_reboot_session_request(
        session: VirtualDesktopSession, context: ApiInvocationContext
    ):
        if Utils.is_empty(session.owner):
            session.owner = context.get_username()
    ...
    @staticmethod
    def complete_stop_session_request(
        session: VirtualDesktopSession, context: ApiInvocationContext
    ):
        if Utils.is_empty(session.owner):
            session.owner = context.get_username()
    ...
    @staticmethod
    def complete_update_session_request(
        session: VirtualDesktopSession, context: ApiInvocationContext
    ):
        if Utils.is_empty(session.owner):
            session.owner = context.get_username()
    ...
    def complete_create_session_request(
        ...
        if Utils.is_empty(session.type):
            if is_windows or gpu_manufacturer is VirtualDesktopGPU.AMD:
    ...
    def _get_session_screenshots(
        ...
        for screenshot in screenshots:
            if Utils.is_empty(screenshot.dcv_session_id):
                session = self.session_db.get_from_db(
                    idea_session_owner=screenshot.idea_session_owner,
                    idea_session_id=screenshot.idea_session_id,
                )
                if Utils.is_empty(session):
                    screenshot.failure_reason = f'invalid IDEA session id: {screenshot.idea_session_id} for user: {screenshot.idea_session_owner}'
                    self._logger.error(screenshot.failure_reason)
                    failed_requests.append(screenshot)
                    continue

                if Utils.is_empty(session.dcv_session_id):
                    screenshot.failure_reason = f'DCV session does not yet exist for IDEA session id: {screenshot.idea_session_id} for user: {screenshot.idea_session_owner}'
                    self._logger.error(screenshot.failure_reason)
                    failed_requests.append(screenshot)
                    continue
        ...
        success_response, fail_response = (
            self.context.dcv_broker_client.get_session_screenshots(valid_screenshots)
        )
        fail_response.extend(failed_requests)
        return success_response, fail_response

    def _update_session(
        self, new_session: VirtualDesktopSession
    ) -> VirtualDesktopSession:
        old_session = self.session_db.get_from_db(
            idea_session_id=new_session.idea_session_id,
            idea_session_owner=new_session.owner,
        )

        if Utils.is_empty(old_session):
            new_session.failure_reason = f'invalid IDEA session id: {new_session.idea_session_id}:{new_session.name} or owner {new_session.owner}. Not updating the session.'
        ...
        if (
            ...
            and Utils.is_not_empty(new_session.name)
            and new_session.name != old_session.name
        ):
        ...
        if (
            not operation_failed
            and Utils.is_not_empty(new_session.description)
            and new_session.description != old_session.description
        ):
            is_session_updated = True
        ...
        if (
            not operation_failed
            and Utils.is_not_empty(new_session.server)
            and Utils.is_not_empty(new_session.server.instance_type)
        ):
            old_server = self.server_db.get(instance_id=new_session.server.instance_id)
            if (
                Utils.is_not_empty(old_server)
                and old_server.instance_type != new_session.server.instance_type
            ):
                if old_session.hibernation_enabled:
                    ...
                    error, success = self.controller_utils.change_instance_type(
                        instance_id=new_session.server.instance_id,
                        instance_type_name=new_session.server.instance_type,
                    )
        ...
        if not operation_failed and is_session_updated:
            ...
            self.controller_utils.create_tag(
                new_session.server.instance_id,
                constants.IDEA_TAG_NAME,
                f'{self.context.cluster_name()}-{new_session.name}-{new_session.owner}',
            )

        return new_session

    def _reboot_sessions(
    ...
    def _create_software_stack_from_session(
        ...
        session = self.session_db.get_from_db(
            idea_session_id=session.idea_session_id, idea_session_owner=session.owner
        )
        if Utils.is_empty(session):
            new_software_stack = VirtualDesktopSoftwareStack(
        ...
        if is_windows:
            ...
            response = self.controller_utils.create_image_for_instance_id(
                ...
                new_software_stack.name,
                new_software_stack.description,
            )
        ...
        new_software_stack.gpu = self.controller_utils.get_gpu_manufacturer(
            session.server.instance_type
        )

        # Preserve user-selected projects if present, otherwise default to the session's project
        if Utils.is_empty(new_software_stack.projects):
            new_software_stack.projects = [session.project]

        new_software_stack.min_ram = SocaMemory(
            value=int(
                self.controller_utils.get_instance_ram(
                    session.server.instance_type
                ).gib()
            ),
    ...
    def _list_session_permissions(
        self, idea_session_id: str, request: ListPermissionsRequest
    ) -> ListPermissionsResponse:
        ...
        if not session_filter_found:
            request.add_filter(
                SocaFilter(
                    key=SESSION_PERMISSIONS_FILTER_SESSION_ID_KEY, value=idea_session_id
                )
            )

        return self.session_permissions_db.list_from_index(request)

    def _list_shared_permissions(
        self, username: str, request: ListPermissionsRequest
    ) -> ListPermissionsResponse:
        ...
        if not actor_filter_found:
            request.add_filter(
                SocaFilter(key=SESSION_PERMISSIONS_FILTER_ACTOR_KEY, value=username)
            )

        return self.session_permissions_db.list_from_index(request)

    def _list_software_stacks(
        self, request: ListSoftwareStackRequest
    ) -> ListSoftwareStackResponse:
        return self.software_stack_db.list_from_index(request)

    def _get_session_connection_info(
        self,
        connection_info_request: VirtualDesktopSessionConnectionInfo,
        context: ApiInvocationContext,
    ) -> VirtualDesktopSessionConnectionInfo:
        connection_info = VirtualDesktopSessionConnectionInfo()
        ...
        if Utils.is_empty(connection_info_request.dcv_session_id):
            session = self.session_db.get_from_db(
                idea_session_id=connection_info_request.idea_session_id,
                idea_session_owner=connection_info_request.idea_session_owner,
            )
            if Utils.is_empty(session):
                connection_info.failure_reason = f'Invalid IDEA session id: {connection_info_request.idea_session_id} for user: {context.get_username()}.'
                is_valid_idea_session = False
            elif (
                VirtualDesktopSessionState[session.state]
        ...
        if is_valid_idea_session:
            connection_info = (
                self.context.dcv_broker_client.get_session_connection_data(
                    dcv_session_id=connection_info_request.dcv_session_id,
            ...
            if Utils.is_not_empty(connection_info.failure_reason):
                # Invalid connection
                self._logger.error(
                    f'Error in getting connection info data for DCV Session: {connection_info_request.dcv_session_id}, Trying to switch to error state.'
                )
                if Utils.is_empty(session) and Utils.is_not_empty(
                    connection_info_request.idea_session_id
                ):
                    self._logger.error(
                        f'We have idea session ID {connection_info_request.idea_session_id}, using it to get session db entry...'
                    )
                    session = self.session_db.get_from_db(
                        idea_session_id=connection_info_request.idea_session_id,
                        idea_session_owner=connection_info_request.idea_session_owner,
                ...
                # Valid connection
                custom_certificate_provided = self.context.config().get_bool(
