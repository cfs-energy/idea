...
from ideavirtualdesktopcontroller.app.session_permissions import (
    constants as session_permissions_constants,
)
...
class VirtualDesktopSessionPermissionDB(VirtualDesktopNotifiableDB, OpenSearchableDB):
    ...
    @staticmethod
    def convert_db_dict_to_session_permission_object(
        db_entry: Dict,
    ) -> Optional[VirtualDesktopSessionPermission]:
        return VirtualDesktopSessionPermission(
            idea_session_id=Utils.get_value_as_string(
                session_permissions_constants.SESSION_PERMISSIONS_DB_HASH_KEY,
                db_entry,
                '',
            ),
            idea_session_owner=Utils.get_value_as_string(
                session_permissions_constants.SESSION_PERMISSIONS_DB_IDEA_SESSION_OWNER_KEY,
                db_entry,
                '',
            ),
            idea_session_instance_type=Utils.get_value_as_string(
                session_permissions_constants.SESSION_PERMISSIONS_DB_IDEA_SESSION_INSTANCE_TYPE_KEY,
                db_entry,
                '',
            ),
            idea_session_base_os=VirtualDesktopBaseOS(
                Utils.get_value_as_string(
                    session_permissions_constants.SESSION_PERMISSIONS_DB_IDEA_SESSION_BASE_OS_KEY,
                    db_entry,
                    '',
                )
            ),
            idea_session_name=Utils.get_value_as_string(
                session_permissions_constants.SESSION_PERMISSIONS_DB_IDEA_SESSION_NAME_KEY,
                db_entry,
                '',
            ),
            idea_session_state=VirtualDesktopSessionState(
                Utils.get_value_as_string(
                    session_permissions_constants.SESSION_PERMISSIONS_DB_IDEA_SESSION_STATE_KEY,
                    db_entry,
                    '',
                )
            ),
            idea_session_created_on=Utils.to_datetime(
                Utils.get_value_as_int(
                    session_permissions_constants.SESSION_PERMISSIONS_DB_IDEA_SESSION_CREATED_ON_KEY,
                    db_entry,
                )
            ),
            idea_session_hibernation_enabled=Utils.get_value_as_bool(
                session_permissions_constants.SESSION_PERMISSIONS_DB_IDEA_SESSION_HIBERNATION_KEY,
                db_entry,
                False,
            ),
            idea_session_type=VirtualDesktopSessionType[
                Utils.get_value_as_string(
                    session_permissions_constants.SESSION_PERMISSIONS_DB_IDEA_SESSION_TYPE_KEY,
                    db_entry,
                )
            ],
            permission_profile=VirtualDesktopPermissionProfile(
                profile_id=Utils.get_value_as_string(
                    session_permissions_constants.SESSION_PERMISSIONS_DB_PERMISSION_PROFILE_ID_KEY,
                    db_entry,
                    '',
                )
            ),
            actor_type=VirtualDesktopSessionPermissionActorType(
                Utils.get_value_as_string(
                    session_permissions_constants.SESSION_PERMISSIONS_DB_ACTOR_TYPE_KEY,
                    db_entry,
                    '',
                )
            ),
            actor_name=Utils.get_value_as_string(
                session_permissions_constants.SESSION_PERMISSIONS_DB_RANGE_KEY,
                db_entry,
                '',
            ),
            created_on=Utils.to_datetime(
                Utils.get_value_as_int(
                    session_permissions_constants.SESSION_PERMISSIONS_DB_CREATED_ON_KEY,
                    db_entry,
                )
            ),
            updated_on=Utils.to_datetime(
                Utils.get_value_as_int(
                    session_permissions_constants.SESSION_PERMISSIONS_DB_UPDATED_ON_KEY,
                    db_entry,
                )
            ),
            expiry_date=Utils.to_datetime(
                Utils.get_value_as_int(
                    session_permissions_constants.SESSION_PERMISSIONS_DB_EXPIRY_DATE_KEY,
                    db_entry,
                )
            ),
        )
    ...
    @staticmethod
    def convert_session_permission_object_to_db_dict(
        session_permission: VirtualDesktopSessionPermission,
    ) -> Dict:
        ...
        return {
            ...
            session_permissions_constants.SESSION_PERMISSIONS_DB_IDEA_SESSION_CREATED_ON_KEY: Utils.to_milliseconds(
                session_permission.idea_session_created_on
            ),
            session_permissions_constants.SESSION_PERMISSIONS_DB_IDEA_SESSION_TYPE_KEY: session_permission.idea_session_type,
            session_permissions_constants.SESSION_PERMISSIONS_DB_IDEA_SESSION_HIBERNATION_KEY: False
            if Utils.is_empty(session_permission.idea_session_hibernation_enabled)
            else session_permission.idea_session_hibernation_enabled,
            ...
            session_permissions_constants.SESSION_PERMISSIONS_DB_EXPIRY_DATE_KEY: Utils.to_milliseconds(
                session_permission.expiry_date
            ),
            session_permissions_constants.SESSION_PERMISSIONS_DB_CREATED_ON_KEY: Utils.to_milliseconds(
                session_permission.created_on
            ),
            session_permissions_constants.SESSION_PERMISSIONS_DB_UPDATED_ON_KEY: Utils.to_milliseconds(
                session_permission.updated_on
            ),
        }
    ...
    def create(
        ...
        self.trigger_create_event(
            db_entry[session_permissions_constants.SESSION_PERMISSIONS_DB_HASH_KEY],
            db_entry[session_permissions_constants.SESSION_PERMISSIONS_DB_RANGE_KEY],
            new_entry=db_entry,
        )
    ...
    def update(
        ...
        result = self._table.update_item(
            Key={
                session_permissions_constants.SESSION_PERMISSIONS_DB_HASH_KEY: db_entry[
                    session_permissions_constants.SESSION_PERMISSIONS_DB_HASH_KEY
                ],
                session_permissions_constants.SESSION_PERMISSIONS_DB_RANGE_KEY: db_entry[
                    session_permissions_constants.SESSION_PERMISSIONS_DB_RANGE_KEY
                ],
            },
    ...
    def delete(self, session_permission: VirtualDesktopSessionPermission):
        if Utils.is_empty(session_permission.idea_session_id) or Utils.is_empty(
            session_permission.actor_name
        ):
            raise exceptions.invalid_params(
                'idea_session_id and actor_name is required'
            )
        ...
        self.trigger_delete_event(
            old_db_entry[session_permissions_constants.SESSION_PERMISSIONS_DB_HASH_KEY],
            old_db_entry[
                session_permissions_constants.SESSION_PERMISSIONS_DB_RANGE_KEY
            ],
    ...
    def get_for_session(
    ...
    ) -> List[VirtualDesktopSessionPermission]:
        ...
        try:
            ...
            for db_entry in db_entry_list:
                session_permissions.append(
                    self.convert_db_dict_to_session_permission_object(db_entry)
                )
    ...
    def get(
        self, idea_session_id: str, actor: str
    ) -> Optional[VirtualDesktopSessionPermission]:
        if Utils.is_any_empty(actor, idea_session_id):
            self._logger.error(
                f'actor: {actor} and/or idea_session_id: {idea_session_id} is invalid'
            )
            return None
    ...
    def list_all_from_db(
        self, cursor: str
    ) -> tuple[List[VirtualDesktopSessionPermission], Optional[str]]:
        scan_request = {}
    ...
    def list_from_index(
        self, options: ListPermissionsRequest
    ) -> ListPermissionsResponse:
        response = self.list_from_opensearch(options)
        permissions: List[VirtualDesktopSessionPermission] = []
        permissions_response = Utils.get_value_as_list(
        ...
        for permission in permissions_response:
            ...
            permissions.append(
                self.convert_db_dict_to_session_permission_object(index_object)
            )
