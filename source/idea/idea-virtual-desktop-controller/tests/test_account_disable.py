...
from ideadatamodel import (
    VirtualDesktopSession, VirtualDesktopSessionState, DayOfWeek,
    VirtualDesktopScheduleType, SocaPaginator,
)
...
def handler():
    ...
    result.schedule_db.get_empty_schedule.side_effect = VirtualDesktopScheduleDB.get_empty_schedule
...
def test_disable_clears_schedules_stops_sessions_and_paginates():
    h = handler()
    sessions = [VirtualDesktopSession(owner='user', idea_session_id=str(i), state=state)
                for i, state in enumerate((VirtualDesktopSessionState.READY, VirtualDesktopSessionState.STOPPED))]
    ...
    h.handle_event('message', 'role:session', SimpleNamespace(detail={'username': 'user'}))
...
def test_untrusted_role_is_rejected_before_any_write():
    ...
    with pytest.raises(Exception):
        h.handle_event('message', 'wrong-role', SimpleNamespace(detail={'username': 'user'}))
    h.session_db.list_all_for_user.assert_not_called()


def test_stop_failure_keeps_event_retryable():
    h = handler()
    session = VirtualDesktopSession(owner='user', idea_session_id='1', state=VirtualDesktopSessionState.READY)
    h.session_db.list_all_for_user.return_value = SimpleNamespace(listing=[session], paginator=None)
...
def test_already_queued_resume_cannot_restart_disabled_owner():
    ...
    session = VirtualDesktopSession(owner='user', idea_session_id='1', state=VirtualDesktopSessionState.STOPPED)
