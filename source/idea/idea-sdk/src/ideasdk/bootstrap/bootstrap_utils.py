...
class BootstrapUtils:
    @staticmethod
    def check_and_attach_cloudwatch_logging_and_metrics(
        ...
        cloudwatch_logs_enabled = (
            cluster_config.get_bool('cluster.cloudwatch_logs.enabled', False)
            and enable_logging
        )
        ...
        cloudwatch_metrics_enabled = cloudwatch_metrics_enabled and Utils.is_true(
        ...
        platform = Utils.get_platform(base_os=bootstrap_context.base_os)
        if platform == 'windows':
        ...
        bootstrap_context.vars.cloudwatch_agent_config = CloudWatchAgentConfig(
            ...
            options=CloudWatchAgentConfigOptions(
                ...
                metrics=CloudWatchAgentMetricsOptions.default_options(
                    namespace=metrics_namespace, include=include_metrics
                ),
            ),
        ).build()

        prometheus_metrics_enabled = Utils.is_not_empty(
            metrics_provider
        ) and metrics_provider in (
            constants.METRICS_PROVIDER_PROMETHEUS,
            constants.METRICS_PROVIDER_AMAZON_MANAGED_PROMETHEUS,
        )
        prometheus_metrics_enabled = prometheus_metrics_enabled and Utils.is_true(
            enable_metrics, default=True
        )
