...
class OpenSearchableDB:
    ...
    def list_from_opensearch(self, options: SocaListingPayload):
            ...
            for listing_filter in options.filters:
                ...
                if listing_filter.key == self.FREE_TEXT_FILTER_KEY:
                    ...
                    if self._free_text_search_support:
                        free_text_filter = FreeTextFilter(
                            text=listing_filter.value
                        )
                    continue
        ...
        response = self._os_client.search(
            ...
            start_from=options.paginator.start if Utils.is_not_empty(options.paginator) and Utils.is_not_empty(options.paginator.start) else 0,
            size=options.paginator.page_size if Utils.is_not_empty(options.paginator) and Utils.is_not_empty(options.paginator.page_size) else self._default_page_size
        )
        return response
