...
FALLBACK_INSTANCE_TYPES_REFRESH_INTERVAL = int(12 * 60 * 60)  # 12-hours default. Value is in seconds
...
class EC2InstanceTypesDB:
    ...
    def __init__(self, context: SocaContextProtocol):
        ...
        self._cache = Cache(
            maxsize=INSTANCE_TYPES_DEFAULT_SIZE,
    ...
    def _add_instance_data_to_cache(self):
        ...
        self._logger.debug(f"Starting EC2 instance type cache collection")

        with self._instance_types_lock:

            if self._cache.size():
                self._logger.info(f"Emptying EC2 instance type cache: {self._cache.size()}")
                self._cache.clear()

            _ec2_paginator = self._context.aws().ec2().get_paginator('describe_instance_types')
            ...
            for _page in _ec2_iterator:
                ...
                if self._cache.size() >= int(self._cache.maxsize * .90):
                    # Expand 15% at a time in minimum increments of 32
                    _new_size: int = 32 * round(ceil(self._cache.maxsize * 1.15)/32)
                    self._logger.debug(f"Auto-growing EC2 instance type cache size {self._cache.maxsize} -> {_new_size}")
                    self._cache.configure(maxsize=_new_size)

                _instance_types: list = Utils.get_value_as_list(key='InstanceTypes', obj=_page)

                # Per instance grinding/update
                for _instance_data in _instance_types:
                    _instance_name: str = Utils.get_value_as_string(key='InstanceType', obj=_instance_data, default=None)

                    if _instance_name is None:
                        self._logger.error(f"Missing InstanceType? InstanceType: {_instance_name} for: {_instance_data}")
                ...
                self._logger.debug(f"Instance Type Cache - Page #{_page_num}: Added {len(_instance_types)} to {self._cache.size()} - duration {_page_stop - _page_start}ms")

            _end_ec2_data: int = Utils.current_time_ms()
            self._cache_last_refresh = Utils.current_time()
            self._logger.debug(f"AWS EC2 instance type cache refresh - Size: {self._cache.size()}/{self._cache.maxsize}  Duration: {_end_ec2_data - _start_ec2_data}ms.  TTL: {self._cache_refresh_interval}")
    ...
    def get(self, instance_type: str) -> Optional[EC2InstanceType]:
        ...
        if (_current_time - self._cache_refresh_interval) > self._cache_last_refresh:
            self._logger.debug(f"Refreshing EC2 Describe_Instance_Types cache... Last refresh: {self._cache_last_refresh} . Current: {_current_time}  MaxAllowed: {self._cache_refresh_interval}")
            self._add_instance_data_to_cache()
