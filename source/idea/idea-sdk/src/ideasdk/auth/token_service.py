...
class ApiAuthorization(SocaBaseModel):
    ...
    username: Optional[str] = Field(
        default=None
    )  # will not exist for APP authorizations
    client_id: Optional[str] = Field(default=None)
...
class TokenService(TokenServiceProtocol):
    def __init__(self, context: SocaContextProtocol, options: TokenServiceOptions):
        ...
        self._jwk: Optional[PyJWKClient] = PyJWKClient(
            ...
            cache_keys=Utils.get_as_bool(
                options.jwk_cache_keys, DEFAULT_JWK_CACHE_KEYS
            ),
            max_cached_keys=Utils.get_as_int(
                options.jwk_max_cached_keys, DEFAULT_JWK_MAX_CACHED_KEYS
            ),
        )
    ...
    def decode_token(self, token: str, verify_exp: Optional[bool] = True) -> Dict:
        ...
        try:
            ...
            raise exceptions.soca_exception(
                error_code=errorcodes.AUTH_TOKEN_EXPIRED, message='Token Expired'
            )
    ...
    def get_authorization(self, decoded_token: Optional[Dict]) -> ApiAuthorization:
        ...
        if (
            Utils.is_not_empty(self.options.administrators_group_name)
            and self.options.administrators_group_name in groups
        ):
            authorization_type = ApiAuthorizationType.ADMINISTRATOR
        elif (
            Utils.is_not_empty(self.options.administrators_group_name)
            and self.options.managers_group_name in groups
        ):
    ...
    def is_scope_authorized(
        self, access_token: str, scope: str, verify_exp=True
    ) -> bool:
        ...
        if authorization.type != ApiAuthorizationType.APP:
            return False
        return (
            Utils.is_not_empty(authorization.scopes) and scope in authorization.scopes
        )
    ...
    def get_access_token_using_sso_auth_code(
        ...
        if self._sso_client_id is None or self._sso_client_secret is None:
            client_id = self._context.config().get_string(
                'identity-provider.cognito.sso_client_id', required=True
            )
            client_secret = self._context.config().get_secret(
        ...
        self._logger.debug(
            f'sso client_id: {self._sso_client_id}, client_secret: {self._sso_client_secret}'
        )
        basic_auth = Utils.encode_basic_auth(
            self._sso_client_id, self._sso_client_secret
        )
    ...
    def revoke_authorization(self, refresh_token: str, sso_auth: bool = False):
        ...
        if sso_auth:
            if (
                Utils.is_empty(self._sso_client_id)
                or Utils.is_empty(self._sso_client_secret) is None
            ):
                self._sso_client_id = self._context.config().get_string(
                    'identity-provider.cognito.sso_client_id', required=True
                )
