...
__all__ = ('PrometheusConfigOptions', 'PrometheusConfig')
...
class PrometheusConfig:
    def __init__(self, cluster_config: ClusterConfig, options: PrometheusConfigOptions):
        ...
        if Utils.is_false(options.node_exporter, True) and Utils.is_false(
            options.app_exporter, True
        ):
        ...
        if (
            Utils.is_true(options.app_exporter, False)
            and self.platform == constants.PLATFORM_WINDOWS
        ):
            raise exceptions.invalid_params(
                f'app_exporter is not supported for platform: {self.platform}'
            )

        # global scrape config
        self.scrape_interval = self.config.get_string(
            'metrics.prometheus.scrape_interval', default='60s'
        )
        self.scrape_timeout = self.config.get_string(
            'metrics.prometheus.scrape_timeout', default='10s'
        )
        ...
        # remote write
        remote_write_config = self.config.get_config(
            'metrics.prometheus.remote_write', required=True
        )
        ...
        if provider == constants.METRICS_PROVIDER_AMAZON_MANAGED_PROMETHEUS:
            self.remote_write['sigv4'] = {'region': self.aws_region}
    ...
    def get_default_app_exporter_scrape_config(self) -> Dict:
        api_context_path = self.config.get_string(
            f'{self.module_id}.server.api_context_path', required=True
        )
        server_port = self.config.get_int(
            f'{self.module_id}.server.port', required=True
        )
        config = {
            ...
            'static_configs': [{'targets': [f'localhost:{server_port}']}],
        }
        enable_tls = self.config.get_bool(
            f'{self.module_id}.server.enable_tls', default=False
        )
        if enable_tls:
            config['scheme'] = 'https'
    ...
    def build(self) -> Dict:
        ...
        env = Jinja2Utils.env_using_package_loader(
            package_name='ideasdk.metrics.prometheus', package_path='templates'
        )
