...
class ArnBuilder:
    ...
    def get_arn(
        self, service: str, resource: str, aws_account_id=None, aws_region=None
    ) -> str:
    ...
    def ec2_common_arns(self) -> List[str]:
        return [
            ...
            self.get_arn(
                'ec2', 'spot-instances-request/*', aws_account_id='*', aws_region='*'
            ),
    ...
    def s3_global_arns(self) -> List[str]:
        return [
            self.get_arn(
                's3',
                f'dcv-license.{self.config.get_string("cluster.aws.region")}/*',
                aws_account_id='',
                aws_region='',
            ),
            self.get_arn(
                's3', 'ec2-linux-nvidia-drivers/*', aws_account_id='', aws_region=''
            ),
            self.get_arn(
                's3', 'ec2-linux-nvidia-drivers', aws_account_id='', aws_region=''
            ),
            self.get_arn(
                's3', 'ec2-windows-nvidia-drivers/*', aws_account_id='', aws_region=''
            ),
            self.get_arn(
                's3', 'ec2-windows-nvidia-drivers', aws_account_id='', aws_region=''
            ),
            ...
            self.get_arn(
                's3', 'nvidia-gaming-drivers', aws_account_id='', aws_region=''
            ),
            self.get_arn(
                's3', 'nvidia-gaming-drivers/*', aws_account_id='', aws_region=''
            ),
            self.get_arn(
                's3', 'ec2-amd-linux-drivers/*', aws_account_id='', aws_region=''
            ),
            self.get_arn(
                's3', 'ec2-amd-linux-drivers', aws_account_id='', aws_region=''
            ),
            self.get_arn(
                's3', 'ec2-amd-windows-drivers/*', aws_account_id='', aws_region=''
            ),
            self.get_arn(
                's3', 'ec2-amd-windows-drivers', aws_account_id='', aws_region=''
            ),
    ...
    def target_group_arn(self) -> str:
        return self.get_arn(
            service='elasticloadbalancing', resource='targetgroup/soca*/*'
        )
    ...
    def service_role_arns(self) -> List[str]:
        ...
        return [
            role_arn(
                f's3.data-source.lustre.fsx.{self.config.get_string("cluster.aws.dns_suffix")}/*'
            ),
            role_arn(
                f'autoscaling.{self.config.get_string("cluster.aws.dns_suffix")}/*'
            ),
    ...
    def ses_arn(self) -> str:
        return self.get_arn(service='ses', resource='identity/*', aws_region='*')
    ...
    def dcv_license_s3_bucket_arns(self) -> List[str]:
        return [
            self.get_arn(
                ...
                aws_region='',
                aws_account_id='',
                resource='dcv-license.*/*',
            ),
            self.get_arn(
                service='s3', aws_region='', aws_account_id='', resource='dcv-license.*'
            ),
        ]
