...
class ConfigGenerator:
    ...
    def get_storage_apps_provider(self) -> str:
        return Utils.get_value_as_string('storage_apps_provider', self.user_values, 'efs')
    ...
    def get_storage_data_provider(self) -> str:
        return Utils.get_value_as_string('storage_data_provider', self.user_values, 'efs')
    ...
    def get_use_existing_apps_fs(self) -> Optional[bool]:
        ...
        if value and not self.get_use_existing_vpc():
            raise exceptions.invalid_params('use_existing_apps_fs cannot be True if use_existing_vpc = False')
        return value
    ...
    def get_existing_apps_fs_id(self) -> Optional[str]:
        ...
        if Utils.is_empty(value) and self.get_use_existing_apps_fs():
            raise exceptions.invalid_params('existing_apps_fs_id is required when existing_apps_fs_id = True')
        return value

    def get_use_existing_data_fs(self) -> Optional[bool]:
        value = Utils.get_value_as_bool('use_existing_data_fs', self.user_values, False)
        if value and not self.get_use_existing_vpc():
            raise exceptions.invalid_params('use_existing_data_fs cannot be True if use_existing_vpc = False')
        return value

    def get_existing_data_fs_id(self) -> Optional[str]:
        value = Utils.get_value_as_string('existing_data_fs_id', self.user_values)
        if Utils.is_empty(value) and self.get_use_existing_apps_fs():
            raise exceptions.invalid_params('existing_data_fs_id is required when use_existing_data_fs = True')
        return value
