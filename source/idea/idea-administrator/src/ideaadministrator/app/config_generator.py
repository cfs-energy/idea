...
class ConfigGenerator:
    ...
    def get_cluster_timezone(self) -> Optional[str]:
        ...
        default_timezone_id = Utils.get_value_as_string(
            'default', region_timezone_config, 'America/Los_Angeles'
        )
        region_timezone_id = Utils.get_value_as_string(
            aws_region, region_timezone_config, default_timezone_id
        )
    ...
    def get_administrator_username(self) -> str:
        return Utils.get_value_as_string(
            'administrator_username', self.user_values, 'clusteradmin'
        )
    ...
    def get_storage_apps_provider(self) -> str:
        return Utils.get_value_as_string(
            'storage_apps_provider', self.user_values, 'efs'
        )

    def get_apps_mount_dir(self) -> str:
        return Utils.get_value_as_string('apps_mount_dir', self.user_values, '/apps')

    def get_storage_data_provider(self) -> str:
        return Utils.get_value_as_string(
            'storage_data_provider', self.user_values, 'efs'
        )

    def get_data_mount_dir(self) -> str:
        return Utils.get_value_as_string('data_mount_dir', self.user_values, '/data')
    ...
    def get_directory_service_provider(self) -> str:
        return Utils.get_value_as_string(
            'directory_service_provider', self.user_values, 'openldap'
        )

    def get_use_existing_directory_service(self) -> Optional[bool]:
        value = Utils.get_value_as_bool(
            'use_existing_directory_service', self.user_values, False
        )
    ...
    def get_directory_service_root_username_secret_arn(self) -> Optional[str]:
        value = Utils.get_value_as_string(
            'directory_service_root_username_secret_arn', self.user_values
        )
        if Utils.is_empty(value) and self.get_use_existing_directory_service():
            raise exceptions.invalid_params(
    ...
    def get_directory_service_root_password_secret_arn(self) -> Optional[str]:
        value = Utils.get_value_as_string(
            'directory_service_root_password_secret_arn', self.user_values
        )
        if Utils.is_empty(value) and self.get_use_existing_directory_service():
            raise exceptions.invalid_params(
                'directory_service_root_password_secret_arn is required when use_existing_directory_service = True'
    ...
    def get_identity_provider(self) -> str:
        return Utils.get_value_as_string(
            'identity_provider', self.user_values, 'cognito-idp'
        )
    ...
    def get_dcv_broker_instance_ami(self) -> str:
        dcv_broker_instance_ami = Utils.get_value_as_string(
            'dcv_broker_instance_ami', self.user_values
        )
    ...
    def get_metrics_provider(self) -> Optional[str]:
        return Utils.get_value_as_string(
            'metrics_provider', self.user_values, 'cloudwatch'
        )
    ...
    def get_prometheus_remote_write_url(self) -> Optional[str]:
        ...
        if metrics_provider != 'prometheus':
            return None
        remote_write_url = Utils.get_value_as_string(
            'prometheus_remote_write_url', self.user_values
        )
    ...
    def get_vpc_id(self) -> Optional[str]:
        ...
        if Utils.is_empty(value) and self.get_use_existing_vpc():
            raise exceptions.invalid_params(
                'vpc_id is required when use_existing_vpc = True'
            )
        return value

    def get_private_subnet_ids(self) -> Optional[str]:
        private_subnet_ids = Utils.get_value_as_list(
            'private_subnet_ids', self.user_values, []
        )
        public_subnet_ids = Utils.get_value_as_list(
            'public_subnet_ids', self.user_values, []
        )
        if (
            self.get_use_existing_vpc()
            and Utils.is_empty(private_subnet_ids)
            and Utils.is_empty(public_subnet_ids)
        ):
            raise exceptions.invalid_params(
                'private_subnet_ids is required when use_existing_vpc = True'
            )
    ...
    def get_public_subnet_ids(self) -> Optional[str]:
        ...
        if (
            ...
            raise exceptions.invalid_params(
                'public_subnet_ids is required when use_existing_vpc = True'
            )
        return public_subnet_ids

    def get_use_existing_apps_fs(self) -> Optional[bool]:
        value = Utils.get_value_as_bool('use_existing_apps_fs', self.user_values, False)
        if value and not self.get_use_existing_vpc():
            raise exceptions.invalid_params(
                'use_existing_apps_fs cannot be True if use_existing_vpc = False'
            )
        return value

    def get_existing_apps_fs_id(self) -> Optional[str]:
        value = Utils.get_value_as_string('existing_apps_fs_id', self.user_values)
        if Utils.is_empty(value) and self.get_use_existing_apps_fs():
            raise exceptions.invalid_params(
                'existing_apps_fs_id is required when existing_apps_fs_id = True'
            )
        return value

    def get_use_existing_data_fs(self) -> Optional[bool]:
        value = Utils.get_value_as_bool('use_existing_data_fs', self.user_values, False)
        if value and not self.get_use_existing_vpc():
            raise exceptions.invalid_params(
                'use_existing_data_fs cannot be True if use_existing_vpc = False'
            )
        return value

    def get_existing_data_fs_id(self) -> Optional[str]:
        value = Utils.get_value_as_string('existing_data_fs_id', self.user_values)
        if Utils.is_empty(value) and self.get_use_existing_apps_fs():
            raise exceptions.invalid_params(
                'existing_data_fs_id is required when use_existing_data_fs = True'
            )
        return value

    def get_use_existing_opensearch_cluster(self) -> Optional[str]:
        value = Utils.get_value_as_bool(
            'use_existing_opensearch_cluster', self.user_values, False
        )
        if value and not self.get_use_existing_vpc():
            raise exceptions.invalid_params(
    ...
    def get_opensearch_domain_arn(self) -> Optional[str]:
        ...
        if Utils.is_empty(value) and self.get_use_existing_opensearch_cluster():
            raise exceptions.invalid_params(
                'opensearch_domain_arn is required when use_existing_opensearch_cluster = True'
            )
        return value

    def get_opensearch_domain_endpoint(self) -> Optional[str]:
        value = Utils.get_value_as_string(
            'opensearch_domain_endpoint', self.user_values
        )
        if Utils.is_empty(value) and self.get_use_existing_opensearch_cluster():
            raise exceptions.invalid_params(
    ...
    def get_alb_custom_certificate_provided(self) -> Optional[bool]:
        return Utils.get_value_as_bool(
            'alb_custom_certificate_provided', self.user_values, default=False
        )
    ...
    def get_alb_custom_certificate_acm_certificate_arn(self) -> Optional[str]:
        ...
        alb_custom_certificate_acm_certificate_arn = Utils.get_value_as_string(
            'alb_custom_certificate_acm_certificate_arn', self.user_values
        )
        if alb_custom_certificate_provided and Utils.is_empty(
    ...
    def get_alb_get_custom_dns_name(self) -> Optional[str]:
        alb_custom_certificate_provided = self.get_alb_custom_certificate_provided()
        alb_custom_dns_name = Utils.get_value_as_string(
            'alb_custom_dns_name', self.user_values
        )
    ...
    def get_dcv_session_traffic_routing(self) -> str:
        value = Utils.get_value_as_string(
            'dcv_session_traffic_routing', self.user_values, default='ALB'
        )
        if value not in {'ALB', 'NLB'}:
            raise exceptions.cluster_config_error(
                f'Invalid value - "{value}" provided for dcv_session_traffic_routing. Can be one of ALB or NLB only.'
            )
        return value

    def get_dcv_session_quic_support(self) -> Optional[bool]:
        return Utils.get_value_as_bool(
            'dcv_session_quic_support', self.user_values, default=False
        )
    ...
    def get_dcv_connection_gateway_custom_dns_hostname(self) -> Optional[str]:
        value = Utils.get_value_as_string(
            'dcv_connection_gateway_custom_dns_hostname', self.user_values
        )
        custom_certificate_provided = (
            self.get_dcv_connection_gateway_custom_certificate_provided()
        )
        if custom_certificate_provided and Utils.is_empty(value):
            raise exceptions.cluster_config_error(
                'Need to provide dcv_connection_gateway_custom_dns_hostname if dcv_connection_gateway_custom_certificate_provided is true'
            )
        return value

    def get_dcv_connection_gateway_custom_certificate_certificate_secret_arn(
        self,
    ) -> Optional[str]:
        value = Utils.get_value_as_string(
        ...
        if custom_certificate_provided and Utils.is_empty(value):
            raise exceptions.cluster_config_error(
                'Need to provide dcv_connection_gateway_custom_certificate_certificate_secret_arn if dcv_connection_gateway_custom_certificate_provided is true'
            )
        return value

    def get_dcv_connection_gateway_custom_certificate_private_key_secret_arn(
        self,
    ) -> Optional[str]:
        value = Utils.get_value_as_string(
        ...
        if custom_certificate_provided and Utils.is_empty(value):
            raise exceptions.cluster_config_error(
                'Need to provide dcv_connection_gateway_custom_certificate_private_key_secret_arn if dcv_connection_gateway_custom_certificate_provided is true'
            )
        return value
    ...
    def get_cluster_config_dir(self):
        return self.props.cluster_config_dir(
            self.get_cluster_name(), self.get_aws_region()
        )
    ...
    def generate_config_from_templates(self, temp=False, path=None):
        values = {
            ...
            'aws_region': self.get_aws_region(),
            'storage_apps_provider': self.get_storage_apps_provider(),
            'storage_data_provider': self.get_storage_data_provider(),
            'apps_mount_dir': self.get_apps_mount_dir(),
            'data_mount_dir': self.get_data_mount_dir(),
            ...
            'use_existing_apps_fs': self.get_use_existing_apps_fs(),
            'existing_apps_fs_id': self.get_existing_apps_fs_id(),
            'use_existing_data_fs': self.get_use_existing_data_fs(),
            'existing_data_fs_id': self.get_existing_data_fs_id(),
        ...
        env = Jinja2Utils.env_using_file_system_loader(
            search_path=self.props.cluster_config_templates_dir
        )
            ...
            for file in config_files:
                ...
                settings = template.render(
                    **values,
                    ...
                    supported_base_os=constants.SUPPORTED_OS,
                )
        ...
        for key in config:
            ...
            if isinstance(value, dict):
                self.traverse_config(
                    config_entries, path_prefix, value, filter_key_prefix
                )
    ...
    def convert_config_to_key_value_pairs(
        self, key_prefix: str, path: str = None
    ) -> List[Dict]:
