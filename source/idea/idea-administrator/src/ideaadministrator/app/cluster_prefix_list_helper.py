...
class ClusterPrefixListHelper:
    def __init__(self, cluster_name: str, aws_region: str, aws_profile: str = None):
        ...
        self.cluster_prefix_list_id = self.context.config().get_string(
    ...
    def _get_current_version(self) -> int:
        describe_result = (
            self.context.aws()
            .ec2()
            .describe_managed_prefix_lists(PrefixListIds=[self.cluster_prefix_list_id])
        )
    ...
    def list_entries(self) -> List[Dict]:
        result = []
        prefix_list_paginator = (
            self.context.aws().ec2().get_paginator('get_managed_prefix_list_entries')
        )
    ...
    def add_entry(self, cidr: str, description: str):
        ...
        for entry in result:
            if entry['cidr'] == cidr:
                self.context.warning(
                    f'CIDR: {cidr} already exists in cluster prefix list: {self.cluster_prefix_list_id}'
                )
                raise SystemExit(1)

        cidr_entry = {'Cidr': cidr}
        ...
        self.context.success(
            f'CIDR: {cidr} added to cluster prefix list: {self.cluster_prefix_list_id}.'
        )
    ...
    def remove_entry(self, cidr: str):
        ...
        if not found:
            self.context.warning(
                f'CIDR: {cidr} not found in cluster prefix list: {self.cluster_prefix_list_id}'
            )
            raise SystemExit(1)
        ...
        self.context.success(
            f'CIDR: {cidr} was removed from cluster prefix list: {self.cluster_prefix_list_id}.'
        )
