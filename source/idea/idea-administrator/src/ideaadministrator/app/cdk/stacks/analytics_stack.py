...
class AnalyticsStack(IdeaBaseStack):
    def __init__(
        ...
        is_existing = self.context.config().get_bool(
            'analytics.opensearch.use_existing', default=False
        )
        if is_existing:
            domain_vpc_endpoint_url = self.context.config().get_string(
                'analytics.opensearch.domain_vpc_endpoint_url', required=True
            )
            ...
            self.add_nag_suppression(
                ...
                suppressions=[
                    ...
                    IdeaNagSuppression(
                        ...
                        reason='Usage is required for Service Linked Role',
                    ),
                    IdeaNagSuppression(
            ...
            data_nodes = self.context.config().get_int(
                'analytics.opensearch.data_nodes', required=True
            )
            if data_nodes == 1:
                self.add_nag_suppression(
                    construct=self.stack,
    ...
    def build_analytics_input_stream(self):
        stream_config = self.context.config().get_string(
            'analytics.kinesis.stream_mode', required=True
        )
        if stream_config not in {'PROVISIONED', 'ON_DEMAND'}:
            raise exceptions.invalid_params(
                'analytics.kinesis.stream_mode needs to be one of PROVISIONED or ON_DEMAND only'
            )

        if stream_config == 'PROVISIONED':
            stream_mode = kinesis.StreamMode.PROVISIONED
            shard_count = self.context.config().get_int(
                'analytics.kinesis.shard_count', required=True
            )
        else:
        ...
        if self.aws_region in Utils.get_value_as_list(
            ...
            self.kinesis_stream.node.default_child.add_property_deletion_override(
                'StreamModeDetails'
            )
        ...
        stream_processing_lambda_role.attach_inline_policy(
            Policy(
                context=self.context,
        ...
        stream_processing_lambda = LambdaFunction(
            ...
            environment={'opensearch_endpoint': self.opensearch.domain_endpoint},
            vpc=self.cluster.vpc,
            vpc_subnets=ec2.SubnetSelection(subnets=self.cluster.private_subnets),
        )

        stream_processing_lambda.add_nag_suppression(
            suppressions=[
                IdeaNagSuppression(
                    rule_id='AwsSolutions-L1',
        ...
        # Check if we're in GovCloud by looking at the region
        if self.aws_region.startswith('us-gov-'):
            # Create the L1 construct directly with no tags for GovCloud
            event_source_mapping = lambda_.CfnEventSourceMapping(
                self.stack,
                f'{lambda_name}-event-source',
                function_name=stream_processing_lambda.function_name,
            ...
            # Set update and deletion policies using CfnDeletionPolicy
            event_source_mapping.cfn_options.deletion_policy = (
                cdk.CfnDeletionPolicy.DELETE
            )
            event_source_mapping.cfn_options.update_replace_policy = (
                cdk.CfnDeletionPolicy.DELETE
            )

            # Add a dependency to ensure the Lambda function exists before creating the mapping
            event_source_mapping.add_dependency(
                stream_processing_lambda.node.default_child
            )
        else:
            # Use standard L2 construct for commercial AWS
            stream_processing_lambda.add_event_source(event_source)
    ...
    def check_service_linked_role_exists(self) -> bool:
        try:
            aws_dns_suffix = self.context.config().get_string(
                'cluster.aws.dns_suffix', required=True
            )
            list_roles_result = (
                self.context.aws()
                .iam()
                .list_roles(PathPrefix=f'/aws-service-role/es.{aws_dns_suffix}')
            )
            ...
            roles.extend(
                Utils.get_value_as_list('Roles', list_roles_result, default=[])
            )
    ...
    def build_opensearch(self):
        ...
        data_node_instance_type = self.context.config().get_string(
            'analytics.opensearch.data_node_instance_type', required=True
        )
        ebs_volume_size = self.context.config().get_int(
            'analytics.opensearch.ebs_volume_size', required=True
        )
        node_to_node_encryption = self.context.config().get_bool(
            'analytics.opensearch.node_to_node_encryption', required=True
        )
        removal_policy = self.context.config().get_string(
            'analytics.opensearch.removal_policy', required=True
        )
        app_log_removal_policy = self.context.config().get_string(
            'analytics.opensearch.logging.app_log_removal_policy', default='DESTROY'
        )
        search_log_removal_policy = self.context.config().get_string(
            'analytics.opensearch.logging.search_log_removal_policy', default='DESTROY'
        )
        slow_index_log_removal_policy = self.context.config().get_string(
        ...
        if kms_key_id is not None:
            kms_key_arn = kms.Key.from_key_arn(
                self.stack,
                'opensearch-kms-key',
                self.get_kms_key_arn(key_id=kms_key_id),
            )
        ...
        self.opensearch = OpenSearch(
            ...
            logging=opensearch.LoggingOptions(
                slow_search_log_enabled=self.context.config().get_bool(
                ...
                app_log_enabled=self.context.config().get_bool(
                    'analytics.opensearch.logging.app_log_enabled', required=True
                ),
                ...
                slow_index_log_enabled=self.context.config().get_bool(
                    'analytics.opensearch.logging.slow_index_log_enabled', required=True
                ),
    ...
    def build_dashboard_endpoints(self):
        ...
        dashboard_endpoint_path_patterns = self.context.config().get_list(
            'analytics.opensearch.endpoints.external.path_patterns', required=True
        )
        dashboard_endpoint_priority = self.context.config().get_int(
            'analytics.opensearch.endpoints.external.priority', required=True
        )
        ...
        if is_existing:
            ...
            describe_domain_result = (
                self.context.aws().opensearch().describe_domain(DomainName=domain_name)
            )
        ...
        opensearch_private_ips = CustomResource(
            ...
            name='opensearch-private-ips',
            properties={'DomainName': domain_name, 'UpdateToken': str(uuid.uuid4())},
        )
        ...
        cdk.CfnOutput(
            self.stack,
            'IPAddresses',
            value=opensearch_private_ips.get_att_string('IpAddresses'),
        )
        ...
        cdk.CustomResource(
            ...
            properties={
                ...
                'actions': [
                    {'Type': 'forward', 'TargetGroupArn': dashboard_target_group.ref}
                ],
                'tags': {
    ...
    def build_cluster_settings(self):
        ...
        if self.security_group is not None:
            cluster_settings['opensearch.security_group_id'] = (
