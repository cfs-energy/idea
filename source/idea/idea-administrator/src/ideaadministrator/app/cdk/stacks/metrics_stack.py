...
import constructs
from aws_cdk import aws_aps as aps, aws_cloudwatch as cloudwatch
...
class MetricsStack(IdeaBaseStack):
    ...
    def build_cloudwatch(self):
        dashboard_name = self.context.config().get_string(
            'metrics.cloudwatch.dashboard_name', required=True
        )
        # todo - add widgets based on modules deployed
        self.cloudwatch_dashboard = cloudwatch.Dashboard(
            scope=self.stack, id='cloudwatch-dashboard', dashboard_name=dashboard_name
        )

    def build_amazon_managed_prometheus(self):
        workspace_name = self.context.config().get_string(
            'metrics.amazon_managed_prometheus.workspace_name', required=True
        )
        self.amazon_prometheus_workspace = aps.CfnWorkspace(
            scope=self.stack, id='prometheus-workspace', alias=workspace_name
        )
    ...
    def build_prometheus(self):
        ...
        self.context.config().get_string(
            'metrics.prometheus.remote_write.url', required=True
        )
    ...
    def build_cluster_settings(self):
        ...
        if self.is_cloudwatch():
            cluster_settings['cloudwatch.dashboard_arn'] = (
                self.cloudwatch_dashboard.dashboard_arn
            )
        elif self.is_amazon_managed_prometheus():
            cluster_settings['amazon_managed_prometheus.workspace_id'] = (
                self.amazon_prometheus_workspace.attr_workspace_id
            )
            cluster_settings['amazon_managed_prometheus.workspace_arn'] = (
                self.amazon_prometheus_workspace.attr_arn
            )
            cluster_settings['prometheus.remote_write.url'] = (
                f'{self.amazon_prometheus_workspace.attr_prometheus_endpoint}api/v1/remote_write'
            )
            cluster_settings['prometheus.remote_read.url'] = (
                f'{self.amazon_prometheus_workspace.attr_prometheus_endpoint}api/v1/query'
            )
        elif self.is_prometheus():
            pass
