...
from aws_cdk import aws_ec2 as ec2, aws_route53 as route53, aws_kms as kms
import constructs
...
class BastionHostStack(IdeaBaseStack):
    ...
    def build_ec2_instance(self):
        ...
        base_os = self.context.config().get_string(
            'bastion-host.base_os', required=True
        )
        instance_ami = self.context.config().get_string(
            'bastion-host.instance_ami', required=True
        )
        ...
        volume_size = self.context.config().get_int(
            'bastion-host.volume_size', default=200
        )
        ...
        enable_detailed_monitoring = self.context.config().get_bool(
            'bastion-host.ec2.enable_detailed_monitoring', default=False
        )
        enable_termination_protection = self.context.config().get_bool(
            'bastion-host.ec2.enable_termination_protection', default=False
        )
        metadata_http_tokens = self.context.config().get_string(
            'bastion-host.ec2.metadata_http_tokens', required=True
        )
        ...
        block_device_type_string = self.context.config().get_string(
            'bastion-host.volume_type', default='gp3'
        )
        ...
        user_data = BootstrapUserDataBuilder(
            ...
            install_commands=['/bin/bash bastion-host/setup.sh'],
            ...
            self.add_nag_suppression(
                ...
                suppressions=[
                    IdeaNagSuppression(
                        ...
                        reason='termination protection is a configurable option. Enable termination protection via AWS EC2 console after deploying the cluster if required.',
                    )
                ],
            )
    ...
    def build_route53_record_set(self):
        hostname = self.context.config().get_string(
            'bastion-host.hostname', required=True
        )
    ...
    def build_cluster_settings(self):
        ...
        cluster_settings['instance_profile_arn'] = (
            self.bastion_host_instance_profile.ref
        )
