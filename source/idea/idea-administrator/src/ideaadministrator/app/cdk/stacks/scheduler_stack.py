...
class SchedulerStack(IdeaBaseStack):
    ...
    def build_iam_roles(self):
        ...
        # scheduler
        scheduler_policy_arns = self.context.config().get_list(
        ...
        # compute node
        compute_node_policy_arns = self.context.config().get_list(
        ...
        self.job_status_sqs_queue = SQSQueue(
            ...
            dead_letter_queue=sqs.DeadLetterQueue(
                max_receive_count=Utils.get_as_int(
                    constants.SQS_MAX_RECEIVE_COUNT_SCHEDULER_JOB_STATUS, default=10
                ),
                queue=SQSQueue(
                    self.context,
    ...
    def build_ec2_instance(self):
        ...
        instance_ami = self.context.config().get_string(
            'scheduler.instance_ami', required=True
        )
        ...
        enable_detailed_monitoring = self.context.config().get_bool(
            'scheduler.ec2.enable_detailed_monitoring', default=False
        )
        enable_termination_protection = self.context.config().get_bool(
            'scheduler.ec2.enable_termination_protection', default=False
        )
        metadata_http_tokens = self.context.config().get_string(
            'scheduler.ec2.metadata_http_tokens', required=True
        )
        ...
        block_device_type_string = self.context.config().get_string(
            'scheduler.volume_type', default='gp3'
        )
        ...
        user_data = BootstrapUserDataBuilder(
            ...
            install_commands=['/bin/bash scheduler/setup.sh'],
        ...
        cdk.Tags.of(self.ec2_instance).add(
    ...
    def build_route53_record_set(self):
        ...
        self.cluster_dns_record_set = route53.RecordSet(
            ...
            target=route53.RecordTarget.from_ip_addresses(
                self.ec2_instance.attr_private_ip
            ),
    ...
    def build_endpoints(self):
        ...
        external_endpoint_priority = self.context.config().get_int(
            'scheduler.endpoints.external.priority', required=True
        )
        external_endpoint_path_patterns = self.context.config().get_list(
            'scheduler.endpoints.external.path_patterns', required=True
        )
        ...
        internal_endpoint_priority = self.context.config().get_int(
            'scheduler.endpoints.internal.priority', required=True
        )
        internal_endpoint_path_patterns = self.context.config().get_list(
            'scheduler.endpoints.internal.path_patterns', required=True
        )
    ...
    def build_cluster_settings(self):
        ...
        if is_public:
            cluster_settings.update({'public_ip': self.ec2_instance.attr_public_ip})
