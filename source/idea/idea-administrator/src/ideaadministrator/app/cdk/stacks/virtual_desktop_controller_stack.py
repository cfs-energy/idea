...
class VirtualDesktopControllerStack(IdeaBaseStack):
    ...
    * Additional components for eVDI functionality.
    ...
    def __init__(
        ...
        self.BROKER_CLIENT_COMMUNICATION_PORT = self.context.config().get_int(
            'virtual-desktop-controller.dcv_broker.client_communication_port',
            required=True,
        )
        self.BROKER_AGENT_COMMUNICATION_PORT = self.context.config().get_int(
            'virtual-desktop-controller.dcv_broker.agent_communication_port',
            required=True,
        )
        self.BROKER_GATEWAY_COMMUNICATION_PORT = self.context.config().get_int(
            'virtual-desktop-controller.dcv_broker.gateway_communication_port',
            required=True,
        )

        self.CLUSTER_ENDPOINTS_LAMBDA_ARN = self.context.config().get_string(
        ...
        self.dcv_broker_security_group: Optional[VirtualDesktopBrokerSecurityGroup] = (
            None
        )
    ...
    def setup_egress_rules_for_quic(self):
        quic_supported = self.context.config().get_bool(
            'virtual-desktop-controller.dcv_session.quic_support', required=True
        )
    ...
    def build_sqs_queues(self):
        self.event_sqs_queue = SQSQueue(
            self.context,
            'virtual-desktop-controller-events-queue',
            self.stack,
            ...
            encryption_master_key=self.context.config().get_string(
                'cluster.sqs.kms_key_id'
            ),
            dead_letter_queue=sqs.DeadLetterQueue(
                max_receive_count=Utils.get_as_int(
                    constants.SQS_MAX_RECEIVE_COUNT_VDC_EVENTS, default=16
                ),
                queue=SQSQueue(
                    self.context,
        ...
        kms_key = self.context.config().get_string(
            'cluster.sqs.kms_key_id', default=None
        )
        encrypt_at_rest = Utils.is_not_empty(kms_key)

        self.controller_sqs_queue = SQSQueue(
            self.context,
            'virtual-desktop-controller-queue',
            self.stack,
            ...
            visibility_timeout=cdk.Duration.seconds(
                constants.SQS_VISIBILITY_VDC_CONTROLLER
            ),
            dead_letter_queue=sqs.DeadLetterQueue(
                max_receive_count=Utils.get_as_int(
                    constants.SQS_MAX_RECEIVE_COUNT_VDC_CONTROLLER, default=16
                ),
                queue=SQSQueue(
                    self.context,
    ...
    def build_controller_ssm_commands_notification_infra(self):
        ...
        self.ssm_command_pass_role.attach_inline_policy(
            Policy(
                context=self.context,
    ...
    def subscribe_to_ec2_notification_events(self):
        ec2_event_sns_topic = sns.Topic.from_topic_arn(
            self.stack,
            f'{self.cluster_name}-{self.module_id}-ec2-state-change-topic',
            self.context.config().get_string(
                'cluster.ec2.state_change_notifications_sns_topic_arn', required=True
            ),
        )

        ec2_event_sns_topic.add_subscription(
            sns_subscription.SqsSubscription(
                queue=self.controller_sqs_queue,
                dead_letter_queue=self.controller_sqs_queue.dead_letter_queue.queue,
                filter_policy={
                    IDEA_TAG_MODULE_ID.replace(
                        ':', '_'
    ...
    def build_scheduled_event_notification_infra(self):
        ...
        self.scheduled_event_transformer_lambda_role.attach_inline_policy(
            Policy(
                context=self.context,
        ...
        scheduled_event_transformer_lambda.add_nag_suppression(
            suppressions=[
                IdeaNagSuppression(
            ...
            o_auth=cognito.OAuthSettings(
                ...
                scopes=[
                    ...
                    cognito.OAuthScope.custom(
                        f'{self.context.config().get_module_id(constants.MODULE_CLUSTER_MANAGER)}/read'
                    ),
    ...
    def _build_alb(
        self, component_name: str, internet_facing: bool, security_group: SecurityGroup
    ...
    def build_dcv_broker(self):
        ...
        client_target_group = elbv2.ApplicationTargetGroup(
            ...
            vpc=self.cluster.vpc,
            target_group_name=self.get_target_group_name(
                f'{self.COMPONENT_DCV_BROKER}-c'
            ),
        )
        client_target_group.configure_health_check(enabled=True, path='/health')
        ...
        cdk.CustomResource(
            ...
            properties={
                ...
                'listener_arn': self.context.config().get_string(
        ...
        self.dcv_broker_security_group = VirtualDesktopBrokerSecurityGroup(
            ...
            vpc=self.cluster.vpc,
            bastion_host_security_group=self.cluster.get_security_group('bastion-host'),
            public_loadbalancer_security_group=self.cluster.get_security_group(
        ...
        # autoscaling group
        dcv_broker_package_uri = self.stack.node.try_get_context(
        ...
        broker_userdata = BootstrapUserDataBuilder(
            ...
            install_commands=['/bin/bash dcv-broker/setup.sh'],
            infra_config={
        ...
        substituted_userdata = cdk.Fn.sub(
            broker_userdata,
            {
    ...
    def build_virtual_desktop_controller(self):
        self.controller_security_group = (
            VirtualDesktopPublicLoadBalancerAccessSecurityGroup(
                context=self.context,
                name=f'{self.module_id}-{self.COMPONENT_CONTROLLER}-security-group',
                scope=self.stack,
                vpc=self.cluster.vpc,
                bastion_host_security_group=self.cluster.get_security_group(
                    'bastion-host'
                ),
                public_loadbalancer_security_group=self.cluster.get_security_group(
                    'external-load-balancer'
        ...
        controller_bootstrap_package_uri = self.stack.node.try_get_context(
        ...
        self.controller_auto_scaling_group = self._build_auto_scaling_group(
            ...
            substituted_userdata=cdk.Fn.sub(
                BootstrapUserDataBuilder(
                    ...
                    install_commands=['/bin/bash virtual-desktop-controller/setup.sh'],
                    proxy_config=proxy_config,
                    base_os=self.context.config().get_string(
        ...
        path_patterns = self.context.config().get_list(
            'virtual-desktop-controller.controller.endpoints.external.path_patterns',
            required=True,
        )
        priority = self.context.config().get_int(
    ...
    def _build_auto_scaling_group(
        ...
        iam_role: Role,
        substituted_userdata: str,
        node_type: str,
    ) -> asg.AutoScalingGroup:
        is_public = (
            self.context.config().get_bool(
                f'virtual-desktop-controller.{self.CONFIG_MAPPING[component_name]}.autoscaling.public',
                False,
            )
        ...
        base_os = self.context.config().get_string(
            f'virtual-desktop-controller.{self.CONFIG_MAPPING[component_name]}.autoscaling.base_os',
            required=True,
        )
        block_device_name = Utils.get_ec2_block_device_name(base_os)
        block_device_type_string = self.context.config().get_string(
            'virtual-desktop-controller.volume_type', default='gp3'
        )
        ...
        enable_detailed_monitoring = self.context.config().get_bool(
            f'virtual-desktop-controller.{self.CONFIG_MAPPING[component_name]}.autoscaling.enable_detailed_monitoring',
            default=False,
        )
        metadata_http_tokens = self.context.config().get_string(
            f'virtual-desktop-controller.{self.CONFIG_MAPPING[component_name]}.autoscaling.metadata_http_tokens',
            required=True,
        )
        ...
        launch_template = ec2.LaunchTemplate(
            ...
            instance_type=ec2.InstanceType(
                self.context.config().get_string(
                    f'virtual-desktop-controller.{self.CONFIG_MAPPING[component_name]}.autoscaling.instance_type',
                    required=True,
                )
            ),
            machine_image=ec2.MachineImage.generic_linux(
                {
                    self.aws_region: self.context.config().get_string(
                        f'virtual-desktop-controller.{self.CONFIG_MAPPING[component_name]}.autoscaling.instance_ami',
                        required=True,
                    )
                }
            ),
                    ...
                    volume=ec2.BlockDeviceVolume(
                        ebs_device=ec2.EbsDeviceProps(
                            ...
                            volume_size=self.context.config().get_int(
                                f'virtual-desktop-controller.{self.CONFIG_MAPPING[component_name]}.autoscaling.volume_size',
                                default=200,
                            ),
        ...
        auto_scaling_group = asg.AutoScalingGroup(
            ...
            min_capacity=self.context.config().get_int(
                f'virtual-desktop-controller.{self.CONFIG_MAPPING[component_name]}.autoscaling.min_capacity',
                default=1,
            ),
            max_capacity=self.context.config().get_int(
                f'virtual-desktop-controller.{self.CONFIG_MAPPING[component_name]}.autoscaling.max_capacity',
                default=3,
            ),
            new_instances_protected_from_scale_in=self.context.config().get_bool(
                f'virtual-desktop-controller.{self.CONFIG_MAPPING[component_name]}.autoscaling.new_instances_protected_from_scale_in',
                default=True,
            ),
            cooldown=cdk.Duration.minutes(
                self.context.config().get_int(
                    f'virtual-desktop-controller.{self.CONFIG_MAPPING[component_name]}.autoscaling.cooldown_minutes',
                    default=5,
                )
            ),
            health_checks=asg.HealthChecks.with_additional_checks(
                additional_types=[asg.AdditionalHealthCheckType.ELB],
                grace_period=cdk.Duration.minutes(
                    self.context.config().get_int(
                        f'virtual-desktop-controller.{self.CONFIG_MAPPING[component_name]}.autoscaling.elb_healthcheck.grace_time_minutes',
                        default=15,
                    )
                ),
            ),
            update_policy=asg.UpdatePolicy.rolling_update(
                max_batch_size=self.context.config().get_int(
                    f'virtual-desktop-controller.{self.CONFIG_MAPPING[component_name]}.autoscaling.rolling_update_policy.max_batch_size',
                    default=1,
                ),
                min_instances_in_service=self.context.config().get_int(
                    f'virtual-desktop-controller.{self.CONFIG_MAPPING[component_name]}.autoscaling.rolling_update_policy.min_instances_in_service',
                    default=1,
                ),
                pause_time=cdk.Duration.minutes(
                    self.context.config().get_int(
                        f'virtual-desktop-controller.{self.CONFIG_MAPPING[component_name]}.autoscaling.rolling_update_policy.pause_time_minutes',
                        default=15,
                    )
                ),
            ),
        ...
        auto_scaling_group.scale_on_cpu_utilization(
            ...
            target_utilization_percent=self.context.config().get_int(
                f'virtual-desktop-controller.{self.CONFIG_MAPPING[component_name]}.autoscaling.cpu_utilization_scaling_policy.target_utilization_percent',
                default=80,
            ),
            estimated_instance_warmup=cdk.Duration.minutes(
                self.context.config().get_int(
                    f'virtual-desktop-controller.{self.CONFIG_MAPPING[component_name]}.autoscaling.cpu_utilization_scaling_policy.estimated_instance_warmup_minutes',
                    default=15,
                )
            ),
        )
        ...
        cdk.Tags.of(auto_scaling_group).add(
            ...
            f'{self.cluster_name}-{self.module_id}-{component_name}',
        )
    ...
    def _build_dcv_connection_gateway_instance_infrastructure(self):
        ...
        dcv_connection_gateway_bootstrap_package_uri = self.stack.node.try_get_context(
        ...
        connection_gateway_userdata = BootstrapUserDataBuilder(
            ...
            install_commands=['/bin/bash dcv-connection-gateway/setup.sh'],
            infra_config={
        ...
        external_certificate_provided = self.context.config().get_bool(
            'virtual-desktop-controller.dcv_connection_gateway.certificate.provided',
            required=True,
        )
        if not external_certificate_provided:
            substituted_userdata = cdk.Fn.sub(
                connection_gateway_userdata,
                {
                    '__CERTIFICATE_SECRET_ARN__': self.dcv_connection_gateway_self_signed_cert.get_att_string(
                        'certificate_secret_arn'
                    ),
                    '__PRIVATE_KEY_SECRET_ARN__': self.dcv_connection_gateway_self_signed_cert.get_att_string(
            ...
            substituted_userdata = cdk.Fn.sub(
                connection_gateway_userdata,
                {
                    '__CERTIFICATE_SECRET_ARN__': self.context.config().get_string(
                        'virtual-desktop-controller.dcv_connection_gateway.certificate.certificate_secret_arn',
                        required=True,
                    ),
                    '__PRIVATE_KEY_SECRET_ARN__': self.context.config().get_string(
    ...
    def _build_dcv_connection_gateway_network_infrastructure(self):
        is_public = self.context.config().get_bool(
            'cluster.load_balancers.external_alb.public', default=True
        )
        external_nlb_subnets = (
            self.cluster.public_subnets
            if is_public is True
            else self.cluster.private_subnets
        )
        ...
        # enable access logs
        external_alb_enable_access_log = self.context.config().get_bool(
            'virtual-desktop-controller.external_nlb.access_logs', default=False
        )
        if external_alb_enable_access_log:
            access_log_destination = s3.Bucket.from_bucket_name(
                scope=self.stack,
                id='cluster-s3-bucket',
                bucket_name=self.context.config().get_string(
                    'cluster.cluster_s3_bucket', required=True
                ),
            )
            self.external_nlb.log_access_logs(
                access_log_destination,
                f'logs/{self.module_id}/external-nlb-access-logs',
            )
        ...
        elbv2.NetworkListener(
            ...
            default_action=elbv2.NetworkListenerAction.forward(
                target_groups=[dcv_connection_gateway_target_group]
            ),
        )
        ...
        cluster_prefix_list_id = self.context.config().get_string(
            'cluster.network.cluster_prefix_list_id', required=True
        )
        self.dcv_connection_gateway_security_group.add_ingress_rule(
        ...
        prefix_list_ids = self.context.config().get_list(
            'cluster.network.prefix_list_ids', default=[]
        )
    ...
    def _build_self_signed_cert_for_dcv_connection_gateway(self):
        self_signed_certificate_lambda_arn = self.context.config().get_string(
            'cluster.self_signed_certificate_lambda_arn', required=True
        )
        self.dcv_connection_gateway_self_signed_cert = cdk.CustomResource(
            ...
            properties={
                ...
                'tags': {
                    ...
                    'idea:ClusterName': self.cluster_name,
    ...
    def build_backups(self):
        cluster_backups_enabled = self.context.config().get_string(
            'cluster.backups.enabled', default=False
        )
        if not cluster_backups_enabled:
            return

        vdi_host_backup_enabled = self.context.config().get_bool(
            'virtual-desktop-controller.vdi_host_backup.enabled', default=False
        )
        if not vdi_host_backup_enabled:
            return

        backup_vault_arn = self.context.config().get_string(
            'cluster.backups.backup_vault.arn', required=True
        )
        backup_role_arn = self.context.config().get_string(
            'cluster.backups.role_arn', required=True
        )

        backup_role = iam.Role.from_role_arn(self.stack, 'backup-role', backup_role_arn)
        backup_vault = backup.BackupVault.from_backup_vault_arn(
            self.stack, 'cluster-backup-vault', backup_vault_arn
        )

        backup_plan_config = self.context.config().get_config(
            'virtual-desktop-controller.vdi_host_backup.backup_plan'
        )

        self.backup_plan = BackupPlan(
            self.context,
            'vdi-host-backup-plan',
            self.stack,
    ...
    def build_cluster_settings(self):
        cluster_settings = {
            ...
            'dcv_host_instance_profile_arn': self.build_instance_profile_arn(
                self.dcv_host_instance_profile.ref
            ),
        ...
        if not self.context.config().get_bool(
            ...
            ] = self.dcv_connection_gateway_self_signed_cert.get_att_string(
                'certificate_secret_arn'
            )
            cluster_settings[
