...
class ClusterStack(IdeaBaseStack):
    ...
    Provisions base infrastructure components for IDEA Cluster:
    ...
    * Cluster Settings
    ...
    def build_backups(self):
        enable_backup = self.context.config().get_bool(
            'cluster.backups.enabled', default=False
        )
        if not enable_backup:
            return

        enable_restore = self.context.config().get_bool(
            'cluster.backups.enable_restore', default=True
        )
        ...
        # backup vault
        backup_vault_removal_policy = self.context.config().get_string(
            'cluster.backups.backup_vault.removal_policy', default='DESTROY'
        )
        backup_vault_kms_key_id = self.context.config().get_string(
            'cluster.backups.backup_vault.kms_key_id', default=None
        )
        backup_vault_encryption_key = None
        if Utils.is_not_empty(backup_vault_kms_key_id):
            backup_vault_encryption_key = kms.Key.from_key_arn(
                self.stack,
                'backup-vault-kms-key',
                self.get_kms_key_arn(key_id=backup_vault_kms_key_id),
            )
    ...
    def build_vpc(self):
        ...
        if self.context.config().get_bool('cluster.network.use_existing_vpc', False):
            self._existing_vpc = ExistingVpc(
                context=self.context, name='existing-vpc', scope=self.stack
            )
        else:
    ...
    def build_private_hosted_zone(self):
        self.private_hosted_zone = PrivateHostedZone(
            context=self.context, scope=self.stack, vpc=self.vpc
        )
    ...
    def build_cluster_prefix_list(self):
        ...
        Use `idea-admin.sh utils cluster-prefix-list --help` to manage external access to the cluster.
        """
        cluster_prefix_list_max_entries = self.context.config().get_int(
            'cluster.network.cluster_prefix_list_max_entries', 10
        )
        ...
        lambda_function.add_nag_suppression(
            suppressions=[
                IdeaNagSuppression(
    ...
    def build_security_groups(self):
        ...
        if len(nat_eips) > 0:
            external_loadbalancer_security_group.add_nat_gateway_ips_ingress_rule(
                nat_eips
            )

        # vpc endpoint
        use_existing_vpc = self.context.config().get_bool(
            'cluster.network.use_existing_vpc'
        )
        use_vpc_endpoints = self.context.config().get_bool(
    ...
    def build_vpc_endpoints(self):
        ...
        use_existing_vpc = self.context.config().get_bool(
            'cluster.network.use_existing_vpc', False
        )
        if use_existing_vpc:
            return
        ...
        gateway_endpoints = self.context.config().get_list(
        ...
        interface_endpoints = self.context.config().get_config(
            'cluster.network.vpc_interface_endpoints', default={}
        )
        for service in interface_endpoints:
            enabled = Utils.get_value_as_bool(
                'enabled', interface_endpoints[service], default=False
            )
            if not enabled:
                continue
    ...
    def build_cluster_settings_lambda(self):
        ...
        cluster_settings_lambda_role.attach_inline_policy(
            self.cluster_settings_lambda_policy
        )
        self.cluster_settings_lambda.node.add_dependency(
            self.cluster_settings_lambda_policy
        )
        self.cluster_settings_lambda.node.add_dependency(cluster_settings_lambda_role)

        self.cluster_settings_lambda.add_nag_suppression(
            suppressions=[
                IdeaNagSuppression(
                    rule_id='AwsSolutions-L1',
                    reason='Python Runtime is selected for stability.',
    ...
    def build_solution_metrics_lambda(self):
        ...
        solution_metrics_lambda_role.attach_inline_policy(
            self.solution_metrics_lambda_policy
        )
        self.solution_metrics_lambda.node.add_dependency(
            self.solution_metrics_lambda_policy
        )
        self.solution_metrics_lambda.node.add_dependency(solution_metrics_lambda_role)

        self.solution_metrics_lambda.add_nag_suppression(
            suppressions=[
                IdeaNagSuppression(
                    rule_id='AwsSolutions-L1',
                    reason='Python Runtime is selected for stability.',
    ...
    def build_self_signed_certificates_lambda(self):
        ...
        self_signed_certificate_role.attach_inline_policy(
            self_signed_certificate_policy
        )
        ...
        # and deletion fails.
        self.self_signed_certificate_lambda.node.add_dependency(
            self_signed_certificate_policy
        )
        self.self_signed_certificate_lambda.node.add_dependency(
            self_signed_certificate_role
        )

        self.self_signed_certificate_lambda.add_nag_suppression(
            suppressions=[
                IdeaNagSuppression(
                    rule_id='AwsSolutions-L1',
                    reason='Python Runtime is selected for stability.',
    ...
    def build_self_signed_certificates(self):
        external_certificate_provided = self.context.config().get_bool(
            'cluster.load_balancers.external_alb.certificates.provided', False
        )
        ...
        private_hosted_zone_name = self.context.config().get_string(
    ...
    def get_alb_listener_default_actions(
        self, listener_arn: str = None
    ) -> List[elbv2.CfnListener.ActionProperty]:
        """
        ...
        ** Note **
        ...
        if Utils.is_not_empty(listener_arn):
            describe_listeners_result = (
                self.context.aws()
                .elbv2()
                .describe_listeners(ListenerArns=[listener_arn])
            )
            ...
            default_actions = [
                elbv2.CfnListener.ActionProperty(
                    ...
                    fixed_response_config=elbv2.CfnListener.FixedResponseConfigProperty(
                        ...
                        message_body=Utils.to_json({'success': True, 'message': 'OK'}),
                    ),
                )
            ]
    ...
    def build_ec2_notification_module(self):
        ...
        ec2_state_event_transformation_lambda_role.attach_inline_policy(
            Policy(
                context=self.context,
                name=f'{lambda_name}-policy',
                scope=self.stack,
        ...
        ec2_state_event_transformation_lambda.add_nag_suppression(
            suppressions=[
                IdeaNagSuppression(
                    rule_id='AwsSolutions-L1',
                    reason='Python Runtime is selected for stability.',
    ...
    def build_cluster_endpoints(self):
        ...
        self.cluster_endpoints_lambda.add_nag_suppression(
            suppressions=[
                IdeaNagSuppression(
                    rule_id='AwsSolutions-L1',
                    reason='Python Runtime is selected for stability.',
        ...
        external_alb_subnets = (
            self.public_subnets() if is_public is True else self.private_subnets()
        )
        ...
        external_alb_enable_access_log = self.context.config().get_bool(
            'cluster.load_balancers.external_alb.access_logs', default=False
        )
        ...
        if external_alb_enable_access_log:
            self.external_alb.log_access_logs(
                ...
                f'logs/{self.module_id}/alb-access-logs/external-alb',
            )
        if internal_alb_enable_access_log:
            self.internal_alb.log_access_logs(
                access_log_destination,
                f'logs/{self.module_id}/alb-access-logs/internal-alb',
            )
        ...
        external_alb_listener_arn = self.context.config().get_string(
            'cluster.load_balancers.external_alb.https_listener_arn'
        )
        external_alb_default_actions = self.get_alb_listener_default_actions(
            external_alb_listener_arn
        )

        if self.external_certificate is None:
            external_acm_certificate_arn = self.context.config().get_string(
                'cluster.load_balancers.external_alb.certificates.acm_certificate_arn',
                required=True,
            )
        else:
            external_acm_certificate_arn = self.external_certificate.get_att_string(
                'acm_certificate_arn'
            )
        ...
        self.external_alb_https_listener = elbv2.CfnListener(
            ...
            'https-listener',
            port=443,
            ssl_policy=self.context.config().get_string(
        ...
        if self.external_certificate is not None:
            self.external_alb_https_listener.node.add_dependency(
                self.external_certificate
            )

        self.internal_alb.node.add_dependency(self.internal_certificate)

        internal_acm_certificate_arn = self.internal_certificate.get_att_string(
            'acm_certificate_arn'
        )
        ...
        if self.aws_region in Utils.get_value_as_list(
            'ROUTE53_CROSS_ZONE_ALIAS_RESTRICTED_REGION_LIST', constants.CAVEATS, []
        ):
        ...
        if self.context.config().is_module_enabled(
            constants.MODULE_VIRTUAL_DESKTOP_CONTROLLER
        ):
            dcv_broker_client_listener_arn = self.context.config().get_string(
                'cluster.external_alb.dcv_broker_client_listener_arn'
            )
            dcv_broker_client_listener_default_actions = (
                self.get_alb_listener_default_actions(dcv_broker_client_listener_arn)
            )
            dcv_broker_client_communication_port = self.context.config().get_int(
            ...
            dcv_broker_agent_listener_arn = self.context.config().get_string(
                'cluster.external_alb.dcv_broker_agent_listener_arn'
            )
            dcv_broker_agent_listener_default_actions = (
                self.get_alb_listener_default_actions(dcv_broker_agent_listener_arn)
            )
            dcv_broker_agent_communication_port = self.context.config().get_int(
            ...
            dcv_broker_gateway_listener_arn = self.context.config().get_string(
                'cluster.external_alb.dcv_broker_gateway_listener_arn'
            )
            dcv_broker_gateway_listener_default_actions = (
                self.get_alb_listener_default_actions(dcv_broker_gateway_listener_arn)
            )
            dcv_broker_gateway_communication_port = self.context.config().get_int(
    ...
    def build_cluster_settings(self):
        ...
        public_subnets = self.context.config().get_list(
            'cluster.network.public_subnets', []
        )
        is_external_alb_public = self.context.config().get_bool(
            'cluster.load_balancers.external_alb.public', default=True
        )
        ...
        private_subnets = self.context.config().get_list(
        ...
        cluster_settings['iam.policies.amazon_ssm_managed_instance_core_arn'] = (
            self.amazon_ssm_managed_instance_core_policy.managed_policy_arn
        )
        cluster_settings['iam.policies.cloud_watch_agent_server_arn'] = (
            self.cloud_watch_agent_server_policy.managed_policy_arn
        )
        if self.amazon_prometheus_remote_write_policy is not None:
            cluster_settings['iam.policies.amazon_prometheus_remote_write_arn'] = (
                self.amazon_prometheus_remote_write_policy.managed_policy_arn
            )

        cluster_settings['solution.solution_metrics_lambda_arn'] = (
            self.solution_metrics_lambda.function_arn
        )
        ...
        cluster_settings['self_signed_certificate_lambda_arn'] = (
            self.self_signed_certificate_lambda.function_arn
        )

        # route53 - private hosted zone settings
        cluster_settings['route53.private_hosted_zone_id'] = (
            self.private_hosted_zone.hosted_zone_id
        )
        cluster_settings['route53.private_hosted_zone_arn'] = (
            self.private_hosted_zone.hosted_zone_arn
        )

        # certificates
        if not self.context.config().get_bool(
            'cluster.load_balancers.external_alb.certificates.provided', required=True
        ):
            cluster_settings[
                'load_balancers.external_alb.certificates.certificate_secret_arn'
            ] = self.external_certificate.get_att_string('certificate_secret_arn')
            cluster_settings[
                'load_balancers.external_alb.certificates.private_key_secret_arn'
            ] = self.external_certificate.get_att_string('private_key_secret_arn')
            cluster_settings[
                'load_balancers.external_alb.certificates.acm_certificate_arn'
            ] = self.external_certificate.get_att_string('acm_certificate_arn')
        else:
        ...
        if self.vpc_interface_endpoints is not None:
            for service in self.vpc_interface_endpoints:
                endpoint_config_key = (
                    f'network.vpc_interface_endpoints.{service}.endpoint_url'
                )
                existing_endpoint_url = self.context.config().get_string(
                    f'cluster.{endpoint_config_key}'
                )
        ...
        if self.context.config().get_bool('cluster.backups.enabled', default=False):
            ...
            cluster_settings['backups.backup_vault.arn'] = (
                self.backup_vault.backup_vault_arn
            )
            cluster_settings['backups.backup_plan.arn'] = (
