...
class IdeaBaseStack(SocaBaseConstruct):
    def __init__(
        ...
        tags = {**tags, **{constants.IDEA_TAG_CLUSTER_NAME: self.cluster_name}}
    ...
    def get_target_group_name(self, identifier: str) -> str:
        ...
        if len(target_group_name) > 32:
            raise exceptions.invalid_params(
                f'target group name: {target_group_name} cannot more more than 32 characters.'
            )
        return target_group_name

    def update_cluster_settings(self, cluster_settings: Dict):
        cluster_settings_lambda_arn = self.context.config().get_string(
            'cluster.cluster_settings_lambda_arn', required=True
        )
        cdk.CustomResource(
            self.stack,
    ...
    def get_ec2_instance_managed_policies(self) -> List[str]:
        ec2_managed_policies = [
            ...
            self.context.config().get_string(
                'cluster.iam.policies.cloud_watch_agent_server_arn', required=True
            ),
        ]

        if self.is_metrics_provider_amazon_managed_prometheus():
            ec2_managed_policies.append(
                self.context.config().get_string(
                    'cluster.iam.policies.amazon_prometheus_remote_write_arn',
                    required=True,
                )
            )

        ec2_managed_policy_arns = self.context.config().get_list(
    ...
    def build_access_control_groups(self, user_pool: cognito.IUserPool):
        ...
        cognito.CfnUserPoolGroup(
            ...
            group_name=group_name_helper.get_module_administrators_group(
                self.module_id
            ),
            precedence=3,
