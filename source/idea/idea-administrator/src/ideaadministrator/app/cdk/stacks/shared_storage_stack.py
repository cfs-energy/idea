...
class FileSystemHolder:
    ...
    def file_system_dns(self) -> str:
        ...
        aws_dns_suffix = self._context.config().get_string(
            'cluster.aws.dns_suffix', required=True
        )
        return (
            f'{self.file_system_id}.{self.get_fs_type()}.{aws_region}.{aws_dns_suffix}'
        )
...
class SharedStorageStack(IdeaBaseStack):
     ...
     * Apps and Data file systems are mandatory and must exist in configuration.
    """
    ...
    def __init__(
        ...
        # verify if `apps` and `data` storage is configured
        # simple check to ensure apps and data config keys exist
        self.context.config().get_string('shared-storage.apps.provider', required=True)
        self.context.config().get_string('shared-storage.data.provider', required=True)
    ...
    def build_shared_storage(self):
        ...
        provisioning of new file systems is not limited to Apps and Data.
        """
        storage_configs = self.context.config().get_config(
            'shared-storage', required=True
        )
        for key in storage_configs:
            ...
            if provider not in constants.SUPPORTED_STORAGE_PROVIDERS:
                raise exceptions.cluster_config_error(
                    f'file system provider: {provider} not supported'
                )

            # if existing file system, add to registry and skip provisioning.
            existing_fs = storage_config.get(
                f'{provider}.use_existing_fs', default=False
            )
            if existing_fs:
                file_system_id = storage_config.get(f'{provider}.file_system_id')
                if Utils.is_empty(file_system_id):
                    raise exceptions.cluster_config_error(
                        f'shared-storage.{key}.{provider}.file_system_id is required'
                    )
    ...
    def build_cluster_settings(self):
        ...
        for file_system in self.file_systems:
            ...
            # if existing file systems configurations are added/updated via ./idea-admin.sh shared-storage utility, dns property will already exist.
            if file_system.provider in [
                constants.STORAGE_PROVIDER_EFS,
                constants.STORAGE_PROVIDER_FSX_LUSTRE,
            ]:
                file_system_dns = self.context.config().get_string(
                    f'shared-storage.{self.name}.dns'
                )
                if Utils.is_empty(file_system_dns):
                    cluster_settings[
                        f'{file_system.name}.{file_system.provider}.dns'
                    ] = file_system.file_system_dns
            ...
            cluster_settings[
                f'{file_system.name}.{file_system.provider}.file_system_id'
