...
class OpenSearch(SocaBaseConstruct, opensearch.Domain):
    def __init__(
        ...
        fine_grained_access_control: Optional[
        ...
        if vpc_subnets is None:
            vpc_subnets = [
                ec2.SubnetSelection(subnets=cluster.private_subnets[0:data_nodes])
            ]

        if zone_awareness is None:
            if data_nodes > 1:
                zone_awareness = opensearch.ZoneAwarenessConfig(
                    enabled=True, availability_zone_count=min(3, data_nodes)
                )
            else:
        ...
        if encryption_at_rest is None:
            encryption_at_rest = opensearch.EncryptionAtRestOptions(
                enabled=True, kms_key=kms_key_arn
            )

        if ebs is None:
            ebs = opensearch.EbsOptions(
                volume_size=ebs_volume_size, volume_type=ec2.EbsDeviceVolumeType.GP3
            )

        if capacity is None:
            capacity = opensearch.CapacityConfig(
                data_node_instance_type=data_node_instance_type, data_nodes=data_nodes
            )
        ...
        if access_policies is None:
            ...
            access_policies = [
                iam.PolicyStatement(
                    ...
                    actions=['es:ESHttp*'],
                    resources=[arn_builder.get_arn('es', f'domain/{domain_name}/*')],
                )
            ]
        ...
        if create_service_linked_role:
            aws_service_name = self.context.config().get_string(
                'global-settings.opensearch.aws_service_name'
            )
        ...
        self.add_nag_suppression(
            suppressions=[
                ...
                IdeaNagSuppression(
                    ...
                    reason='Use existing resources flow to provision an even more scalable OpenSearch cluster with dedicated master nodes',
                ),
                IdeaNagSuppression(
