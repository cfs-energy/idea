...
class BastionHostSecurityGroup(SecurityGroup):
    ...
    def setup_ingress(self):
        ...
        prefix_list_ids = self.context.config().get_list('cluster.network.prefix_list_ids')
...
class SharedStorageSecurityGroup(SecurityGroup):
    ...
    def __init__(self, context: AdministratorContext, name: str, scope: constructs.Construct, vpc: ec2.IVpc):
        super().__init__(context, name, scope, vpc, description='Shared Storage security group for EFS/FSx file systems')
        self.setup_ingress()
        self.setup_egress()
