...
from aws_cdk import aws_iam as iam, aws_backup as backup, aws_events as events
...
class BackupPlan(SocaBaseConstruct):
    ...
    def _build_backup_plan_rules(self) -> List[backup.BackupPlanRule]:
        ...
        for rule_name, rule in rules.items():
            ...
            completion_window_minutes = Utils.get_value_as_int(
                'completion_window_minutes', rule
            )
            schedule_expression = Utils.get_value_as_string('schedule_expression', rule)
            move_to_cold_storage_after_days = Utils.get_value_as_int(
                'move_to_cold_storage_after_days', rule, default=None
            )
            ...
            if move_to_cold_storage_after_days is not None:
                move_to_cold_storage_after = cdk.Duration.days(
    ...
    def _build_backup_plan(self):
        ...
        # build backup plan
        backup_plan_enable_windows_vss = Utils.get_value_as_bool(
            'enable_windows_vss', self.backup_plan_config, default=False
        )
        ...
        selection = Utils.get_value_as_dict(
            'selection', self.backup_plan_config, default={}
        )
        ...
        # Create backup selection with disable_default_backup_policy=True to prevent CDK
        # from automatically attaching AWSBackupServiceRolePolicyForBackup to imported roles.
        # The imported backup role already has the necessary policies attached.
        backup_selection = backup.BackupSelection(
            self.scope,
            f'{self.backup_plan_name}-selection',
