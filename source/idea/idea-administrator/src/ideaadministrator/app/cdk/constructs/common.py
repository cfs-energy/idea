...
class LambdaFunction(SocaBaseConstruct, lambda_.Function):
    ...
    def __init__(
        ...
        if Utils.is_empty(idea_code_asset):
            if Utils.are_empty(code, handler):
                raise exceptions.invalid_params(
                    'Provide either idea_code_asset or (code and handler)'
                )
        else:
        ...
        if len(function_name) > self.MAX_NAME_LENGTH:
            function_name = self.build_trimmed_resource_name(
                name, trim_length=self.MAX_NAME_LENGTH
            )
        ...
        self.add_nag_suppression(
            suppressions=[
                IdeaNagSuppression(
                    ...
                    reason='Lambda runtime uses Python 3.13 by default.',
                )
            ]
        )
    ...
    def __init__(
        ...
        self.add_nag_suppression(
            suppressions=[
                IdeaNagSuppression(
                    rule_id='AwsSolutions-IAM5',
                    reason='AWS Managed Policies are expected to be customized and scoped down.'
...
class Role(SocaBaseConstruct, iam.Role):
    ...
    def __init__(
        ...
        if len(role_name) > self.MAX_NAME_LENGTH:
            role_name = self.build_trimmed_resource_name(
                name, region_suffix=True, trim_length=self.MAX_NAME_LENGTH
            )
            ...
            for policy in managed_policies:
                if policy.startswith('arn:'):
                    ...
                    self.add_managed_policy(
                        iam.ManagedPolicy.from_managed_policy_arn(self, name, policy)
                    )
                else:
                    self.add_managed_policy(
...
class InstanceProfile(SocaBaseConstruct, iam.CfnInstanceProfile):
    def __init__(
        ...
        super().__init__(
            ...
            name,
            scope,
            instance_profile_name=self.build_resource_name(name, region_suffix=True),
            roles=role_names,
        )
...
class CustomResource(SocaBaseConstruct):
    ...
    def build_lambda_function(self):
        ...
        self.lambda_function.add_nag_suppression(
            suppressions=[
                IdeaNagSuppression(
                    rule_id='AwsSolutions-L1',
...
class CreateTagsCustomResource(CustomResource):
    ...
    def apply(
        self, name: str, resource_id: str, tags: Dict[str, Any]
    ) -> cdk.CustomResource:
        aws_tags = []
        for key, value in tags.items():
            aws_tags.append({'Key': key, 'Value': str(value)})
        return super().invoke(
            name, properties={'ResourceId': resource_id, 'Tags': aws_tags}
        )
        ...
        *,
        ...
        if encrypt_at_rest:
            ...
            if Utils.is_not_empty(encryption_master_key):
                ...
                encryption_master_key_ = kms.Key.from_key_arn(
                    scope=scope, id=f'{id_}-kms-key', key_arn=key_arn
                )
        ...
        if is_dead_letter_queue:
            self.add_nag_suppression(
                suppressions=[
                    IdeaNagSuppression(
                        rule_id='AwsSolutions-SQS3', reason='Dead letter queue'
                    )
                ]
            )
...
class SNSSubscription(SocaBaseConstruct, sns.Subscription):
    def __init__(
        ...
        super().__init__(
            context, name, scope, protocol=protocol, endpoint=endpoint, topic=topic
        )
...
class KinesisStream(SocaBaseConstruct, kinesis.Stream):
    def __init__(
        ...
        if kms_key_id is not None:
            ...
            kinesis_encryption_key = kms.Key.from_key_arn(
                scope=scope, id='kinesis-kms-key', key_arn=kms_key_arn
            )
        else:
            kinesis_encryption_key = kms.Alias.from_alias_name(
                scope=scope,
                id='kinesis-kms-key-default',
                alias_name='alias/aws/kinesis',
            )
