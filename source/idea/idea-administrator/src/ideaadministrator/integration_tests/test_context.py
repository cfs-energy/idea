...
from ideadatamodel import AuthResult, InitiateAuthRequest, InitiateAuthResult
...
class TestContext:
    def __init__(
        ...
        self.filter_test_case_ids = (
            test_case_ids if Utils.is_not_empty(test_case_ids) else None
        )
        ...
        self.virtual_desktop_controller_active_module_id = (
            self.cluster_config.get_module_id(
        ...
        cluster_manager_module_id = self.cluster_config.get_module_id(
            constants.MODULE_CLUSTER_MANAGER
        )
        self._admin_http_client = SocaClient(
            context=self.idea_context,
    ...
    def get_client(self, module_name: str, timeout: Optional[int] = 10) -> SocaClient:
        ...
        if module_info is None:
            raise exceptions.general_exception(
                f'module not found for module id: {module_id}'
            )
        ...
        if Utils.is_empty(api_path):
            raise exceptions.general_exception(
                f'could not find api context path for module: {module_id}'
            )
        ...
        if self.virtual_desktop_controller_active_module_id in self.module_ids:
            if 'test_username' in extra_params and 'test_password' in extra_params:
                if Utils.is_not_empty(
                    extra_params.get('test_username')
                ) and Utils.is_not_empty(extra_params.get('test_password')):
                    self.non_admin_username = extra_params.get('test_username')
                    self.non_admin_password = extra_params.get('test_password')
                    return True
                else:
                    raise exceptions.general_exception(
                        f"Invalid 'test_username' or 'test_password' value. Required parameter to execute {self.virtual_desktop_controller_active_module_id} tests"
                    )
            else:
                raise exceptions.general_exception(
                    f"Missing Parameter 'test_username' or 'test_password'. Required parameter to execute {self.virtual_desktop_controller_active_module_id} tests."
                )

    def get_virtual_desktop_controller_client(
    ...
    def get_non_admin_access_token(self):
        if self.non_admin_auth is None:
            raise exceptions.general_exception(
                'non admin authentication not initialized'
            )
