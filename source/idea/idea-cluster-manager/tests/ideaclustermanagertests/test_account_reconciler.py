...
from ideaclustermanager.app.accounts.account_reconciler import AccountReconciler, ReconcileUsersRequest
...
def build(values=None, records=None, users=None):
    context = FakeContext({'metrics.provider': 'cloudwatch', 'directoryservice.provider': 'activedirectory', **(values or {})})
    users = users or [User(username=f'user{index}', enabled=True) for index in range(4)]
    records = records if records is not None else {'user0': {'user_account_control': 514}}
    accounts = Mock()
    accounts.ldap_client.ldap_root_username = 'bind-service'
    accounts.ldap_client.get_user.side_effect = lambda username, **kw: records.get(username, {'user_account_control': 512})
    accounts.is_cluster_administrator.side_effect = lambda username: username == 'cluster-admin'
...
@pytest.mark.parametrize('record,missing', [({'user_account_control': 514}, 0), (None, 1)])
...
def test_incomplete_read_refuses_whole_run(control):
    service, context = build(records={'user0': None, 'user1': {'user_account_control': control}})
    report = service.run_once(dry_run=False)
...
def test_reenable(reenable):
    service, context = build({PREFIX + 'reenable': reenable, PREFIX + 'check_cognito': True}, records={}, users=[User(username='user0', enabled=False)])
    report = service.run_once(dry_run=False)
...
@pytest.mark.parametrize('status', ['DEPROVISIONED', 'SUSPENDED', 'DEACTIVATED', 'ACTIVE', 'STAGED'])
def test_okta(status, monkeypatch):
    service, context = build({PREFIX + 'okta.org_url': 'https://id.example.invalid', PREFIX + 'okta.api_token_secret_arn': 'secret-ref'}, records={})
    context._config.secrets['secret-ref'] = 'test-token'
    response = Mock(status_code=200)
    response.json.return_value = {'status': status}
    get = Mock(return_value=response)
    monkeypatch.setattr('ideaclustermanager.app.accounts.account_reconciler.requests.get', get)
    report = service.run_once()
    assert len(report['changes']) == (4 if status in ('DEPROVISIONED', 'SUSPENDED', 'DEACTIVATED') else 0)
    assert get.call_args.kwargs['allow_redirects'] is False
    assert get.call_args.kwargs['timeout'] == 15


@pytest.mark.parametrize('status,missing,errors', [(404, 4, 0), (429, 0, 4), (403, 0, 4), (500, 0, 4)])
def test_okta_errors_are_not_missing(status, missing, errors, monkeypatch):
    service, context = build({PREFIX + 'okta.org_url': 'https://id.example.invalid', PREFIX + 'okta.api_token_secret_arn': 'secret-ref'}, records={})
    context._config.secrets['secret-ref'] = 'test-token'
    monkeypatch.setattr('ideaclustermanager.app.accounts.account_reconciler.requests.get', Mock(return_value=Mock(status_code=status)))
    report = service.run_once(dry_run=False)
...
def test_cognito_fresh_native_read():
    service, context = build({'directoryservice.provider': 'openldap', PREFIX + 'check_cognito': True})
    context.accounts.user_pool.admin_get_user.side_effect = [CognitoUser(Enabled=False)] + [CognitoUser(Enabled=True)] * 3
    assert service.run_once(dry_run=False)['disabled'] == 1
    assert all(call.kwargs == {'use_cache': False} for call in context.accounts.user_pool.admin_get_user.call_args_list)


def test_protected_and_pagination():
    service, context = build()
    context.accounts.list_users.side_effect = [
        ListUsersResult(listing=[User(username='cluster-admin', enabled=True), User(username='bind-service', enabled=True), User(username='ideaserviceaccount', enabled=True)], paginator=SocaPaginator(cursor='next')),
        ListUsersResult(listing=[User(username=f'user{i}', enabled=True) for i in range(4)]),
    ]
    report = service.run_once()
...
def test_metrics_have_all_families_on_both_providers(provider):
    ...
    assert {entry['MetricName'] for entry in context.published()} == {
        f'accounts.reconcile.{name}' for name in ('checked', 'disabled', 'reenabled', 'missing', 'errors', 'refused')
    }
...
def test_override_does_not_bypass_read_errors():
    service, context = build(records={'user0': None, 'user1': {'user_account_control': None}})
...
def test_periodic_never_overrides():
    service, context = build({PREFIX + 'dry_run': False}, records={'user0': None, 'user1': None})
...
def test_api_override_requires_admin_and_logs_actor(admin):
    ...
    invocation.get_request_payload_as.return_value = ReconcileUsersRequest(dry_run=False, override_max_disable_fraction=True)
    if admin:
...
@pytest.mark.parametrize('minutes,seconds', [(0, 60), (1, 60), (60, 3600), (1440, 86400), (1441, 86400)])
