    ...
    getFormattedValue() {
        ...
        switch (this.props.type) {
            ...
            case 'ec2:asg-arn':
                ...
                return <Link external={true} href={Utils.getASGUrl(AppContext.get().auth().getAwsRegion(), name)}>{value}</Link>
            case 'ec2:asg-name':
                return <Link external={true} href={Utils.getASGUrl(AppContext.get().auth().getAwsRegion(), Utils.asString(value))}>{value}</Link>
            case 'cognito:user-pool-id':
                return <Link external={true} href={Utils.getCognitoUserPoolUrl(AppContext.get().auth().getAwsRegion(), Utils.asString(value))}>{value}</Link>
            case 's3:bucket-name':
                return <Link external={true} href={Utils.getS3BucketUrl(AppContext.get().auth().getAwsRegion(), Utils.asString(value))}>{value}</Link>
            case 'link':
                return <Link href={Utils.asString(value)}>{value}</Link>
            case 'external-link':
                return <Link external={true} href={Utils.asString(value)}>{value}</Link>
        }
