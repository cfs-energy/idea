...
import {
    IdeaFormField,
    IdeaFormFieldRegistry,
    IdeaFormFieldStateChangeEvent,
    IdeaFormFieldLifecycleEvent
} from "../form-field";
...
class IdeaWizard extends Component<IdeaWizardProps, IdeaWizardState> {
    ...
    triggerReset(params: string[]) {
        ...
        Promise.all(fetchOptions).then((fields: IdeaFormField[]) => {
    ...
    buildReviewParams(section: SocaUserInputSectionMetadata, group?: SocaUserInputGroupMetadata) {
        ...
        if (group) {
            return (
                <Container key={key} variant={"stacked"} header={
                    <Header variant={"h3"} description={group.description}>{group.title}</Header>
                }>
                    <ColumnLayout
                        columns={3}
                        variant="text-grid">
                        {
                            params?.filter((param) => {
                                return this.state.isVisible(param.name!)
                                    ...
                                    return <IdeaFormReviewField key={this.paramKey(section, param, 'review', group)}
                                                                param={param}
                                                                value={getReviewFieldValue(param.name!)}
                                    />
                                }
                            )}
                    </ColumnLayout>
            ...
            return (
                <Box padding={{bottom: "xl"}}>
                    <SpaceBetween size={"l"}>
                        <Header variant="h2"
                                actions={
                                    <Button onClick={() => this.setActiveStep(index)}>
                                        Edit
                                    </Button>
                                } key={uuid()}>
                            {section.title}
                        ...
                        <Box>
                            {section.groups?.map((group) => {
                                return (
                                    this.buildReviewParams(section, group)
                                )
                            })}
                        </Box>
                    </SpaceBetween>
    ...
    buildReviewStep(): WizardProps.Step {
        return {
            ...
            content: (
                <SpaceBetween size="s">
                    {this.props.onReview && <Box textAlign="right">
    ...
    buildParams(section: SocaUserInputSectionMetadata, group?: SocaUserInputGroupMetadata) {
        ...
        return (
            <Container key={key}
                       header={<Header variant="h2" description={description}>
                           {title}
                       </Header>}>
                <ColumnLayout
                    columns={1}>
