...
export class SharedStorageFileSystem {
        ...
        if (Utils.isEmpty(title)) {
            ...
            switch (name) {
                case 'apps':
                    ...
                    break
                case 'data':
                    title = 'Data (User Home Directories)'
                    break
                default:
                    title = name
                    break
    ...
    getMountTarget(): string {
        if(this.isFsxWindowsFileServer()) {
            return `${this.getMountDrive()}:`
        }
        if(this.isFsxNetAppOntap()) {
            const securityStyle = this.getVolumeSecurityStyle()
            switch (securityStyle) {
                case 'MIXED':
                    return `${this.getMountDirectory()} | ${this.getMountDrive()}:`
                case 'NTFS':
                    return `${this.getMountDrive()}:`
                case 'UNIX':
                    return this.getMountDirectory()
            }
        }
        return this.getMountDirectory()
    }
