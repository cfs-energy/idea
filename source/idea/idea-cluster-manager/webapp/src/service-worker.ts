    ...
    if (event.data && event.data.type) {
        switch (event.data.type) {
            ...
            case Constants.ServiceWorker.IDEA_AUTH_ACCESS_TOKEN:
                IDEA_AUTH_CONTEXT.getAccessToken().then(accessToken => {
                    event.ports[0].postMessage({
            ...
            case Constants.ServiceWorker.IDEA_AUTH_IS_LOGGED_IN:
                IDEA_AUTH_CONTEXT.isLoggedIn().then(status => {
                    event.ports[0].postMessage({
                        status: status
                    })
                }).catch(error => {
            ...
            case Constants.ServiceWorker.IDEA_AUTH_LOGOUT:
                IDEA_AUTH_CONTEXT.logout().then(status => {
                    event.ports[0].postMessage({
                        status: status
                    })
                }).catch(error => {
            ...
            case Constants.ServiceWorker.IDEA_HTTP_FETCH:
                IDEA_AUTH_CONTEXT.fetch(
                    event.data.options.url,
                    event.data.options.options,
                    (typeof event.data.options.isPublic !== 'undefined') ? event.data.options.isPublic : false
                ).then(response => {
                    event.ports[0].postMessage({
                        response: response
                    })
            ...
            case Constants.ServiceWorker.IDEA_API_INVOCATION:
                if (event.data.options.request.header.namespace === 'Auth.InitiateAuth') {
                    IDEA_AUTH_CONTEXT.initiateAuth(event.data.options.request).then(result => {
                        event.ports[0].postMessage({
                            response: result
                        })
                    }).catch(error => {
