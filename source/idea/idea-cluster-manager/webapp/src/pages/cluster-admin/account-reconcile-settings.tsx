...
export default function AccountReconcileSettings({settings}: {settings: any}) {
    ...
    return <Container header={<Header variant="h2" description="Keep IDEA user state aligned with the directory. Protected accounts are excluded.">Account reconciliation</Header>}>
        <SpaceBetween size="l">
            ...
            <FormField label="Okta org URL" description="Optional HTTPS origin; set both Okta fields or leave both empty."><Input ariaLabel="Okta org URL" value={values.okta.org_url ?? ''} disabled={busy} onChange={e => setValues({...values, okta: {...values.okta, org_url: e.detail.value}})}/></FormField>
