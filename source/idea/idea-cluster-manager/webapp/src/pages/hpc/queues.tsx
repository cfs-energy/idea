                ...
                <div>
                    {
                        queue.projects?.map((project, index) => {
                            return <li key={index}>{project.title} ({project.name})</li>
                        })
                    }
                </div>
            )
...
class Queues extends Component<QueuesProps, QueuesState> {
                    ...
                    },
                    {
                        ...
                        onClick: () => {
                            if (this.isSelectedQueueProfileEnabled()) {
                                this.schedulerAdmin().disableQueueProfile({
                                ...
                                this.schedulerAdmin().enableQueueProfile({
                                    queue_profile_id: this.getSelected()?.queue_profile_id
                                }).then(_ => {
                                    this.setState({
                                        queueSelected: false
    ...
    buildSplitPanelContent() {
        ...
        return (
            this.isSelected() &&
            <IdeaSplitPanel
                title={`Queue Profile: ${this.getSelected()?.name}`}
            >
                <Tabs
                    tabs={[
                        {
                        ...
                        },
                        {
                            ...
                            content: (
                                <ColumnLayout columns={3}>
                                    <KeyValue title="Allowed Instance Types"
                                              value={queueParams().allowed_instance_types}/>
                                    <KeyValue title="Excluded Instance Types"
                                              value={queueParams().excluded_instance_types}/>
                                    <KeyValue title="Restricted Parameters"
                                              value={queueParams().restricted_parameters}/>
                                    <KeyValue title="Allowed Instance Profiles"
                                              value={queueParams().allowed_instance_profiles}/>
                                    <KeyValue title="Allowed Security Groups"
                                              value={queueParams().allowed_security_groups}/>
                                </ColumnLayout>
                            )
                        },
                        {
                        ...
                        },
                        {
                            ...
                            content: (
                                <ColumnLayout columns={2}>
                                    ...
                                    {queueUtils().isScratchNewFsxLustre() && <KeyValueGroup title="Scratch Storage: New FSx for Lustre">
