...
class UpdateHpcApplication extends Component<UpdateHpcApplicationProps, UpdateHpcApplicationState> {
    ...
    componentDidMount() {
        if (this.isUpdate()) {
            ...
            this.schedulerAdmin().getHpcApplication({
                        ...
                        steps={[
                            {
                                ...
                                info: <Link variant="info" onFollow={() => this.showHelp('job-design-form')}>Info</Link>,
                                content: (
                                    <IdeaFormBuilder
                                        ref={this.formBuilder}
                                        params={this.getRecommendedJobParameters()}
                                        infoSection={this.isCreate() &&
                                            <div>
                                                <p>We have generated a test job submission form for you. You can customize it as needed or
                                                    click <strong>Reset Form</strong> to start from scratch.</p>
                                            </div>
                                        }
                                    />
                                )
                                    ...
                                    <Container variant="default">
                                        <SpaceBetween direction="vertical" size="xl">
                                            ...
                                            <FormField
                                                ...
                                                description="Below variables are available for use in your job script, based on the form designed in the previous step."
                                            >
                                                <ul>
                                                    <li><AvailableVariable name="project_name"
                                                                           isReferenced={isVariableReferencedInScript('project_name')}
                                                                           isDefined={this.isVariableDefined('project_name')}
                                                                           required={true}
                                                                           description={"included automatically"}/></li>
                                                    <li><AvailableVariable name="cpus" isReferenced={isVariableReferencedInScript('cpus')}
                                                                           required={true}
                                                                           isDefined={this.isVariableDefined('cpus')}
                                                                           description={"used for node count computation"}/></li>
                                                    {this.isFormDesigned() && this.getDesignedFormVariables().map(param => {
                                                        ...
                                                        return <li key={param.name}>
                                                            <AvailableVariable name={param.name}
                                                                               isReferenced={isVariableReferencedInScript(param.name)}
                                                                               isDefined={this.isVariableDefined(param.name)}
                                                                               required={Utils.isTrue(param.validate?.required)}/>
                                                        </li>
                                                    })}
                                                </ul>
                                            </FormField>
                                            <FormField
                                                label="Job Script"
                                                description="Build the job script template based on your simulation requirements"
                                                stretch={true}
                                            >
                                                <SpaceBetween size="xs" direction="vertical">
                                                    ...
                                                    />
                                                    <Button variant="normal" onClick={this.onResetJobScript}>Reset Job Script</Button>
                                                </SpaceBetween>
                                            </FormField>
                                        </SpaceBetween>
                                    </Container>
                                    ...
                                    <ColumnLayout columns={2}>
                                        <Container variant="default">
                                            <SpaceBetween size="m" direction="vertical">
                                                ...
                                                <FormField label="Select Thumbnail"
                                                           description="Select an image (.jpg or .png) to use as thumbnail">
                                                    <Box>
                                                        <input type="file"
                                                    ...
                                                    {this.state.thumbnailData && <Box padding="m">
