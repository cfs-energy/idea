...
class UpdateHpcLicense extends Component<UpdateHpcLicenseProps, UpdateHpcLicenseState> {
    ...
    componentDidMount() {
        ...
        if (this.isUpdate()) {
            ...
            this.schedulerAdmin().getHpcLicenseResource({
                name: name
            }).then(result => {
                this.setState({
    ...
    loadSchedulerInstanceId() {
        AppContext.get().getClusterSettingsService().getModuleSettings(Constants.MODULE_SCHEDULER).then(settings => {
            this.setState({
                        ...
                        steps={[
                            ...
                            },
                            {
                                ...
                                content: (
                                    <SpaceBetween size={"l"}>
                                        <Alert>
                                            A few manual configuration steps are required to configure the license resource in OpenPBS. Review the License Resource definition and follow the steps outlined below.
                                        </Alert>
                                        <Container header={<Header variant={"h3"} actions={<SpaceBetween size={"s"}>
                                            <Button variant={"normal"} onClick={() => {
                                                this.setState({
                                                    activeStepIndex: 0
                                                })
                                            }}>Edit</Button>
                                        </SpaceBetween>}>License Resource</Header>}>
                                        ...
                                        <Container header={<Header variant={"h3"}>SSH into Scheduler EC2 Instance</Header>}>
                                            <ColumnLayout columns={2}>
                                                <Box>
                                                    <h3>SSH via SSM</h3>
                                                    <p><Link external={true} href={Utils.getSessionManagerConnectionUrl(AppContext.get().auth().getAwsRegion(), this.state.schedulerInstanceId)}>Connect via SSM</Link></p>
                                                </Box>
                                                <Box>
                                                    <h3>SSH via Bastion Host</h3>
                                                    <p><Link external={true} href={'#/home/ssh-access'}>SSH Connection Details</Link></p>
                                                </Box>
                                            </ColumnLayout>
                                        </Container>
