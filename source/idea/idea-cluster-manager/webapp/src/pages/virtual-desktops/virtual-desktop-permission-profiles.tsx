...
class VirtualDesktopPermissionProfiles extends Component<VirtualDesktopPermissionProfilesProps, VirtualDesktopPermissionProfilesState> {
    ...
    componentDidMount() {
        ...
        this.getVirtualDesktopUtilsClient().getBasePermissions({}).then(response => {
            this.setState({
        ...
        return (
            <VirtualDesktopPermissionProfileForm
                ...
                onSubmit={(permissionProfile: VirtualDesktopPermissionProfile) => {
                    return this.getVirtualDesktopAdminClient().createPermissionProfile({
                            profile: permissionProfile
                        }
                    ).then(_ => {
                        this.setFlashMessage(
                            <p key={permissionProfile.profile_id}>Permission Profile: {permissionProfile.profile_id}, Create request submitted</p>, "success"
                        )
    ...
    buildEditPermissionProfileForm() {
        return (
            <VirtualDesktopPermissionProfileForm
                ...
                onSubmit={(permissionProfile: VirtualDesktopPermissionProfile) => {
                    return this.getVirtualDesktopAdminClient().updatePermissionProfile({
                            profile: permissionProfile
                        }
                    ).then(_ => {
                        this.setFlashMessage(
                            <p key={permissionProfile.profile_id}>Permission Profile: {permissionProfile.profile_id}, Edit request submitted</p>, "success"
                        )
                        this.getListing().fetchRecords()
                        return Promise.resolve(true)
                    }).catch(error => {
    ...
    buildListing() {
        return (
            <IdeaListView
                ...
                onFetchRecords={() => {
                    return this.getVirtualDesktopUtilsClient().listPermissionProfiles({})
                        .catch(error => {
                            this.props.onFlashbarChange({
                                items: [
                                    {
