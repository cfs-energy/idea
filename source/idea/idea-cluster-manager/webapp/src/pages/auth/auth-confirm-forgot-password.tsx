    ...
    onSubmit() {
        this.setState({
            ...
            if (this.getForm().validate()) {
                ...
                AppContext.get().auth().confirmForgotPassword(values.verificationCode, values.password).then(ok => {
                    if (ok) {
                        this.props.navigate('/')
                    } else {
        ...
        return <AuthLayout
            content={
                <ColumnLayout columns={1} className="auth-content">
                    ...
                    <p className="description">We've sent a verification code to your account's email address.
                        Please enter the verification code to reset your password.</p>
                    ...
                    <SpaceBetween size={"xs"} direction={"vertical"} className="actions">
                        {!this.state.loading && <div>
                            ...
                            <p>Did not receive verification email? Please verify if the username is valid or if
                                the email is marked as Spam or Junk. If not, click Resend Verification Email link below:</p>
