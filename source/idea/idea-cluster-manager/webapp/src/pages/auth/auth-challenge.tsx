    ...
    onSubmit = () => {
        this.setState({
            ...
            if (this.getForm().validate()) {
                ...
                AppContext.get().auth().respondToAuthChallenge(values.password).then(_ => {
                    this.props.navigate('/')
                }).catch(error => {
            ...
            content={
                <ColumnLayout columns={1} className="auth-content">
                    ...
                    <SpaceBetween size={"xs"} direction={"vertical"} className="actions">
                        {!this.state.loading && <Button variant="primary" onClick={() => {
                            this.onSubmit()
                        }} disabled={!this.state.canSubmit}>Change Password</Button>}
