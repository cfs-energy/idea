...
class ClusterManagerUtils:
    ...
    def check_duplicate_users(
        usernames: List[str], context: SocaCliContext
    ) -> List[str]:
        duplicates = []
    ...
    @staticmethod
    def create_users(
        users: List[User], context: SocaCliContext
    ) -> List[Tuple[User, str]]:
        ...
        for entry in users:
            try:
                context.unix_socket_client.invoke_alt(
                    ...
                    payload=CreateUserRequest(user=entry),
    ...
    @staticmethod
    def print_create_user_status(
        users: List[Tuple[User, str]], context: SocaCliContext
    ):
        num_tries = 0
        while True:
            all_done = True
            ...
            for entry in users:
                if Utils.are_equal(entry[1], 'n/a'):
                    response = context.shell.invoke(
                        cmd=f'id {entry[0].username}', shell=True
                    )
                    if response.returncode == 0:
                        table.add_row(
                            entry[0].username,
                            entry[0].email,
                            Utils.get_as_string(entry[0].sudo, default='False'),
                            'SUCCESS',
                            'n/a',
                            style='green',
                        )
                        ...
                        table.add_row(
                            entry[0].username,
                            entry[0].email,
                            Utils.get_value_as_string(entry[0].sudo, default='False'),
                            'IN PROGRESS',
                            'n/a',
                        )
                        all_done = False
    ...
    def check_has_valid_csv_headers(headers, context: SocaCliContext):
        if len(headers) < 2 or len(headers) > 3:
            context.error(
                'Incorrect number of columns provided; run with --generate-template to see an example'
            )
            raise SystemExit(1)
        elif len(headers) == 2:
            if 'Username' not in headers or 'Email' not in headers:
                context.error(
                    '"Username" and "Email" fields must be provided in the header; "Is Admin?" is optional; run with --generate-template to see an example'
                )
                raise SystemExit(1)
        elif len(headers) == 3:
            if (
                'Username' not in headers
                or 'Email' not in headers
                or 'Is Admin?' not in headers
            ):
                context.error(
                    '"Username" and "Email" fields must be provided in the header; "Is Admin?" is optional; run with --generate-template to see an example'
                )
    ...
    def check_if_csv_file(filepath: str, context: SocaCliContext):
        if not Utils.is_file(filepath):
            context.error(
                f'This path {filepath} is not a file; Please provide a csv file'
            )
            raise SystemExit(1)
        elif not filepath.endswith('.csv'):
            context.error(
                f'This file: {filepath} is not a csv file; Please provide a csv file'
            )
            raise SystemExit(1)
