...
class GroupMembersDAO:
    ...
    def create_membership(self, group_name: str, username: str):
        ...
        self.table.put_item(Item={'group_name': group_name, 'username': username})
    ...
    def delete_membership(self, group_name: str, username: str):
        ...
        self.table.delete_item(Key={'group_name': group_name, 'username': username})
    ...
    def list_users_in_group(
        ...
        self.logger.debug(
            f'list_users_in_group - PageSize: {request.page_size} - Listing users in groups: {group_names}'
        )
        ...
        if Utils.is_not_empty(cursor):
            ...
            self.logger.debug(
                f'list_users_in_group - Using exclusive_start_keys: {exclusive_start_keys}'
            )

        for group_name in group_names:
            exclusive_start_key = Utils.get_value_as_dict(
                group_name, exclusive_start_keys, default={}
            )
            self.logger.debug(
                f'list_users_in_group - Group {group_name} - Exclusive_start_key: {exclusive_start_key}'
            )
            ...
            self.logger.debug(
                f'list_users_in_group - Group {group_name} - db_user_groups: {db_user_groups}'
            )
            ...
            for db_user_group in db_user_groups:
                ...
                if db_username in username_set:
                    self.logger.debug(
                        f'list_users_in_group - Group {group_name} - User {db_username} already listed'
                    )
                    continue
                self.logger.debug(
                    f'list_users_in_group - Adding user {db_username} to group {group_name}'
                )
                username_set.add(db_username)
                db_user = self.user_dao.get_user(db_username)
                self.logger.debug(
                    f'list_users_in_group - Read Db_user entry for: {db_user}'
                )
                if db_user is None:
                    continue
                user = self.user_dao.convert_from_db(db_user)
                self.logger.debug(
                    f'list_users_in_group - DB_user entry for: {user} (adding)'
                )
                users.append(user)
            ...
            self.logger.debug(
                f'list_users_in_group - Group {group_name} - last_evaluated_key: {last_evaluated_key}'
            )
        ...
        self.logger.debug(
            f'list_users_in_group - response_cursor: {response_cursor}: Users: {users}'
        )

        _query_end = Utils.current_time_ms()
        self.logger.debug(
            f'list_users_in_group - Listing completed for {group_names}. - Query took: {_query_end - _query_start}ms'
        )
