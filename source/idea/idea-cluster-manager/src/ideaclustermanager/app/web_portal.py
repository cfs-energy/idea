...
class WebPortal:
    ...
    def get_logo_url(self) -> str:
        ...
        return self.make_route_path('/logo.png')  # default IDEA logo in public directory
    ...
    def build_app_init_data(self, http_request) -> Dict:
        ...
        app_init_data = {
            ...
            'title': self.context.config().get_string('cluster-manager.web_portal.title', 'Integrated Digital Engineering on AWS'),
    ...
    async def index_route(self, http_request):
        ...
        result = template.render(app_init_data=Utils.base64_encode(Utils.to_json(app_init_data)))
    ...
    async def sso_oauth2_callback_route(self, http_request):
        ...
        try:
            ...
            if existing_user is None:
                self.logger.warning(f'SSO auth: {username} is not previously known. User must be created before first SSO attempt')
