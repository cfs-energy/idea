...
describe('CdkInvoker.invoke', () => {
  ...
  it('renders and uploads the bootstrap package, and passes its uri as a context param', async () => {
    ...
    // The archive is named by the rendered tree's content id, not by the deployment id, so an
    // unchanged package keeps its location across deploys.
    const uploaded = [...deps.puts.keys()].find((put) => put.startsWith('sample-bucket/idea/bootstrap/bootstrap-directoryservice-'));
    assert.ok(uploaded !== undefined, 'the bootstrap archive was not uploaded');
    const key = uploaded.slice('sample-bucket/'.length);
    assert.match(key, /^idea\/bootstrap\/bootstrap-directoryservice-[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}\.tar\.gz$/);
    assert.notEqual(key, `idea/bootstrap/bootstrap-directoryservice-${DEPLOYMENT_ID}.tar.gz`);
    assert.ok(
