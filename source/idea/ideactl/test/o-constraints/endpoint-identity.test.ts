 ...
 * three are pinned, with the flag off and with the flag on.
 */
...
/** Synthetic stand-ins for the ARNs the ECS stack publishes. */
const TARGET_GROUP = (name: string): { S: string } => ({
  S: `arn:aws:elasticloadbalancing:us-east-2:123456789012:targetgroup/${name}/1111111111111111`,
});
...
const CASES: Array<{
  ...
  flagOn: Record<string, { S: string } | { BOOL: boolean } | { L: Array<{ S: string }> }>;
...
}> = [
  {
    ...
    flagOn: {
      ...
      "ecs.cluster-manager.service_arn": { S: "service/cluster-manager-service" },
      "ecs.cluster-manager.target_group_arns": {
        L: [TARGET_GROUP("cm-ecs-e"), TARGET_GROUP("cm-ecs-i"), TARGET_GROUP("cm-ecs-w")],
      },
    },
    expected: {
  ...
  },
  {
    ...
    flagOn: {
      "ecs.enabled": { BOOL: true },
      "ecs.scheduler.service_arn": { S: "service/scheduler-service" },
      "ecs.scheduler.target_group_arns": { L: [TARGET_GROUP("sched-ecs-e"), TARGET_GROUP("sched-ecs-i")] },
    },
    expected: {
  ...
  },
  {
    ...
    flagOn: {
      ...
      "ecs.vdc.target_group_arns": { L: [TARGET_GROUP("vdc-ecs-e"), TARGET_GROUP("vdc-ecs-i")] },
      "ecs.dcv-broker.service_arn": { S: "service/dcv-broker-service" },
      "ecs.dcv-broker.target_group_arns": {
        L: [TARGET_GROUP("broker-ecs-c"), TARGET_GROUP("broker-ecs-a"), TARGET_GROUP("broker-ecs-g")],
      },
      "ecs.dcv-gateway.service_arn": { S: "service/dcv-gateway-service" },
      "ecs.dcv-gateway.target_group_arns": { L: [TARGET_GROUP("gw-ecs-TN"), TARGET_GROUP("gw-ecs-TUN")] },
    },
    expected: {
