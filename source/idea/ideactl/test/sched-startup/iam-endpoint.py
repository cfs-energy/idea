...
def allowed(action, resource):
    ...
    for statement in POLICY["Statement"]:
        if not any(glob_matches(a, action) for a in as_list(statement.get("Action"))):
            continue
        if not any(glob_matches(r, resource) for r in as_list(statement.get("Resource"))):
            continue
...
class Handler(BaseHTTPRequestHandler):
    ...
    def do_POST(self):
        ...
        try:
            if target.startswith("DynamoDB_"):
                ...
                self.send_json(500, {"message": f"unhandled request {target or self.path}"})
    ...
    def route53(self, zone):
        ...
        if not allowed(action, resource):
            log(f"DENY  {action} {resource}")
            self.send_xml(403, "<ErrorResponse><Error><Code>AccessDenied</Code></Error></ErrorResponse>")
            return
        log(f"ALLOW {action} {resource}")
    ...
    def ssm(self, operation):
        ...
        self.send_json(200, {"Command": {"CommandId": "00000000-0000-0000-0000-000000000000"}})
        ...
        for attempt in range(3):
            try:
                with urllib.request.urlopen(request, timeout=20) as response:
                    self.respond(response.status, response.read(), "application/x-amz-json-1.0")
                return
            except urllib.error.HTTPError as error:
                self.respond(error.code, error.read(), "application/x-amz-json-1.0")
                return
            except OSError as error:
                log(f"RETRY proxy attempt {attempt + 1}: {type(error).__name__}: {error}")
