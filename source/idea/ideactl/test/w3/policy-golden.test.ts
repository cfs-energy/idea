...
const ATTRIBUTED: Attribution[] = [
  ...
  { stack: 'cluster', path: 'update-cluster-prefix-list-policy/Resource', dev27Id: 'updateclusterprefixlistpolicy57E07488', template: 'custom-resource-update-cluster-prefix-list.yml', conditional: true },
  ...
  { stack: 'identity-provider', path: 'oauth-credentials-policy/Resource', dev27Id: 'oauthcredentialspolicy92D4C6F2', template: 'custom-resource-get-user-pool-client-secret.yml' },
...
describe('the classification itself', () => {
  ...
  it('names a policy template that exists for every template-rendered entry', () => {
    ...
    const missing = ATTRIBUTED.filter((entry) => entry.template !== undefined && !existsSync(join(dir, entry.template))).map((entry) => entry.template);
...
describe('renderPolicy vs the deployed dev27 templates', () => {
  ...
  it('renders every template-rendered policy identically', () => {
    ...
    assert.equal(matched.length + absent.length, ATTRIBUTED.filter((entry) => entry.template !== undefined).length);
  });
});
