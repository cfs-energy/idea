...
function configWith(overrides: Record<string, string | string[]>): string {
  const attribute = (value: string | string[]): Json =>
    Array.isArray(value) ? { L: value.map((entry) => ({ S: entry })) } : { S: value };
...
describe('cluster-manager removal policies', () => {
  test('every resource keeps the deployed DeletionPolicy and UpdateReplacePolicy', async () => {
    ...
    assert.deepEqual(removalPolicies(template), removalPolicies(withRetainedStateful(readJson(LIVE_TEMPLATE))));
  });
...
describe('cluster-manager branches', () => {
  ...
  test('the cdk-nag suppressions are the deployed ones, resource for resource', async () => {
    ...
    const live = nagSuppressions(readJson(LIVE_TEMPLATE));
    ...
    assert.deepEqual(live.detachprojectboundarieslambdaBF006DAF, [
      ['AwsSolutions-L1', 'Lambda runtime uses Python 3.13 by default.'],
      ['AwsSolutions-L1', 'Python Runtime is selected for stability.'],
    ]);
  ...
  test("ecs.retain_existing_hosts routes the endpoints and keeps the host shape", async () => {
    ...
    // the host group is still there to go back to. It registers with nothing, because the container
    // target groups take IP targets.
    const externalTargetGroup = "cluster-manager-external-target-group";
    const internalTargetGroup = "cluster-manager-internal-target-group";
    const webPortalTargetGroup = "cluster-manager-web-portal-target-group";
    const { template } = await synthClusterManager(
      configWith({
        "ecs.enabled": "true",
        "ecs.retain_existing_hosts": "true",
        "ecs.cluster-manager.service_arn": "service/cluster-manager-service",
    ...
    assert.deepEqual(resources.webportalendpoint.Properties.actions, [
      { Type: "forward", TargetGroupArn: webPortalTargetGroup },
    ]);
    assert.deepEqual(resources.externalendpoint.Properties.actions, [
      { Type: "forward", TargetGroupArn: externalTargetGroup },
    ]);
    assert.deepEqual(resources.internalendpoint.Properties.actions, [
      { Type: "forward", TargetGroupArn: internalTargetGroup },
    ]);
  });
  ...
  test("ecs.enabled removes the host shape and repoints the retained endpoints", async () => {
    ...
    const internalTargetGroup = "cluster-manager-internal-target-group";
    const webPortalTargetGroup = "cluster-manager-web-portal-target-group";
    const serviceArn = "service/cluster-manager-service";
    const { template } = await synthClusterManager(
      configWith({
        "ecs.enabled": "true",
        "ecs.cluster-manager.service_arn": serviceArn,
        "ecs.cluster-manager.target_group_arns": [
          externalTargetGroup,
          internalTargetGroup,
          webPortalTargetGroup,
    ...
    // The container stack creates its own task roles, so the host role never trusts the task
    ...
    assert.equal(settings.asg_name, "cluster-manager-service");
    assert.equal(settings.asg_arn, serviceArn);
