...
def build_tree(scan_file, overrides):
    ...
    entries = {
        item['key']['S']: decode(item['value']) for item in items if 'value' in item
    }
...
class ConfigStub:
    ...
    def __init__(self, tree, module_set=DEFAULT_MODULE_SET):
        ...
        self.module_info = (
            None  # render_policy is called on a config with no current module
        )
...
class Utils:
    ...
    def to_yaml(payload, sort_keys=False, width=140):
        return yaml.dump(
            json.loads(json.dumps(payload)), sort_keys=sort_keys, width=width
        )
...
def main():
    ...
    env = Environment(
        loader=FileSystemLoader(searchpath=job['policies_dir'], followlinks=False),
        autoescape=False,
    )
    context = {
