...
function configWith(overrides: Record<string, string | boolean | string[]>): string {
  const attribute = (value: string | boolean | string[]): Json => {
...
describe('scheduler stack', () => {
  test('matches the deployed dev27 template resource for resource', async () => {
    ...
    assert.deepEqual(deployedResources(template), deployedResources(withRetainedStateful(readJson(LIVE_TEMPLATE))));
  });

  test('every resource carries the deletion policies the deployed template has', async () => {
    const { template } = await synthScheduler(deployedConfig());
    const live = retentionPolicies(withRetainedStateful(readJson(LIVE_TEMPLATE)));
    ...
    for (const id of ['schedulercreds', 'externalendpoint', 'internalendpoint', 'ideadev27schedulersettings']) {
...
describe('scheduler stack branches', () => {
  ...
  test('ecs.enabled needs no prior write of use_stable_server_name', async () => {
    ...
    const { template } = await synthScheduler(
      configWith({
        'ecs.enabled': true,
        'ecs.scheduler.target_group_arns': ['external-target-group', 'internal-target-group'],
        'scheduler.use_stable_server_name': false,
      }),
    );
    const statements = template.Resources.schedulerpolicyFF65A604.Properties.PolicyDocument
  ...
  test("ecs.retain_existing_hosts routes the endpoints and keeps the scheduler host", async () => {
    ...
    // the instance is still there to go back to. The DNS record still leaves the template, because
    // the running task owns that name from ECS_SCHEDULER_READY onward.
    const externalTargetGroup = "scheduler-external-target-group";
    const internalTargetGroup = "scheduler-internal-target-group";
    const { template } = await synthScheduler(
      configWith({
        "ecs.enabled": true,
        "ecs.retain_existing_hosts": true,
        "ecs.scheduler.target_group_arns": [externalTargetGroup, internalTargetGroup],
  ...
  test("ecs.enabled removes the scheduler host and keeps the service-facing resources", async () => {
    ...
    assert.equal(resources.externalendpoint.Properties.target_group_arn, externalTargetGroup);
    assert.deepEqual(resources.internalendpoint.Properties.actions, [
      { Type: "forward", TargetGroupArn: internalTargetGroup },
    ]);
    assert.equal(resources.internalendpoint.Properties.target_group_arn, internalTargetGroup);
