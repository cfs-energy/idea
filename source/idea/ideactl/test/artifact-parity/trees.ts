...
/** Repository root that the copy scripts walk to for administrator resources. */
export const REPOSITORY_ROOT = resolve(PACKAGE_ROOT, "..", "..", "..");

/** Directories the copy scripts take from the administrator resource tree. */
export const ADMINISTRATOR_GROUPS = [
  "cdk",
  "config",
  "input_params",
  "integration_tests",
...
export const CONTAINER_VALUES = join(PACKAGE_ROOT, "test", "shell-path", "values.yml");
...
/**
 * The checkout files the assemblers are supposed to ship: the six administrator
 * groups, the container overlay on `config/`, and the bootstrap tree.
 *
 * @returns relative paths in assembled-tree layout
 */
export function checkoutUnion(): string[] {
  const administrator = join(REPOSITORY_ROOT, "source", "idea", "idea-administrator", "resources");
  const overlay = join(PACKAGE_ROOT, "resources-ecs", "config");
  const bootstrap = join(REPOSITORY_ROOT, "source", "idea", "idea-bootstrap");
  const paths = new Set<string>();
  for (const group of ADMINISTRATOR_GROUPS) {
    for (const file of listFiles(join(administrator, group))) {
      paths.add(`${group}/${file}`);
    }
  }
  for (const file of listFiles(overlay)) {
    paths.add(`config/${file}`);
  }
...
/**
 * Writes a copy of `copy-resources.mjs` with the container overlay removed, then
 * runs it. Used to prove the comparison goes red when one assembler forgets a
 * directory outside the main resource tree.
 *
 * @param destination output resources directory
 */
export function assembleSabotagedCopyResources(destination: string): void {
  ...
  if (!original.includes(overlay)) {
    throw new Error("copy-resources overlay block not found; sabotage cannot remove it");
  }
