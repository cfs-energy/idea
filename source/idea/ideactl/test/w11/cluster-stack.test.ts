...
describe('cluster stack', () => {
  ...
  test('lambda assets go to the cluster bucket under cdk/', () => {
    ...
    assert.ok(destinations.length >= 7, `expected the six lambda assets and the template, got ${destinations.length}`);
  ...
  test('deletion policies match the deployed stack, resource for resource', () => {
    ...
    const liveResources = withRetainedStateful(live).Resources as Json;
    ...
    // and by name, so the assertion above cannot pass on an empty set. The custom resources hold
    // no state of their own and still delete in both directions.
    for (const id of [
      'ideadev27clusterexternalcert',
      'ideadev27clusterinternalcert',
      'ideadev27clusterclusterprefixlist',
      'ideadev27clustersettings',
    ]) {
      assert.equal(synthResources[id].DeletionPolicy, 'Delete', id);
      assert.equal(synthResources[id].UpdateReplacePolicy, 'Delete', id);
    }
