/**
 * Cutover blockers in the container stack.
 *
 * Two of them are proved by synthesizing the stack: target group names have to fit the longest
...
import { PUBLIC_CHECKOUT_ENV, requiredService } from "../support/fixtures.ts";
...
test("every target group name fits the longest cluster name a cluster can have", () => {
  ...
  const names = byType(template, "AWS::ElasticLoadBalancingV2::TargetGroup").map((targetGroup) => {
    const properties = record(targetGroup["Properties"], "target group must contain properties");
    const name = properties["Name"];
    if (typeof name !== "string") throw new Error("target group must carry a name");
    return name;
  });

  // Eleven: the gateway publishes the one target group matching the session protocol setting.
  // Both were created and both attached to the service, while the desktop stack attaches only the
  // matching one, so the other had no load balancer and the service could never be created.
  assert.equal(names.length, 11, "IP target groups");
...
test("the broker single address family option reaches the process that starts the broker", () => {
  const brokerEnvironment = roleEnvironment(template, "dcv-broker");
...
test("a virtual machine turns the broker option into the property it needs", { skip: javaSkip }, () => {
  ...
  const brokerOption = roleEnvironment(template, "dcv-broker")["JAVA_TOOL_OPTIONS"];
...
test("the host image and every task follow the configured family's architecture", () => {
  ...
  for (const { instanceType, parameter, cpu } of expected) {
    ...
    const taskDefinitions = byType(template, "AWS::ECS::TaskDefinition");
    assert.equal(taskDefinitions.length, 5, `${instanceType} application task definitions`);
