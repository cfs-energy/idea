...
import { fakeDeps, moduleRow } from "../w20a/harness.ts";
...
test("a host deploy uploads its rendered bootstrap package before synthesis", async () => {
  ...
  try {
    ...
    writeFileSync(join(downloads, "idea-cluster-manager-26.09.0.tar.gz"), "release");
    ...
    const releaseKey = "sample-bucket/idea/releases/idea-cluster-manager-26.09.0.tar.gz";
    const bootstrapKey = "sample-bucket/idea/bootstrap/bootstrap-cm-00000000-0000-4000-8000-000000000000.tar.gz";
    assert.ok(deps.puts.has(releaseKey));
    assert.ok(deps.puts.has(bootstrapKey));
    const spawn = deps.spawns[0] ?? [];
    assert.ok(spawn.includes("-c"));
    assert.ok(
      spawn.includes(
        "bootstrap_package_uri=s3://sample-bucket/idea/bootstrap/bootstrap-cm-00000000-0000-4000-8000-000000000000.tar.gz",
      ),
    );
