...
describe('identity-provider stack, cognito-idp provider', () => {
  test('matches the deployed dev27 template, asset hashes aside', async () => {
    ...
    assert.deepEqual(
      ...
      maskAssetHashes(deployedResources(withRetainedStateful(readJson(LIVE_TEMPLATE)))),
    );
  });
  ...
  test('the cdk-nag suppressions are the deployed ones, resource for resource', async () => {
    ...
    assert.deepEqual(live.idtokenclaim18B64AB5, [
      ['AwsSolutions-L1', 'Lambda runtime uses Python 3.13 by default.'],
      ['AwsSolutions-L1', 'Python Runtime is selected for stability.'],
    ]);
    assert.deepEqual(live.oauthcredentials54DBE54B, live.idtokenclaim18B64AB5);
  ...
  test('every deletion policy is what the live stack has, including the ones it does not have', async () => {
    ...
    for (const id of ['idtokenclaimroleF1630968', 'oauthcredentials54DBE54B']) {
  ...
  test('the cluster settings carry the five keys downstream modules read', async () => {
    const { template } = await synthIdentityProvider();
    ...
    assert.deepEqual(settings.Properties.settings['cognito.oauth_credentials_lambda_arn'], {
      'Fn::GetAtt': ['oauthcredentials54DBE54B', 'Arn'],
    });
  });
});
