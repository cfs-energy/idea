...
describe("the container host group", () => {
  ...
  it("exists before anything that can fail", () => {
    // Depending on the group alone is not enough. The release and the services are created in
    // parallel, and a failing service cancels the release's own policy, leaving the rollback
    // with nothing to clear the protection.
    const [releaseId] = resourcesOfType(template, "Custom::ReleaseScaleInProtection")[0] ?? [];
    assert.ok(releaseId !== undefined, "a release resource");
    const services = resourcesOfType(template, "AWS::ECS::Service");
    assert.ok(services.length > 0, "services to order against");
