...
before(() => {
  ...
  cpSync(join(PACKAGE_ROOT, "test", "shell-path", "values.yml"), VALUES_FILE);
...
test("clean runtime prints its version and renders configuration", () => {
  ...
  assert.match(about.stdout, /^ideactl 26\.09\.0$/m);
  ...
  assert.match(deploymentCli.stdout, /^2\.1137\.0/m);
