  ...
  for (const host of hosts) {
    test(`${host.moduleName} renders, uploads, and passes every package location`, async () => {
      ...
      for (const expected of host.packages) {
        const archive = `${expected.basename}.tar.gz`;
...
const contextParameterCases = [
  {
    ...
    python: /'bootstrap_package_uri': bootstrap_package_uri/,
    modules: ["directoryservice", "cluster-manager", "scheduler", "bastion-host"],
  },
  {
    name: "controller_bootstrap_package_uri",
    python: /'controller_bootstrap_package_uri': controller_bootstrap_package_uri/,
    modules: ["virtual-desktop-controller"],
  },
  {
    name: "dcv_broker_bootstrap_package_uri",
    python: /'dcv_broker_bootstrap_package_uri': dcv_broker_package_uri/,
    modules: ["virtual-desktop-controller"],
  },
  {
    name: "dcv_connection_gateway_package_uri",
    python: /'dcv_connection_gateway_package_uri': dcv_connection_gateway_package_uri/,
    modules: ["virtual-desktop-controller"],
  },
] as const;
...
for (const parameter of contextParameterCases) {
  test(`"${parameter.name}" is the Python context parameter`, () => {
    assert.match(pythonInvoker, parameter.python);
    assert.ok(
