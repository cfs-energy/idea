...
const ECS_SETTINGS_TEMPLATE = fileURLToPath(
  new URL('../../resources/config/templates/ecs/settings.yml', import.meta.url),
);
...
describe('the ASCII quoter is shared, not copied', () => {
  ...
  it('is the only definition left in the handler that asked for it', () => {
    for (const handler of ['../../src/lambda/idea_ec2_state_event_transformation_lambda/index.ts']) {
