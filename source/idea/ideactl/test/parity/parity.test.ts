...
test("oracle guard detects a create and remove cycle", async () => {
  ...
  try {
    ...
    // Under suite load the watcher delivers late; wait for it to see the write before the file
    // goes, so the assertion is about detection rather than about timing.
    const deadline = Date.now() + 5000;
    while (guard.observedEvents() === 0 && Date.now() < deadline) await new Promise((r) => setTimeout(r, 20));
    rmSync(temporary);
