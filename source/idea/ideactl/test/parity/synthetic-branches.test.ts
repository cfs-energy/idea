...
function valuesFor(file: string): UserValues {
  const base = loadValuesFile(join(here, "fixtures-synthetic", BASE_FILE));
  if (file === BASE_FILE) return base;
  return { ...base, ...loadValuesFile(join(here, "fixtures-synthetic", file)) };
}

function yamlFiles(): string[] {
  return readdirSync(join(here, "fixtures-synthetic"))
...
describe("synthetic fixtures carry no live account data", () => {
  it("contains no production cluster name, foreign account id, or live hostname", () => {
    for (const name of yamlFiles()) {
      const text = readFileSync(join(here, "fixtures-synthetic", name), "utf-8");
