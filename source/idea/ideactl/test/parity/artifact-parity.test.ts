...
import { assembleCopyResources, assembleDockerfileResources, assembleSabotagedCopyResources, assembleShellResources, checkoutUnion, CONTAINER_VALUES, diffTrees, generateConfig, isDirectory, listFiles, missingFromCheckout, renderSamplePolicy, scratch, stageTool, treesAgree } from "./artifact-trees.ts";
...
test("current assemblers write the same resource files, including the container template", () => {
...
test("dropping the container template from one assembler makes the comparison and generate fail", async () => {
