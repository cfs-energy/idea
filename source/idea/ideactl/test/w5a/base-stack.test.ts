...
describe('IdeaBaseStack manifest parity', () => {
  test('stack name, description, terminationProtection, tags, synthesizer qualifier', async () => {
    ...
    assert.equal(template.Description, `ModuleId: metrics, Cluster: ${CLUSTER}, Version: 26.09.0`);
  ...
  test('cdk-nag runs by default and IDEA_ADMIN_ENABLE_CDK_NAG_SCAN=false switches it off', async () => {
    ...
    assert.deepEqual(names(withNag), ['AwsSolutionsChecks', 'RetainStatefulOnUpdateReplace']);
...
describe('common constructs', () => {
  test('cluster-settings trio: live logical ids and DependsOn', async () => {
    ...
    for (const field of ['FunctionName', 'Handler', 'Runtime', 'MemorySize', 'Timeout', 'Description', 'Tags']) {
...
describe('IdeaCodeAsset', () => {
  test('handler name and asset path resolution', () => {
    ...
    assert.equal(asset.lambdaHandler, 'idea_custom_resource_update_cluster_settings.handler.handler');
    const path = asset.assetPath();
    assert.ok(existsSync(path));
    // The asset root is zipped by `lambda.Code.fromAsset`, and the runtime imports
    // `<package>.handler`, so the package and the commons package both sit inside the root.
    assert.ok(
      existsSync(join(path, 'idea_custom_resource_update_cluster_settings', 'handler.py')),
      `asset root ${path} has no idea_custom_resource_update_cluster_settings/handler.py`,
    );
    assert.ok(
      existsSync(join(path, 'idea_lambda_commons')),
      `asset root ${path} has no idea_lambda_commons/`,
    );
  });
