...
import { harness } from '../w5c/harness.ts';
import type { Json } from '../w5c/harness.ts';
...
describe('IdeaCodeAsset packages the constructs reference', () => {
  ...
  for (const packageName of ['idea_custom_resource_create_tags', 'idea_custom_resource_get_ad_security_group']) {
    test(packageName, () => {
      // The asset root is what `lambda.Code.fromAsset` zips, so the handler package has to be a
      // directory inside it rather than the root itself.
      const root = new IdeaCodeAsset(packageName).assetPath();
      assert.ok(existsSync(join(root, packageName, 'handler.py')), `${root} has no ${packageName}/handler.py`);
    });
  }
});
