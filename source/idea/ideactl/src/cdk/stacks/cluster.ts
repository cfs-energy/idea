...
function requiredString(config: ClusterConfig, key: string): string {
  return config.getString(key, undefined, { required: true }) as string;
}
...
export class ClusterStack extends IdeaBaseStack {
  ...
  /**
   * The certificates are generated by the deploy tool now (`src/cli/certificates.ts`), which
   * publishes their ARNs as settings rows. These two resources stay for one release so a cluster
   * that has them can update them in place to the Node handler, which only finds and returns and
   * whose Delete does nothing. Both carry `Retain`, so nothing here can destroy a certificate a
   * listener is serving.
   *
   * Remove them, and the lambda and its role and policy, in the release after this one, once every
   * cluster has deployed this one.
   */
  ...
  /**
   * Anonymous telemetry is off and the handler now answers without sending anything. The function
   * itself stays for one release: job stacks created before the upgrade carry a SendAnonymousData
   * resource whose service token is this ARN, and a Delete against a missing service token wedges
   * their teardown. Delete this method, the policy and the published
   * `solution.solution_metrics_lambda_arn` row once no job stack carries that resource.
   */
  ...
  /**
   * The cluster prefix list is the one place administrators manage external access from. The
   * stack only creates the list: the configured client addresses are merged into it by the deploy
   * tool once this stack has published the list id, and entries are otherwise managed out of band
   * by `idea-admin utils cluster-prefix-list`. Nothing here ever removes an entry.
   */
  ...
  buildVpcEndpoints(): void {
    ...
    for (const service of this.config.getList<string>('cluster.network.vpc_gateway_endpoints', [])) {
      gatewayEndpoints[service] = new VpcGatewayEndpoint(this.context, this.stack, service, this.vpc);
    }
 ...
 * `forward` is carried over.
  ...
  buildClusterEndpoints(): void {
    ...
    const externalAcmCertificateArn = this.config.getString(
    ...
    const internalAcmCertificateArn = this.config.getString(
  ...
  buildClusterSettings(): void {
    ...
    if (!requiredBool(this.config, 'cluster.load_balancers.external_alb.certificates.provided')) {
      clusterSettings['load_balancers.external_alb.certificates.certificate_secret_arn'] = requiredString(
        this.config,
        'cluster.load_balancers.external_alb.certificates.certificate_secret_arn',
      );
      clusterSettings['load_balancers.external_alb.certificates.private_key_secret_arn'] = requiredString(
        this.config,
        'cluster.load_balancers.external_alb.certificates.private_key_secret_arn',
      );
      clusterSettings['load_balancers.external_alb.certificates.acm_certificate_arn'] = requiredString(
        this.config,
        'cluster.load_balancers.external_alb.certificates.acm_certificate_arn',
      );
    } else {
    ...
    clusterSettings['load_balancers.internal_alb.certificates.certificate_secret_arn'] = requiredString(
      this.config,
      'cluster.load_balancers.internal_alb.certificates.certificate_secret_arn',
    );
    clusterSettings['load_balancers.internal_alb.certificates.private_key_secret_arn'] = requiredString(
      this.config,
      'cluster.load_balancers.internal_alb.certificates.private_key_secret_arn',
    );
    clusterSettings['load_balancers.internal_alb.certificates.acm_certificate_arn'] = requiredString(
      this.config,
      'cluster.load_balancers.internal_alb.certificates.acm_certificate_arn',
    );
