...
The stopped desktop cleanup deletes virtual desktops that have sat stopped longer than a cutoff, and session records whose EC2 instance no longer exists, through the same delete path an administrator uses on the Sessions page.
...
The cleanup runs on the same 30-minute schedule as the other controller sweeps, with a short time budget per pass; a large cluster is walked to the end across passes.
...
| `keep_tags` | `['idea:keep', 'ideal:keep']` | Instance tag keys the cleanup honors. The exemption action below writes `idea:keep`; add a key only for automation outside IDEA that already tags instances. |
...
On **Virtual Desktops > Sessions**, select one or more sessions and choose **Actions > Exempt from Cleanup**. The reason you enter is shown on the session; with no reason, your username is used. **Clear Cleanup Exemption** removes it. Exempt sessions carry a "Cleanup exempt" badge in the list. The flag on the session record is what the cleanup checks; IDEA also records the exemption on the instance's `idea:keep` tag for automation that reads tags, and clears it with the exemption.

## Recommended rollout

1. Set `enabled` to true and leave `dry_run` on.
2. After a pass or two, read the virtual desktop controller log. Each pass logs a summary such as `stopped desktop cleanup pass (dry run): candidates=12 kept=2 no_stop_time=0 not_due=7 raced=0 deleted=2 orphans_cleaned=1 failed=0`, with one `[dry run] would delete session ...` line per desktop, naming the session, the owner and how long it has been stopped.
