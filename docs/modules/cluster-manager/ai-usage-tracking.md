...
* Usage reconciliation keeps each row's stored user and project attribution. A partial project teardown deletes rows only for projects resolved in that run; it does not remove unrelated or temporarily unresolvable project history.
