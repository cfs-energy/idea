...
1. Follow the complete [image build-and-push workflow](../../../../.github/workflows/build_push.yaml), including its linked development setup and module-build actions. Dispatch from your patch branch with separate `control_plane_image_name` and `scheduler_image_name` inputs, the target public `ecr_repository`, and the repository's ECR push role configured. It builds the release bundles, stages the DCV signing key, broker RPM, both gateway RPMs and DCV archives with checksums, pushes the PBS image first, then builds the control-plane image with `PBS_IMAGE` and the `dcv-packages` context. Both images are built for `linux/arm64,linux/amd64`; use a registry reachable from your cluster and verify the pushed image manifest supports its hosts. For private ECR, adapt the workflow's login/push steps to the target account and region as in [the migration preparation](move-to-containers.md#once-per-cluster).
