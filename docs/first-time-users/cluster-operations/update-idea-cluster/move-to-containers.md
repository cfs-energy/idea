...
New clusters already use the container control plane. Use this runbook once for an existing host-based cluster. `upgrade-cluster --drain` moves the bastion, cluster manager, scheduler, virtual desktop controller, DCV broker, and connection gateway to container tasks on a shared host pool. There is no separate migration command. Later upgrades replace tasks while preserving running jobs, with a brief scheduler submission and API interruption.
...
* The container bastion does not publish or use a host bootstrap package. Do not treat the absence of that package as an incomplete upgrade.
...
The change-set guard refuses definite replacements of any resource, conditional replacements of stateful resources, stateful removals and unrecognized custom-resource removals, with specific named exceptions such as retained task-definition revisions. It also refuses removal of VPCs, subnets, gateways, routes, route tables, and Elastic IPs. Review any refusal and its data or lifecycle impact before passing `--allow-replacement <LOGICAL_ID>` for that one entry. The bastion instance is removed during the move, so neither that removal nor later SSH task revisions require `--allow-replacement bastionhostinstance`.
