...
| Desktops | Recorded session intervals intersected with each day, multiplied by the instance rate. Deleting a legacy stopped desktop does not bill its stopped interval as running. An uncertain historical stop time is marked as estimated, and incomplete restart history remains a limitation. |
