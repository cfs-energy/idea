...
$IDEARevision = if ($Env:IDEA_REVISION) {$Env:IDEA_REVISION} else {"v25.06.2"}
...
if ($IDEADevMode -ne "") {
    if (Test-Path -Path "${ScriptDir}/IDEA_VERSION.txt") {
        $IDEADevMode="true"
    }
    else {
        $IDEADevMode="false"
...
if ($IDEADevMode -eq "true") {
    ...
    if ($VirtualEnv -eq "") {
        if (Test-Path -Path "$ScriptDir/venv") {
            ...
            Verify-Command "Get-Command" "Python Virtual Environment not detected. Install virtual environment to execute idea-admin.sh in dev mode." "source ${ScriptDir}/venv/bin/activate"
        }
    }
    IDEA_SKIP_WEB_BUILD=${IDEA_SKIP_WEB_BUILD:-'0'}
    $IDESkipWebBuild = if (IDEA_SKIP_WEB_BUILD) {$Env:IDEA_DEV_MODE} else {""}
